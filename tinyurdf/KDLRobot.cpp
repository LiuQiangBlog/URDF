...
Eigen::MatrixXd KDLRobot::dQuatToDxJac(const Eigen::VectorXd &quat)
{
    ...
    J << -quat(1), quat(0), -quat(3), quat(2), -quat(2), quat(3), quat(0), -quat(1), -quat(3), -quat(2), quat(1),
        quat(0);
    return J;
}
...
void KDLRobot::printChain()
{
    ...
    for (int i = 0; i < chain_.getNrOfSegments(); i++)
    {
        ...
        std::cout << "Segment xyz: " << chain_.getSegment(i).getFrameToTip().p.x() << " "
                  << chain_.getSegment(i).getFrameToTip().p.y() << " " << chain_.getSegment(i).getFrameToTip().p.z()
                  << std::endl;
        ...
        std::cout << "Joint axis: " << chain_.getSegment(i).getJoint().JointAxis().x() << " "
                  << chain_.getSegment(i).getJoint().JointAxis().y() << " "
                  << chain_.getSegment(i).getJoint().JointAxis().z() << std::endl;
        std::cout << "Joint origin: " << chain_.getSegment(i).getJoint().JointOrigin().x() << " "
                  << chain_.getSegment(i).getJoint().JointOrigin().y() << " "
