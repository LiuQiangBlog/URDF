...
void TinyURDFParser::init(tinyxml2::XMLDocument &urdf_xml)
{
    ...
    if (root != nullptr)
    {
        ...
        {
            throw std::runtime_error("[urdf_parsing] Root element should be named <robot> not <" +
...
void TinyURDFParser::buildKDLTree(const KinematicElement *current, const std::string &hook)
{
    ...
    if (current->TAG == "JOINT")
    {
        ...
        KDL::Rotation kdl_transform_rpy =
            KDL::Rotation::RPY(current_joint->rpy(0), current_joint->rpy(1), current_joint->rpy(2));
...
TinyURDFParser::Joint TinyURDFParser::parseXMLJoint(const tinyxml2::XMLElement *joint_xml)
{
    ...
    if (joint_xml->Attribute("name") != nullptr)
    ...
    if (joint_xml->Attribute("type") != nullptr)
    {
    ...
    if (joint_xml->FirstChildElement("parent")->Attribute("link") != nullptr)
    ...
    if (joint_xml->FirstChildElement("child")->Attribute("link") != nullptr)
    ...
    if (joint_xml->FirstChildElement("origin") != nullptr)
    {
        if (joint_xml->FirstChildElement("origin")->Attribute("rpy") != nullptr)
        {
            std::string rpy_raw = std::string(joint_xml->FirstChildElement("origin")->Attribute("rpy"));
            joint.rpy           = rawArrayToEigenVector(rpy_raw);
        }

        if (joint_xml->FirstChildElement("origin")->Attribute("xyz") != nullptr)
        {
    ...
    if ((joint.type == "revolute" || joint.type == "continuous" || joint.type == "prismatic" ||
         joint.type == "planar") &&
        joint_xml->FirstChildElement("axis") != nullptr)
    {
        if (joint_xml->FirstChildElement("axis")->Attribute("xyz") != nullptr)
        {
...
TinyURDFParser::Segment TinyURDFParser::parseXMLSegment(tinyxml2::XMLElement *segment_xml)
{
    ...
    for (tinyxml2::XMLElement *collision_elem = segment_xml->FirstChildElement("collision"); collision_elem != nullptr;
         collision_elem                       = collision_elem->NextSiblingElement())
    {
        Eigen::Vector3d rpy, xyz;

        if (collision_elem->FirstChildElement("origin") == nullptr ||
            collision_elem->FirstChildElement("geometry") == nullptr)
        {
            continue;
        }

        if (collision_elem->FirstChildElement("origin")->Attribute("rpy") != nullptr)
        {
            std::string rpy_raw = std::string(collision_elem->FirstChildElement("origin")->Attribute("rpy"));
            rpy                 = rawArrayToEigenVector(rpy_raw);
        }

        if (collision_elem->FirstChildElement("origin")->Attribute("xyz") != nullptr)
        {
        ...
        if (type == "box")
        {
            obj = new TinyURDFParser::Box(rawArrayToEigenVector(
                std::string(collision_elem->FirstChildElement("geometry")->FirstChildElement()->Attribute("size"))));
        }
        else if (type == "cylinder")
        {
            double radius =
                std::stod(collision_elem->FirstChildElement("geometry")->FirstChildElement()->Attribute("radius"));
            double length =
                std::stod(collision_elem->FirstChildElement("geometry")->FirstChildElement()->Attribute("length"));
...
{
    // Trim from end
    raw_array.erase(std::find_if(raw_array.rbegin(), raw_array.rend(),
                                 [](unsigned char ch)
                                 {
