...
Rotation::Rotation(double Xx, double Yx, double Zx, double Xy, double Yy, double Zy, double Xz, double Yz, double Zz)
{
    data[0] = Xx;
    data[1] = Yx;
    data[2] = Zx;
...
{
    return Vector(data[0] * v.data[0] + data[3] * v.data[1] + data[6] * v.data[2],
...
IMETHOD void Vector2::x(double _x)
{
    data[0] = _x;
}
IMETHOD void Vector2::y(double _y)
{
    data[1] = _y;
}
...
IMETHOD Frame2::Frame2(const Frame2 &arg) : p(arg.p), M(arg.M) {}
...
{
    return (Equal(a.data[0], b.data[0], eps) && Equal(a.data[1], b.data[1], eps) && Equal(a.data[2], b.data[2], eps));
}
...
IMETHOD Rotation Rot(const Vector &axis_a_b)
{
    ...
    double st = ::sin(angle);
    double vt = 1 - ct;
    return Rotation(ct + vt * rotvec(0) * rotvec(0), -rotvec(2) * st + vt * rotvec(0) * rotvec(1),
                    rotvec(1) * st + vt * rotvec(0) * rotvec(2), rotvec(2) * st + vt * rotvec(1) * rotvec(0),
                    ct + vt * rotvec(1) * rotvec(1), -rotvec(0) * st + vt * rotvec(1) * rotvec(2),
                    -rotvec(1) * st + vt * rotvec(2) * rotvec(0), rotvec(0) * st + vt * rotvec(2) * rotvec(1),
