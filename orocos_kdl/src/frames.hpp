...
public:
    ...
    /**  Gives back a vector in RPY coordinates, variables are bound by
         ...
         -  -PI/2 <= PITCH <= PI/2
             ...
             if pitch == PI/2 or pitch == -PI/2, multiple solutions for gamma and alpha exist.  The solution where
    roll==0 is chosen.
...
public:
    ...
    /// Spatial cross product for 6d motion vectors, beware all of them have to be expressed in the same reference
    /// frame/point
    inline friend Twist operator*(const Twist &lhs, const Twist &rhs);
    /// Spatial cross product for 6d force vectors, beware all of them have to be expressed in the same reference
    /// frame/point
 ...
 * \param dt [optional][obsolete] time interval over which the numerical differentiation takes place. By default this is
 * set to 1.0. \return rotation axis to rotate from b1 to b2, scaled by the rotation angle, expressed in frame a.
 ...
 * see also the corresponding diff() routine.
 * \param R_w_a Rotation matrix of frame a expressed to some frame w.
 * \param da_w  axis and angle of the rotation (da_w.rot), together with a displacement vector for the origin
 * (da_w.vel),  expressed to some frame w. \returns the frame resulting from the rotation of frame a by the axis and
 * angle specified with da.rot, and the translation of the origin da_w.vel .  The resulting frame is expressed with
 * respect to frame w.
 */
