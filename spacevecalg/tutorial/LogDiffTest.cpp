...
{
    {
        ...
        testd(0);
        for (int i = -6; i <= 6; ++i)
            testf(std::pow(10.f, static_cast<float>(i)));
        for (int i = -6; i <= 6; ++i)
            testf(-std::pow(10.f, static_cast<float>(i)));
        testf(0);
    }
    {
        ...
        // The two next computations do not use the taylor approximation yet, but are subtracting numbers quite close,
        // hence a small loss of accuracy
    ...
    {
        {
            ...
            // Ground truth numbers were computed with matlab vpa function and 100 digits precision (50 is not enough
            // for the smallest entries).

            testd(2., 0.00679694292146532720196937, 1e-14);
            testd(1., 0.0029151856912366650224031, 1e-13);
            // This is about the worst scenario: 0.267 ~ taylor_12_bound, but still using the general formula that
    ...
    }
    {
        for (int i = 0; i < 1000; ++i)
        {
            ...
            assert(std::abs(R1.determinant() - 1) <
                   1e-14); // We use the fact that the QR implementation gives a determinant
            ...
            // Computations are getting less precise as we approach the singularity of the log for a rotation angle of
            // pi. We skip those cases so that we can keep a tight bound on the precision of the remaining tests. This
            // loss of precision is not a problem in practice because (i) in the target applications of sva, rotation
            // angles close to pi is no something we should write/encounter and (ii) even if the precision deteriorates,
            // the derivative is still not bad, and provides a correct descent direction for gradient-based computation
            // schemes.
