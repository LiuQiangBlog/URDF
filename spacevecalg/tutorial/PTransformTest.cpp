...
int main()
{
    ...
    }
    {
        ...
        BOOST_CHECK_SMALL(
            (res.rotation() - AngleAxisd(cst::pi<double>() / 2., Vector3d::UnitZ()).toRotationMatrix()).norm(), TOL);
    ...
    }
    {
        ...
        vector3ToCrossMatrix(rotationVelocity(r1)).exp().transpose().isApprox(r1);
        vector3ToCrossMatrix(rotationVelocity(r2)).exp().transpose().isApprox(r2);
        vector3ToCrossMatrix(rotationVelocity(r3)).exp().transpose().isApprox(r3);
        vector3ToCrossMatrix(rotationVelocity(r4)).exp().transpose().isApprox(r4);
        ...
        for (int i = 0; i < 20; ++i)
        {
            ...
            vector3ToCrossMatrix(rotationVelocity(r)).exp().transpose().isApprox(r);
        }
        ...
        {
            Matrix3d r1 = AngleAxisd(Pi + std::pow(10, e), u).toRotationMatrix();
            Matrix3d r2 = AngleAxisd(Pi - std::pow(10, e), u).toRotationMatrix();
            vector3ToCrossMatrix(rotationVelocity(r1)).exp().transpose().isApprox(r1, 1e-6);
            vector3ToCrossMatrix(rotationVelocity(r2)).exp().transpose().isApprox(r2, 1e-6);
        };
        ...
        {
            Matrix3d r1 = AngleAxisd(Pi, u).toRotationMatrix();
            vector3ToCrossMatrix(rotationVelocity(r1)).exp().transpose().isApprox(r1, 1e-6)
        };
