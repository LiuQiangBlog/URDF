...
Model::Model()
{
    ...
    SpatialRigidBodyInertia rbi(0., Vector3d(0., 0., 0.), Matrix3d::Zero());
    Ic.push_back(rbi);
    I.push_back(rbi);
                               ...
                               std::string body_name)
{
    ...
    if (!body_name.empty())
    {
        ...
        {
            std::ostringstream errormsg;
            errormsg << "Error: Body with name '" << body_name << "' already exists!" << std::endl;
            throw Errors::RBDLError(errormsg.str());
        }
                                  ...
                                  std::string body_name)
{
    ...
    {
        std::ostringstream errormsg;
        errormsg << "Error: Invalid joint type: " << joint.mJointType << std::endl;
        throw Errors::RBDLError(errormsg.str());
    }
                            ...
                            const std::string &body_name)
{
    ...
    if (joint.mJointType == JointTypeFixed)
    {
        ...
        return previously_added_body_id;
             ...
             (joint.mJointType == JointTypeEulerZXY) || (joint.mJointType == JointTypeTranslationXYZ) ||
             (joint.mJointType == JointTypeCustom))
    {
        // no action required
    }
    else if (joint.mJointType != JointTypePrismatic && joint.mJointType != JointTypeRevolute &&
             joint.mJointType != JointTypeRevoluteX && joint.mJointType != JointTypeRevoluteY &&
             joint.mJointType != JointTypeRevoluteZ && joint.mJointType != JointTypeHelical)
    {
    ...
    {
        std::ostringstream errormsg;
        errormsg << "Error: cannot add more than " << fixed_body_discriminator
                 << " movable bodies. You need to modify Model::fixed_body_discriminator for this." << std::endl;
        throw Errors::RBDLError(errormsg.str());
    }
