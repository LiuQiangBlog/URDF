...
ContactConstraint::ContactConstraint()
    : Constraint("", ConstraintTypeContact, 1, std::numeric_limits<unsigned int>::max())
{
}
    ...
    : Constraint(contactConstraintName, ConstraintTypeContact, unsigned(int(1)), userDefinedIdNumber)
{
    ...
    bodyFrames.push_back(Math::SpatialTransform(Math::Matrix3dIdentity, bodyPoint));

    bodyIds.push_back(0);
    bodyFrames.push_back(Math::SpatialTransform(Math::Matrix3dIdentity, groundPoint));
                                               ...
                                               bool                  updateKinematics)
{
    ...
    {
        GSysUpd.block(rowInSystem + i, 0, 1, GSysUpd.cols()) = T[i].transpose() * cache.mat3NA;
    }
}
                                  ...
                                  bool                  updateKinematics)
{
    ...
    for (unsigned int i = 0; i < sizeOfConstraint; ++i)
    {
        gammaSysUpd.block(rowInSystem + i, 0, 1, 1) = -T[i].transpose() * cache.vec3A;
    }
}
...
//==============================================================================
void ContactConstraint::appendNormalVector(const Math::Vector3d &normal, bool velocityLevelConstraint)
{
                                               ...
                                               bool                         updateKinematics)
{
    ...
    {
        pointAccelerationsSysUpd[rowInSystem + i] = pointAccelerationsSysUpd[rowInSystem];
    }
}
                                                   ...
                                                   Math::VectorNd                    &ddErrSysUpd)
{
    for (unsigned int i = 0; i < sizeOfConstraint; ++i)
    {
        ddErrSysUpd[rowInSystem + i] = T[i].dot(pointAccelerationsSys[rowInSystem + i]);
    }
}
