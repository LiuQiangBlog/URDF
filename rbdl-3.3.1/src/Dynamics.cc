    ...
    for (i = model.mBodies.size() - 1; i > 0; i--)
    {
        ...
        else if (model.mJoints[i].mJointType == JointTypeCustom)
        {
            ...
            model.mCustomJoints[kI]->Dinv =
                (model.mCustomJoints[kI]->S.transpose() * model.mCustomJoints[kI]->U).inverse();
#else
            model.mCustomJoints[kI]->Dinv =
...
RBDL_DLLAPI void
CalcMInvTimesTau(Model &model, const VectorNd &Q, const VectorNd &Tau, VectorNd &QDDot, bool update_kinematics)
{
    ...
    for (unsigned int i = model.mBodies.size() - 1; i > 0; i--)
    {
        ...
        else if (model.mJoints[i].mJointType == JointTypeCustom)
        {
            ...
            if (lambda != 0)
            {
                SpatialVector pa = model.pA[i] + (model.mCustomJoints[kI]->U * model.mCustomJoints[kI]->Dinv *
