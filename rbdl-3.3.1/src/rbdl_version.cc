...
RBDL_DLLAPI void rbdl_check_api_version(int version)
{
    ...
    if (version_major != compile_major)
    {
        ...
        std::cerr << "The library version is: " << compile_version_string.str() << " but rbdl_config.h is version "
                  << link_version_string.str() << std::endl;
        abort();
    }
    else if (version_minor != compile_minor)
    {
        std::cout << "Warning: RBDL library is of version " << compile_version_string.str()
                  << " but rbdl_config.h is from version " << link_version_string.str() << std::endl;
    }
}
...
RBDL_DLLAPI void rbdl_print_version()
{
    ...
    {
        std::cout << "  commit       : " << RBDL_BUILD_COMMIT << " (branch: " << RBDL_BUILD_BRANCH << ")" << std::endl
