                                                 ...
                                                 unsigned int userDefinedId)
{
    ...
    if (contactConstraints.size() > 0)
    {
        ...
        {
            Vector3d pointErr = body_point - contactConstraints[i]->getBodyFrames()[0].r;
    ...
    if (constraintAppended == false)
    {
        ...
        contactConstraints.push_back(std::shared_ptr<ContactConstraint>(
    ...
    if (nameStr.size() > 0)
    {
        ...
        iter = nameGroupMap.insert(
            std::pair<std::string, unsigned int>(name[name.size() - 1], unsigned(constraints.size() - 1)));
    ...
    if (userDefinedId < std::numeric_limits<unsigned int>::max())
    {
        ...
        iter = userDefinedIdGroupMap.insert(
            std::pair<unsigned int, unsigned int>(userDefinedId, unsigned(constraints.size() - 1)));
        // if(iter.second == false){
    ...
    std::pair<std::map<unsigned int, unsigned int>::iterator, bool> iter;
    iter = idGroupMap.insert(
        std::pair<unsigned int, unsigned int>(unsigned(rowsInG - 1), unsigned(constraints.size() - 1)));
    if (iter.second == false)
    {
                                              ...
                                              unsigned int userDefinedId)
{
    ...
    if (loopConstraints.size() > 0)
    {
            ...
            loopConstraints[idx]->getBodyIds()[1] == idSuccessor)
        {
            ...
            if (framesNumericallyIdentical && loopConstraints[idx]->getUserDefinedId() == userDefinedId)
            {
                constraintAppended = true;
    ...
    if (constraintAppended == false)
    {
        ...
        loopConstraints.push_back(std::shared_ptr<LoopConstraint>(new LoopConstraint(
            idPredecessor, idSuccessor, XPredecessor, XSuccessor, constraintAxisInPredecessor,
...
unsigned int ConstraintSet::AddCustomConstraint(std::shared_ptr<Constraint> customConstraint)
{
    ...
    if (customConstraint->getUserDefinedId() < std::numeric_limits<unsigned int>::max())
    {
        std::pair<std::map<unsigned int, unsigned int>::iterator, bool> iter;
        iter = userDefinedIdGroupMap.insert(std::pair<unsigned int, unsigned int>(customConstraint->getUserDefinedId(),
...
//==============================================================================
void ConstraintSet::SetActuationMap(const Model &model, const std::vector<bool> &actuatedDofUpd)
{
                                  ...
                                  bool update_kinematics)
{
    ...
    {
        CS.constraints[i]->calcPositionError(model, 0, Q, err, CS.cache, update_kinematics);
    }
}
                             ...
                             bool update_kinematics)
{
    ...
    for (unsigned int i = 0; i < CS.constraints.size(); ++i)
    {
        CS.constraints[i]->calcConstraintJacobian(model, 0, Q, CS.cache.vecNZeros, G, CS.cache, update_kinematics);
    }
}
                                  ...
                                  bool update_kinematics)
{
    ...
    for (unsigned int i = 0; i < CS.constraints.size(); ++i)
    {
        CS.constraints[i]->calcVelocityError(model, 0, Q, QDot, CS.G, err, CS.cache, update_kinematics);
    }
}
                                    ...
                                    std::vector<Math::SpatialVector> *f_ext)
{
    ...
    for (unsigned int i = 0; i < CS.constraints.size(); ++i)
    {
        ...
        {
            CS.constraints[i]->addInBaumgarteStabilizationForces(CS.err, CS.errd, CS.gamma);
        }
    }
}
                                      ...
                                      std::vector<Math::SpatialVector> *f_ext)
{
    ...
    SolveConstrainedSystemDirect(CS.H, CS.G, Tau - CS.C, CS.gamma, CS.force, CS.A, CS.b, CS.x, CS.linear_solver);
                                                ...
                                                std::vector<Math::SpatialVector> *f_ext)
{
    ...
    SolveConstrainedSystemRangeSpaceSparse(model, CS.H, CS.G, Tau - CS.C, CS.gamma, QDDot, CS.force, CS.K, CS.a,
                                           CS.linear_solver);
}
                                         ...
                                         std::vector<Math::SpatialVector> *f_ext)
{
    ...
    SolveConstrainedSystemNullSpace(CS.H, CS.G, Tau - CS.C, CS.gamma, QDDot, CS.force, CS.Y, CS.Z, CS.qddot_y,
                                     ...
                                     Math::VectorNd &QDotPlus)
{
    ...
    SolveConstrainedSystemDirect(CS.H, CS.G, CS.H * QDotMinus, CS.v_plus, CS.impulse, CS.A, CS.b, CS.x,
                                 CS.linear_solver);
                                               ...
                                               Math::VectorNd &QDotPlus)
{
    ...
    SolveConstrainedSystemRangeSpaceSparse(model, CS.H, CS.G, CS.H * QDotMinus, CS.v_plus, QDotPlus, CS.impulse, CS.K,
                                        ...
                                        Math::VectorNd &QDotPlus)
{
    ...
    SolveConstrainedSystemNullSpace(CS.H, CS.G, CS.H * QDotMinus, CS.v_plus, QDotPlus, CS.impulse, CS.Y, CS.Z,
                                    CS.qddot_y, CS.qddot_z, CS.linear_solver);
}
#endif
...
void ForwardDynamicsApplyConstraintForces(Model &model, const VectorNd &Tau, ConstraintSet &CS, VectorNd &QDDot)
{
    ...
    for (i = 1; i < model.mBodies.size(); i++)
    {
        ...
        {
#endif
            LOG << "External force (" << i << ") = " << model.X_base[i].toMatrixAdjoint() * CS.f_ext_constraints[i]
                << std::endl;
    ...
    for (i = model.mBodies.size() - 1; i > 0; i--)
    {
        ...
        if (model.mJoints[i].mDoFCount == 3 && model.mJoints[i].mJointType != JointTypeCustom)
        {
            ...
            model.multdof3_u[i] = Vector3d(Tau[q_index], Tau[q_index + 1], Tau[q_index + 2]) -
                                  model.multdof3_S[i].transpose() * model.pA[i];

            if (lambda != 0)
            {
                SpatialMatrix Ia =
                    model.IA[i] - (model.multdof3_U[i] * model.multdof3_Dinv[i] * model.multdof3_U[i].transpose());

                SpatialVector pa =
                    model.pA[i] + Ia * model.c[i] + model.multdof3_U[i] * model.multdof3_Dinv[i] * model.multdof3_u[i];
                ...
                model.IA[lambda].noalias() +=
                    (model.X_lambda[i].toMatrixTranspose() * Ia * model.X_lambda[i].toMatrix());
        ...
        else if (model.mJoints[i].mJointType == JointTypeCustom)
        {
            ...
            if (lambda != 0)
            {
                SpatialMatrix Ia = model.IA[i] - (model.mCustomJoints[kI]->U * model.mCustomJoints[kI]->Dinv *
                                                  model.mCustomJoints[kI]->U.transpose());

                SpatialVector pa =
                    model.pA[i] + Ia * model.c[i] +
                    (model.mCustomJoints[kI]->U * model.mCustomJoints[kI]->Dinv * model.mCustomJoints[kI]->u);
                                       ...
                                       const std::vector<SpatialVector> &f_t)
{
    ...
    for (unsigned int i = body_id; i > 0; i--)
    {
        ...
        else if (model.mJoints[i].mDoFCount == 1 && model.mJoints[i].mJointType != JointTypeCustom)
        {
            ...
            {
                CS.d_pA[lambda] = CS.d_pA[lambda] +
                                  model.X_lambda[i].applyTranspose(CS.d_pA[i] + model.U[i] * CS.d_u[i] / model.d[i]);
            }
        }
        else if (model.mJoints[i].mJointType == JointTypeCustom)
        {
            ...
            if (lambda != 0)
            {
                CS.d_pA[lambda] =
                    CS.d_pA[lambda] + model.X_lambda[i].applyTranspose(
                                          CS.d_pA[i] + (model.mCustomJoints[kI]->U * model.mCustomJoints[kI]->Dinv *
                                     ...
                                     VectorNd &QDDot)
{
    ...
    if (CS.constraints.size() != CS.contactConstraints.size())
    {
        ...
        errormsg << "Incompatible constraint types: all constraints"
                 << " must be ContactConstraints for the Kokkevis method" << std::endl;
        throw Errors::RBDLError(errormsg.str());
    }
    ...
    for (bi = 0; bi < CS.contactConstraints.size(); ++bi)
    {
        ...
        {
            LOG << "body_id = " << CS.contactConstraints[bi]->getBodyIds()[0] << std::endl;
            LOG << "point = " << CS.contactConstraints[bi]->getBodyFrames()[0].r << std::endl;
            LOG << "QDDot_0 = " << CS.QDDot_0.transpose() << std::endl;
        }
        {
            SUPPRESS_LOGGING;
            CS.contactConstraints[bi]->calcPointAccelerations(model, Q, QDot, CS.QDDot_0, CS.point_accel_0, false);
            CS.contactConstraints[bi]->calcPointAccelerationError(CS.point_accel_0, CS.a);
        }
    }
    ...
    for (bi = 0; bi < CS.contactConstraints.size(); bi++)
    {
        ...
        movable_body_id = GetMovableBodyId(model, CS.contactConstraints[bi]->getBodyIds()[0]);

        // assemble the test force
        LOG << "point_global = " << point_global.transpose() << std::endl;

        CS.contactConstraints[bi]->calcPointForceJacobian(model, Q, CS.cache, CS.f_t, false);

        for (unsigned int j = 0; j < CS.contactConstraints[bi]->getConstraintNormalVectors().size(); ++j)
        {

            CS.f_ext_constraints[movable_body_id] = CS.f_t[ci + j];

            LOG << "f_t[" << movable_body_id << "] = " << CS.f_t[ci + j].transpose() << std::endl;
            {
            ...
            for (unsigned int dj = 0; dj < CS.contactConstraints.size(); dj++)
            {

                cj = CS.contactConstraints[dj]->getConstraintIndex();
                {
                    SUPPRESS_LOGGING;
                    CS.contactConstraints[dj]->calcPointAccelerations(model, Q, QDot, CS.QDDot_t, point_accel_t, false);
                }

                LOG << "point_accel_0  = " << CS.point_accel_0[ci + j].transpose() << std::endl;
                LOG << "point_accel_t = " << point_accel_t.transpose() << std::endl;
                for (unsigned int k = 0; k < CS.contactConstraints[dj]->getConstraintNormalVectors().size(); ++k)
                {

                    CS.K(ci + j, cj + k) = CS.contactConstraints[dj]->getConstraintNormalVectors()[k].dot(
    ...
    for (bi = 0; bi < CS.contactConstraints.size(); ++bi)
    {
        ...
        for (unsigned int k = 0; k < CS.contactConstraints[bi]->getConstraintSize(); ++k)
        {
            CS.f_ext_constraints[movable_body_id] -= CS.f_t[ci + k] * CS.force[ci + k];
            LOG << "f_ext[" << movable_body_id << "] = " << CS.f_ext_constraints[movable_body_id].transpose()
                << std::endl;
        }
    }
                                ...
                                std::vector<Math::SpatialVector> *f_ext)
{
    ...
    SolveLinearSystem(CS.GTl.transpose(), CS.gamma - CS.GTu.transpose() * CS.u, CS.v, CS.linear_solver);

    // lambda
    SolveLinearSystem(CS.GTl, -CS.P * CS.C - CS.Fll * CS.u - CS.Flr * CS.v, CS.force, CS.linear_solver);
    ...
    // Evaluating tau
    TauOutput = -CS.S.transpose() * (-CS.S * CS.C - (CS.Ful * CS.u + CS.Fur * CS.v - CS.GTu * CS.force));
}
                                       ...
                                       std::vector<Math::SpatialVector> *f_ext)
{
    ...
    CS.u = CS.S * CS.C - CS.W * (CS.S * (QDDotControls + (CS.S.transpose() * CS.WinvSC)));
    ...
    //(n-nc) x (n-nc) system
    SolveLinearSystem(CS.Z.transpose() * CS.F * CS.Z, CS.Z.transpose() * (-CS.F * CS.Y * CS.py - CS.g), CS.pz,
                      CS.linear_solver);

    // nc x nc system
    SolveLinearSystem(CS.Ru, CS.Y.transpose() * (CS.g + CS.F * CS.Y * CS.py + CS.F * CS.Z * CS.pz), CS.force,
                      CS.linear_solver);
    ...
    TauOutput = (CS.S.transpose() * CS.W * CS.S) * (QDDotControls + (CS.S.transpose() * CS.WinvSC) - QDDotOutput);
                               ...
                               bool updKin)
{
    ...
    constraints[groupIndex]->calcConstraintForces(model, 0., Q, QDot, G, force, constraintBodyIdsUpd,
                                                  constraintBodyFramesUpd, constraintForcesUpd, cache,
                                                  resolveAllInRootFrame, updKin);
}
                                 ...
                                 bool updKin)
{
    ...
    constraints[groupIndex]->calcConstraintForces(model, 0., Q, QDot, G, impulse, constraintBodyIdsUpd,
                                                  constraintBodyFramesUpd, constraintImpulsesUpd, cache,
                                                  resolveAllInRootFrame, updKin);
                                      ...
                                      bool updKin)
{
    ...
    constraints[groupIndex]->calcConstraintJacobian(model, 0., Q, QDot, G, cache, updKin);

    // Update the velocity-level errors of this constraint
    constraints[groupIndex]->calcVelocityError(model, 0., Q, QDot, G, errd, cache, updKin);
                                                     ...
                                                     Math::VectorNd &baumgarteForces)
{
    ...
    constraints[groupIndex]->getBaumgarteStabilizationForces(positionError, velocityError, baumgarteForces);
}
