...
inline void
sincosp(RBDLCasadiMath::MX_Xd_scalar x, RBDLCasadiMath::MX_Xd_scalar *sinp, RBDLCasadiMath::MX_Xd_scalar *cosp)
{
    *sinp = std::sin(x);
    *cosp = std::cos(x);
}
...
#ifdef __APPLE__
inline void
sincosp(RigidBodyDynamics::Math::Scalar x, RigidBodyDynamics::Math::Scalar *sinp, RigidBodyDynamics::Math::Scalar *cosp)
{
...
RBDL_DLLAPI void jcalc(Model &model, unsigned int joint_id, const VectorNd &q, const VectorNd &qdot)
{
    ...
    else if (model.mJoints[joint_id].mJointType == JointTypeSpherical)
    {
        ...
        Vector3d omega(qdot[model.mJoints[joint_id].q_index], qdot[model.mJoints[joint_id].q_index + 1],
                       qdot[model.mJoints[joint_id].q_index + 2]);

        model.v_J[joint_id] = SpatialVector(omega[0], omega[1], omega[2], 0., 0., 0.);
        model.X_lambda[joint_id] = X_J * model.X_T[joint_id];
    }
    ...
    else if (model.mJoints[joint_id].mJointType == JointTypeEulerXYZ)
    {
        ...
        model.c_J[joint_id].set(-s2 * c1 * qdot2 * qdot0 - c2 * s1 * qdot1 * qdot0 + c2 * qdot2 * qdot1,
    ...
    else if (model.mJoints[joint_id].mJointType == JointTypeEulerYXZ)
    {
        ...
        model.c_J[joint_id].set(c2 * c1 * qdot2 * qdot0 - s2 * s1 * qdot1 * qdot0 - s2 * qdot2 * qdot1,
    ...
    else if (model.mJoints[joint_id].mJointType == JointTypeEulerZXY)
    {
        ...
        model.c_J[joint_id].set((-c1 * c2 * qdot2 + s1 * s2 * qdot1) * qdot0 - s2 * qdot1 * qdot2, c1 * qdot1 * qdot0,
                                (-s1 * c2 * qdot1 - c1 * s2 * qdot2) * qdot0 + c2 * qdot2 * qdot1, 0., 0., 0.);
    }
    ...
    else
    {
        ...
        errormsg << "Error: invalid joint type " << model.mJoints[joint_id].mJointType << " at id " << joint_id
                 << std::endl;
    ...
    if (model.mJoints[joint_id].mDoFCount == 1 && model.mJoints[joint_id].mJointType != JointTypeCustom)
    {
        if (model.mJoints[joint_id].mJointType == JointTypeRevolute)
        {
            return Xrot(q[model.mJoints[joint_id].q_index],
                        Vector3d(model.mJoints[joint_id].mJointAxes[0][0], model.mJoints[joint_id].mJointAxes[0][1],
        ...
        else if (model.mJoints[joint_id].mJointType == JointTypeHelical)
        {
            ...
            SpatialTransform trans =
                Xtrans(Vector3d(model.mJoints[joint_id].mJointAxes[0][3] * q[model.mJoints[joint_id].q_index],
                                model.mJoints[joint_id].mJointAxes[0][4] * q[model.mJoints[joint_id].q_index],
                                model.mJoints[joint_id].mJointAxes[0][5] * q[model.mJoints[joint_id].q_index]));
            return rot * trans;
        }
...
RBDL_DLLAPI void jcalc_X_lambda_S(Model &model, unsigned int joint_id, const VectorNd &q)
{
    ...
    else if (model.mJoints[joint_id].mJointType == JointTypeHelical)
    {
        ...
        model.S[joint_id] =
            SpatialVector(model.mJoints[joint_id].mJointAxes[0][0], model.mJoints[joint_id].mJointAxes[0][1],
                          model.mJoints[joint_id].mJointAxes[0][2], trans[0], trans[1], trans[2]);
    }
    else if (model.mJoints[joint_id].mDoFCount == 1 && model.mJoints[joint_id].mJointType != JointTypeCustom)
    {
    ...
    {
        model.X_lambda[joint_id] =
            SpatialTransform(model.GetQuaternion(joint_id, q).toMatrix(), Vector3d(0., 0., 0.)) * model.X_T[joint_id];
    ...
    else if (model.mJoints[joint_id].mJointType == JointTypeEulerXYZ)
    {
        ...
        model.X_lambda[joint_id] =
            SpatialTransform(Matrix3d(c2 * c1, s2 * c0 + c2 * s1 * s0, s2 * s0 - c2 * s1 * c0, -s2 * c1,
                                      c2 * c0 - s2 * s1 * s0, c2 * s0 + s2 * s1 * c0, s1, -c1 * s0, c1 * c0),
                             Vector3d(0., 0., 0.)) *
            model.X_T[joint_id];
    ...
    else if (model.mJoints[joint_id].mJointType == JointTypeEulerYXZ)
    {
        ...
        model.X_lambda[joint_id] =
            SpatialTransform(Matrix3d(c2 * c0 + s2 * s1 * s0, s2 * c1, -c2 * s0 + s2 * s1 * c0, -s2 * c0 + c2 * s1 * s0,
                                      c2 * c1, s2 * s0 + c2 * s1 * c0, c1 * s0, -s1, c1 * c0),
                             Vector3d(0., 0., 0.)) *
            model.X_T[joint_id];
    ...
    else if (model.mJoints[joint_id].mJointType == JointTypeTranslationXYZ)
    {
        ...
        model.X_lambda[joint_id] = SpatialTransform(Matrix3d::Identity(), Vector3d(q0, q1, q2)) * model.X_T[joint_id];
