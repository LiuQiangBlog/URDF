...
RBDL_DLLAPI std::string GetModelDOFOverview(const Model &model)
{
    ...
    for (unsigned int i = 1; i < model.mBodies.size(); i++)
    {
        ...
        {
            result << setfill(' ') << setw(3) << q_index << ": " << get_body_name(model, i) << "_"
                   << get_dof_name(model.S[i]) << endl;
            q_index++;
        }
        else
        {
...
std::string print_hierarchy(const RigidBodyDynamics::Model &model, unsigned int body_index = 0, int indent = 0)
{
    ...
    while (model.mBodies[body_index].mIsVirtual)
    {
        ...
        else if (model.mu[body_index].size() > 1)
        {
            ...
            {
                errormsg << "  id: " << model.mu[body_index][ci]
                                  ...
                                  bool                  update_kinematics)
{
    ...
    assert((com_acceleration == NULL && change_of_angular_momentum == NULL) || (qddot != NULL));
...
RBDL_DLLAPI Scalar CalcPotentialEnergy(Model &model, const Math::VectorNd &q, bool update_kinematics)
{
    Scalar   mass;
    Vector3d com;
    CalcCenterOfMass(model, q, VectorNd::Zero(model.qdot_size), NULL, mass, com, NULL, NULL, NULL, NULL,
                     update_kinematics);
...
RBDL_DLLAPI Scalar CalcKineticEnergy(Model                &model,
                                     const Math::VectorNd &q,
                                     const Math::VectorNd &qdot,
                                     bool                  update_kinematics)
{
