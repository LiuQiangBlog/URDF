...
LoopConstraint::LoopConstraint()
    : Constraint("", ConstraintTypeLoop, unsigned(int(0)), std::numeric_limits<unsigned int>::max())
{
}
                                            ...
                                            bool                  updateKinematics)
{
    ...
    cache.stA.r = CalcBodyToBaseCoordinates(model, Q, bodyIds[0], bodyFrames[0].r, updateKinematics);
    cache.stA.E = CalcBodyWorldOrientation(model, Q, bodyIds[0], updateKinematics).transpose() * bodyFrames[0].E;
                               ...
                               bool                  updateKinematics)
{
    ...
    // vel_p
    cache.svecA = CalcPointVelocity6D(model, Q, QDot, bodyIds[0], bodyFrames[0].r, updateKinematics);

    // vel_s
    cache.svecB = CalcPointVelocity6D(model, Q, QDot, bodyIds[1], bodyFrames[1].r, updateKinematics);
    ...
    // acc_p
    cache.svecC =
        CalcPointAcceleration6D(model, Q, QDot, cache.vecNZeros, bodyIds[0], bodyFrames[0].r, updateKinematics);
    // acc_s
    cache.svecD =
        CalcPointAcceleration6D(model, Q, QDot, cache.vecNZeros, bodyIds[1], bodyFrames[1].r, updateKinematics);
                                       ...
                                       bool                  updateKinematics)
{
    ...
    // Ks: successor frame
    cache.stB.r = CalcBodyToBaseCoordinates(model, Q, bodyIds[1], bodyFrames[1].r, updateKinematics);
    cache.stB.E = CalcBodyWorldOrientation(model, Q, bodyIds[1], updateKinematics).transpose() * bodyFrames[1].E;
