...
public:
  ...
  \mathbf{t}_V(\dot{\theta}/\dot{\theta}_{MAX})
    ...
    */
    static void
    calcHuntCrossleyContactForce(double z, double dz, double k, double p, double beta, HuntCrossleyContactInfo &hcInfo)
    {
        if (z < 0.0)
        {
    ...
    */
        ...
        RigidBodyDynamics::Addons::Geometry::SmoothSegmentedFunction &smoothSegmentedFunctionToUpdate)
    {
        ...
        {
            std::cerr << "Required: staticFrictionSpeed > 0, but given " << staticFrictionSpeed << std::endl;
            assert(0);
            abort();
        }
        if (staticFrictionCoefficient < 0)
        {
            std::cerr << "Required: staticFrictionCoefficient >= 0, but given " << staticFrictionCoefficient
                      << std::endl;
            assert(0);
            abort();
        }
        ...
        if (dynamicFrictionSpeed < staticFrictionSpeed)
        {
            std::cerr << "Required: dynamicFrictionSpeed > staticFrictionSpeed,"
                      << "but given " << dynamicFrictionSpeed << " !> " << staticFrictionSpeed << std::endl;
            assert(0);
            abort();
        }
        if (dynamicFrictionCoefficient > staticFrictionCoefficient)
        {
            std::cerr << "Required: dynamicFrictionCoefficient <= staticFrictionCoeffient,"
                      << "but given " << dynamicFrictionCoefficient << " !<= " << staticFrictionCoefficient
                      << std::endl;
            assert(0);
            abort();
        }
        ...
        {
            std::cerr << "Required: viscousFrictionSlope > 0, but given " << viscousFrictionSlope << std::endl;
            assert(0);
            abort();
        }
        ...
        double c = RigidBodyDynamics::Addons::Geometry::SegmentedQuinticBezierToolkit::scaleCurviness(curviness);
        ...
        pts = RigidBodyDynamics::Addons::Geometry::SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
        ...
        smoothSegmentedFunctionToUpdate.updSmoothSegmentedFunction(xM, yM, x0, x3, y0, y3, dydx0, dydx3, curveName);
    }
