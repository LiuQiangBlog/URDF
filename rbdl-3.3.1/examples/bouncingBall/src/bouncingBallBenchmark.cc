...
public:
    rbdlToBoost(Model       *model,
        ...
        : model(model), r(ballRadius), r0P0(pointOnPlane), eN0(planeNormal), k(stiffness), p(exponent), beta(damping),
          ...
          dynamicFrictionSpeed(dynamicFrictionSpeed), dynamicFrictionCoefficient(dynamicFrictionCoefficient),
          viscousFrictionSlope(viscousFrictionSlope), numberOfWorkTermsInState(numberOfWorkTermsInState)
    {
        ...
        frictionCoefficientCurve.printCurveToCSVFile("../output/", "frictionCoefficientCurve", 0,
                                                     dynamicFrictionSpeed * 2);
    ...
    void operator()(const state_type &x, state_type &dxdt, const double t)
    {
        ...
        j++;
        assert((numberOfWorkTermsInState + model->q_size + model->qdot_size) == j);
    }
    ...
    */
    ...
    RigidBodyDynamics::Addons::Geometry ::SmoothSegmentedFunction frictionCoefficientCurve;
...
int main(int argc, char *argv[])
{
    ...
    rbdlToBoost rbdlModel(&model, ballName, radius, pointOnPlane, planeNormal, stiffness, exponent, damping,
                          staticFrictionSpeed, staticFrictionCoefficient, dynamicFrictionSpeed,
                          dynamicFrictionCoeffient, viscousFrictionSlope, numWorkTermsInState);
    ...
    for (unsigned int i = 0; i <= npts; ++i)
    {
        ...
        integrate_adaptive(controlled_stepper, rbdlModel, xState, tp, t, (t - tp) / 10);
        tp = t;
        ...
        printf("%e, %e, %e, %e, %e, %e, %e\n", t, q[2], qd[2], ke, pe, w, (ke + pe - w - kepe0));
