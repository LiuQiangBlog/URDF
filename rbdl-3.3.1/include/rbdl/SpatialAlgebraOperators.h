...
{
    SpatialRigidBodyInertia() : m(0.), h(Vector3d::Zero()), Ixx(0.), Iyx(0.), Iyy(0.), Izx(0.), Izy(0.), Izz(0.) {}
    SpatialRigidBodyInertia(Scalar mass, const Vector3d &com_mass, const Matrix3d &inertia)
        : m(mass), h(com_mass), Ixx(inertia(0, 0)), Iyx(inertia(1, 0)), Iyy(inertia(1, 1)), Izx(inertia(2, 0)),
          Izy(inertia(2, 1)), Izz(inertia(2, 2))
    {
    }
    ...
    SpatialVector operator*(const SpatialVector &mv)
    {
        ...
        Vector3d res_upper = Vector3d(Ixx * mv[0] + Iyx * mv[1] + Izx * mv[2], Iyx * mv[0] + Iyy * mv[1] + Izy * mv[2],
                                      Izx * mv[0] + Izy * mv[1] + Izz * mv[2]) +
                             h.cross(mv_lower);
        Vector3d res_lower = m * mv_lower - h.cross(Vector3d(mv[0], mv[1], mv[2]));

        return SpatialVector(res_upper[0], res_upper[1], res_upper[2], res_lower[0], res_lower[1], res_lower[2]);
    }

    SpatialRigidBodyInertia operator+(const SpatialRigidBodyInertia &rbi)
    {
        return SpatialRigidBodyInertia(m + rbi.m, h + rbi.h, Ixx + rbi.Ixx, Iyx + rbi.Iyx, Iyy + rbi.Iyy, Izx + rbi.Izx,
    ...
    createFromMassComInertiaC(Scalar mass, const Vector3d &com, const Matrix3d &inertia_C)
    {
...
inline SpatialTransform Xrot(Scalar angle_rad, const Vector3d &axis)
{
    ...
    return {Matrix3d(axis[0] * axis[0] * (1.0f - c) + c, axis[1] * axis[0] * (1.0f - c) + axis[2] * s,
                     axis[0] * axis[2] * (1.0f - c) - axis[1] * s,

                     axis[0] * axis[1] * (1.0f - c) - axis[2] * s, axis[1] * axis[1] * (1.0f - c) + c,
                     axis[1] * axis[2] * (1.0f - c) + axis[0] * s,

                     axis[0] * axis[2] * (1.0f - c) + axis[1] * s, axis[1] * axis[2] * (1.0f - c) - axis[0] * s,
...
{
    return {0, -v[2], v[1], 0, 0,     0,    v[2], 0, -v[0], 0,    0, 0,     -v[1], v[0], 0, 0,     0,    0,
