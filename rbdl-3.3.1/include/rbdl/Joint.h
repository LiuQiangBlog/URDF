 ...
 * a change of interpretation for the values \f$\mathbf{q}, \mathbf{\dot{q}}, \mathbf{\ddot{q}}\f$, and
 \f$\mathbf{\tau}\f$:
 *
 * <ul>
 ...
 * TX, Spherical, TY, Spherical, the values of \f$\mathbf{q},\mathbf{\bar{q}},\mathbf{\bar{\bar{q}}},\mathbf{\tau}\f$
 are:
 *

 \f{eqnarray*}
        \mathbf{q} &=& ( q_{tx}, q_{q1,x}, q_{q1,y}, q_{q1,z}, q_{ty}, q_{q2,x}, q_{q2,y}, q_{q2,z}, q_{q1,w},
 q_{q2,w})^T \\
  \mathbf{\bar{q}} &=& ( \dot{q}_{tx}, \omega_{1,x}, \omega_{1,y}, \omega_{1,z}, \dot{q}_{ty}, \omega_{2,x},
 \omega_{2,y}, \omega_{2,z} )^T \\
  \mathbf{\bar{\bar{q}}} &=& ( \ddot{q}_{tx}, \dot{\omega}_{1,x}, \dot{\omega}_{1,y}, \dot{\omega}_{1,z}, \ddot{q}_{ty},
 \dot{\omega}_{2,x}, \dot{\omega}_{2,y}, \dot{\omega}_{2,z} )^T \\ \mathbf{\tau} &=& ( \tau_{tx}, \tau_{1,x},
 \tau_{1,y}, \tau_{1,z}, \tau_{ty}, \tau_{2,x}, \tau_{2,y}, \tau_{2,z} )^T \f}
...
enum JointType
{
    ...
    JointTypeRevoluteY,
    JointTypeRevoluteZ,
    JointTypeSpherical, ///< 3 DoF joint using Quaternions for joint positional variables and angular velocity for joint
                        ///< velocity variables.
...
static const JointMapEntry JointMap[] = {{JointTypeUndefined, "Undefined", "UNDEFINED"},
...
struct RBDL_DLLAPI Joint
{
    ...
    Joint(JointType type) : mJointAxes(nullptr), mJointType(type), mDoFCount(0), q_index(0), custom_joint_index(-1)
    {
        ...
        else if (type == JointTypeCustom)
        {
            ...
            errormsg << "Error: Invalid use of Joint constructor Joint(JointType"
                     << " type). Only allowed when type != JointTypeCustom" << std::endl;
            throw Errors::RBDLError(errormsg.str());
        }
        else if (type != JointTypeFixed && type != JointTypeFloatingBase)
        {
            std::ostringstream errormsg;
            errormsg << "Error: Invalid use of Joint constructor Joint(JointType type). Only allowed when type == "
                        "JointTypeFixed or JointTypeSpherical."
                     << std::endl;
            throw Errors::RBDLError(errormsg.str());
        }
    }
    ...
    Joint(const Math::SpatialVector &axis_0)
    {
        ...
        if (fabs(fabs(casadi::MX::evalf(axis_0[0])) - 1.0).scalar() < std::numeric_limits<double>::epsilon())
        {
#else
        ...
        else if (fabs(fabs(casadi::MX::evalf(axis_0[1])) - 1.0).scalar() < std::numeric_limits<double>::epsilon())
        {
#else
        }
        ...
        else if (fabs(fabs(casadi::MX::evalf(axis_0[2])) - 1.0).scalar() < std::numeric_limits<double>::epsilon())
        {
#else
        }
        ...
        else if (fabs(casadi::MX::evalf(axis_0[0]).scalar() < std::numeric_limits<double>::epsilon()) &&
