 ...
 */
RBDL_DLLAPI void
UpdateKinematics(Model &model, const Math::VectorNd &Q, const Math::VectorNd &QDot, const Math::VectorNd &QDDot);
 ...
 */
RBDL_DLLAPI void
UpdateKinematicsCustom(Model &model, const Math::VectorNd *Q, const Math::VectorNd *QDot, const Math::VectorNd *QDDot);
 ...
 */
RBDL_DLLAPI
Math::SpatialVector CalcPointVelocity6D(Model &model,
                                        const Math::VectorNd &Q,
                                        const Math::VectorNd &QDot,
                                        unsigned int body_id,
                                        const Math::Vector3d &point_position,
 ...
 */
RBDL_DLLAPI
Math::Vector3d CalcPointAcceleration(Model &model,
                                     const Math::VectorNd &Q,
                                     const Math::VectorNd &QDot,
 ...
 */
RBDL_DLLAPI
Math::SpatialVector CalcPointAcceleration6D(Model &model,
                                            const Math::VectorNd &Q,
                                            const Math::VectorNd &QDot,
                                            const Math::VectorNd &QDDot,
                                            unsigned int body_id,
...
/** \brief Computes the inverse kinematics iteratively using a damped Levenberg-Marquardt method (also known as Damped
...
struct RBDL_DLLAPI InverseKinematicsConstraintSet
{
    ...
    double step_tol; // Step tolerance (default = 1.0e-12). If the computed step length is smaller than this value the
                     // algorithm terminates successfully (i.e. returns true). If error_norm is still larger than
                     // constraint_tol then this usually means that the target is unreachable.
    double constraint_tol; // Constraint tolerance (default = 1.0e-12). If error_norm is smaller than this value the
                           // algorithm terminates successfully, i.e. all constraints are satisfied.
    ...
    // coordinates.
    unsigned int
    AddOrientationConstraint(unsigned int body_id, const Math::Matrix3d &target_orientation, float weight = 1.);
...
RBDL_DLLAPI bool
InverseKinematics(Model &model, const Math::VectorNd &Qinit, InverseKinematicsConstraintSet &CS, Math::VectorNd &Qres);
#endif
