...
struct RBDL_DLLAPI ConstraintSet
{
    ...
    */
    ...
    /**
        ...
        @param resolveAllInRootFrame:
                - false: spatial forces are resolved into the local frames attached to the bodies involved in this
       constraint
                - true: spatial forces are resolved into the base frame. Note that this means that all entires in
       constraintBodyIdsOutput will be 0, the frames in constraintFramesOutput will have their origins at the contact
    ...
    /**
        ...
        @param resolveAllInRootFrame:
                - false: spatial impulses are resolved into the local frames attached to the bodies involved in this
       constraint
                - true: spatial impulses are resolved into the base frame. Note that this means that all entires in
       constraintBodyIdsOutput will be 0, the frames in constraintFramesOutput will have their origins at the contact
       frames but with directions that match the base frame.
    ...
    */
    ...
    {
        constraints[groupIndex]->getBaumgarteStabilizationParameters(baumgartePositionVelocityCoefficientsOutput);
    }
...
/**
 ...
 \note Two modifications have been made to this implementation to bring the
       ...
       -# The vector \f$u^*\f$ has been modifed to \f$u^* = Sx + (S^T W^{-1} S)C\f$ so that the term \f$SC\f$ in the
 upper right hand side is compensated
       -# The weighting matrix \f$W\f$ has a main diagional that is scaled to be uniformly 100 times larger than the
 biggest element in M. This will drive the solution closer to \f$S x\f$ without hurting the scaling of the matrix too
 badly.
 ...
 <b>References</b>
  -# Koch KH (2015). Using model-based optimal control for conceptional motion generation for the humannoid robot hrp-2
 and design investigations for exo-skeletons. Heidelberg University (Doctoral dissertation).
  -# Kudruss M (under review as of May 2019). Nonlinear model-predictive control for the motion generation of humanoids.
 Heidelberg University  (Doctoral dissertation)
