...
void calcCuboidInertia(double mass, double xLength, double yLength, double zLength, Vector3d &iner)
{
...
struct PlanarBipedFloatingBase
{
    PlanarBipedFloatingBase()
        : model(), cs(), q(), qd(), qdd(), tau(), pelvisWidth(0.2), segmentLength(0.5), pelvisMass(50.0),
          segmentMass(1.0), X_p(Xtrans(Vector3d(0., 0., -segmentLength))), X_s(Xtrans(Vector3d(0., 0., 0.)))
    {

        model.gravity = Vector3d(0., 0., -9.81);

        Body segment = Body(segmentMass, Vector3d(0, 0., -0.5 * segmentLength),
                            Vector3d(0., segmentMass * segmentLength * segmentLength / 3., 0.));
        Body pelvis =
            Body(pelvisMass, Vector3d(0, 0., 0.), Vector3d(0., pelvisMass * pelvisWidth * pelvisWidth / 3., 0.));

        Joint joint_TxzRy = Joint(SpatialVector(0., 0., 0., 1., 0., 0.), SpatialVector(0., 0., 0., 0., 0., 1.),
        ...
        // double stance
        cs[0].AddContactConstraint(idxLeftLowerLeg, Vector3d(0., 0., -segmentLength), Vector3d(1., 0., 0.));
        cs[0].AddContactConstraint(idxLeftLowerLeg, Vector3d(0., 0., -segmentLength), Vector3d(0., 0., 1.));
        cs[0].AddContactConstraint(idxRightLowerLeg, Vector3d(0., 0., -segmentLength), Vector3d(1., 0., 0.));
        cs[0].AddContactConstraint(idxRightLowerLeg, Vector3d(0., 0., -segmentLength), Vector3d(0., 0., 1.));

        cs[0].Bind(model);

        // single stance
        cs[1].AddContactConstraint(idxLeftLowerLeg, Vector3d(0., 0., -segmentLength), Vector3d(1., 0., 0.));
...
struct SpatialBipedFloatingBase
{
    SpatialBipedFloatingBase()
        : model(), cs(), q(), qd(), qdd(), tau(), pelvisWidth(0.2), segmentLength(0.5), footLength(0.3),
          pelvisMass(50.0), segmentMass(5.0), footMass(1.0), X_p(Xtrans(Vector3d(0., 0., -segmentLength))),
          X_s(Xtrans(Vector3d(0., 0., 0.)))
    {
        ...
        calcCuboidInertia(segmentMass, 0.1 * segmentLength, 0.1 * segmentLength, segmentLength, segmentInertia);
        calcCuboidInertia(pelvisMass, 0.75 * pelvisWidth, pelvisWidth, 0.50 * pelvisWidth, pelvisInertia);
        ...
        Joint joint_TxyzRxyz = Joint(SpatialVector(0., 0., 0., 1., 0., 0.), SpatialVector(0., 0., 0., 0., 1., 0.),
        ...
        idxPelvis = model.AddBody(0, Xtrans(Vector3d(0., 0., 0.)), joint_TxyzRxyz, pelvis);
        ...
        cs.AddLoopConstraint(idxLeftFoot, 0, X_zero, X_zero, SpatialVector(0, 0, 0, 0, 1, 0), baumgarteEnabled,
                             timeStabilityInverse);
        cs.AddLoopConstraint(idxLeftFoot, 0, X_zero, X_zero, SpatialVector(0, 0, 0, 0, 0, 1), baumgarteEnabled,
                             timeStabilityInverse);
        cs.AddLoopConstraint(idxLeftFoot, 0, X_zero, X_zero, SpatialVector(1, 0, 0, 0, 0, 0), baumgarteEnabled,
                             timeStabilityInverse);
        cs.AddLoopConstraint(idxLeftFoot, 0, X_zero, X_zero, SpatialVector(0, 1, 0, 0, 0, 0), baumgarteEnabled,
                             timeStabilityInverse);
        cs.AddLoopConstraint(idxLeftFoot, 0, X_zero, X_zero, SpatialVector(0, 0, 1, 0, 0, 0), baumgarteEnabled,
                             timeStabilityInverse);

        cs.AddLoopConstraint(idxRightFoot, 0, X_zero, X_zero, SpatialVector(0, 0, 0, 1, 0, 0), baumgarteEnabled,
                             timeStabilityInverse);
        cs.AddLoopConstraint(idxRightFoot, 0, X_zero, X_zero, SpatialVector(0, 0, 0, 0, 1, 0), baumgarteEnabled,
                             timeStabilityInverse);
        cs.AddLoopConstraint(idxRightFoot, 0, X_zero, X_zero, SpatialVector(0, 0, 0, 0, 0, 1), baumgarteEnabled,
                             timeStabilityInverse);
        cs.AddLoopConstraint(idxRightFoot, 0, X_zero, X_zero, SpatialVector(1, 0, 0, 0, 0, 0), baumgarteEnabled,
                             timeStabilityInverse);
        cs.AddLoopConstraint(idxRightFoot, 0, X_zero, X_zero, SpatialVector(0, 1, 0, 0, 0, 0), baumgarteEnabled,
                             timeStabilityInverse);
        cs.AddLoopConstraint(idxRightFoot, 0, X_zero, X_zero, SpatialVector(0, 0, 1, 0, 0, 0), baumgarteEnabled,
                             timeStabilityInverse);
...
TEST_CASE_METHOD(PlanarBipedFloatingBase, __FILE__ "_TestCorrectness", "")
{
    ...
    bool isCompatible = isConstrainedSystemFullyActuated(model, q, qd, cs[0]);
    CHECK(isCompatible);
    ...
    InverseDynamicsConstraintsRelaxed(model, q, qd, qddTarget, cs[0], qddIDCR, tauIDCR);
    lambdaIdc = cs[0].force;
    ...
    // Test to see if this model is compatiable with the exact IDC operator
    isCompatible = isConstrainedSystemFullyActuated(model, q, qd, cs[1]);
    CHECK(isCompatible == false);

    // Check the relaxed method
    qddTarget.setZero();
    InverseDynamicsConstraintsRelaxed(model, q, qd, qddTarget, cs[1], qddIDCR, tauIDCR);
...
TEST_CASE_METHOD(SpatialBipedFloatingBase, __FILE__ "_TestCorrectness2", "")
{
    ...
    // Test to see if this model is compatiable with the exact IDC operator
    bool isCompatible = isConstrainedSystemFullyActuated(model, q, qd, cs);
    CHECK(isCompatible);
    ...
    InverseDynamicsConstraintsRelaxed(model, q, qd, qddTarget, cs, qddIDCR, tauIDCR);
    lambdaIdc = cs.force;
...
TEST_CASE(__FILE__ "_CorrectnessTestWithSinglePlanarPendulum", "")
{
    ...
    SinglePendulumAbsoluteCoordinates spa = SinglePendulumAbsoluteCoordinates();

    // Without any joint coordinates
    SinglePendulumJointCoordinates spj = SinglePendulumJointCoordinates();
    ...
    Vector3d r010 = CalcBodyToBaseCoordinates(spj.model, spj.q, spj.idB1, Vector3d(0., 0., 0.), true);
    ...
    ForwardDynamicsConstraintsDirect(spa.model, spa.q, spa.qd, spa.tau, spa.cs, spa.qdd);
    ...
    // Test to see if this model is compatiable with the exact IDC operator
    bool isCompatible = isConstrainedSystemFullyActuated(spa.model, spa.q, spa.qd, spa.cs);
    CHECK(isCompatible);

    InverseDynamicsConstraints(spa.model, spa.q, spa.qd, qddDesired, spa.cs, qddIdc, tauIdc);
    ...
    InverseDynamicsConstraintsRelaxed(spa.model, spa.q, spa.qd, qddDesired, spa.cs, qddIdc, tauIdc);
...
TEST_CASE(__FILE__ "_CorrectnessTestWithDoublePerpendicularPendulum", "")
{
    ...
    // Test to see if this model is compatiable with the exact IDC operator
    bool isCompatible = isConstrainedSystemFullyActuated(dba.model, dba.q, dba.qd, dba.cs);
    CHECK(isCompatible);

    InverseDynamicsConstraints(dba.model, dba.q, dba.qd, qddDesired, dba.cs, qddIdc, tauIdc);
    ...
    InverseDynamicsConstraintsRelaxed(dba.model, dba.q, dba.qd, qddDesired, dba.cs, qddIdc, tauIdc);
...
TEST_CASE(__FILE__ "_CorrectnessTestWithUnderactuatedCartPendulum", "")
{
    ...
    Body cart1 = Body(m1, Vector3d(0., 0., 0.), Matrix3d(J1xx, 0., 0., 0., J1yy, 0., 0., 0., J1zz));

    Body link2 = Body(m2, Vector3d(0., -l2 * 0.5, 0.), Matrix3d(J2xx, 0., 0., 0., J2yy, 0., 0., 0., J2zz));

    Joint jointXYRz(SpatialVector(0., 0., 0., 1., 0., 0.), SpatialVector(0., 0., 0., 0., 0., 1.),
                    SpatialVector(0., 1., 0, 0., 0., 0.));
