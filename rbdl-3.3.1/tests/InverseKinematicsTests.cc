...
TEST_CASE_METHOD(Human36, __FILE__ "_ChainSinglePointConstraint", "")
{
    ...
    auto target_position = CalcBodyToBaseCoordinates(*model, q, body_id_emulated[BodyFootRight], local_point);
...
TEST_CASE_METHOD(Human36, __FILE__ "_ManyPointConstraints", "")
{
    ...
    UpdateKinematicsCustom(*model, &q, NULL, NULL);
    auto target_position1 = CalcBodyToBaseCoordinates(*model, q, body_id_emulated[BodyFootRight], local_point1);
    auto target_position2 = CalcBodyToBaseCoordinates(*model, q, body_id_emulated[BodyFootLeft], local_point2);
    auto target_position3 = CalcBodyToBaseCoordinates(*model, q, body_id_emulated[BodyHandRight], local_point3);
    auto target_position4 = CalcBodyToBaseCoordinates(*model, q, body_id_emulated[BodyHandLeft], local_point4);
    auto target_position5 = CalcBodyToBaseCoordinates(*model, q, body_id_emulated[BodyHead], local_point5);
    ...
    auto result_position1 = CalcBodyToBaseCoordinates(*model, qres, body_id_emulated[BodyFootRight], local_point1);
    auto result_position2 = CalcBodyToBaseCoordinates(*model, qres, body_id_emulated[BodyFootLeft], local_point2);
    auto result_position3 = CalcBodyToBaseCoordinates(*model, qres, body_id_emulated[BodyHandRight], local_point3);
    auto result_position4 = CalcBodyToBaseCoordinates(*model, qres, body_id_emulated[BodyHandLeft], local_point4);
    auto result_position5 = CalcBodyToBaseCoordinates(*model, qres, body_id_emulated[BodyHead], local_point5);

    CHECK_THAT(target_position1, AllCloseVector(result_position1, TEST_PREC, TEST_PREC));
    CHECK_THAT(target_position2, AllCloseVector(result_position2, TEST_PREC, TEST_PREC));
    CHECK_THAT(target_position3, AllCloseVector(result_position3, TEST_PREC, TEST_PREC));
    CHECK_THAT(target_position4, AllCloseVector(result_position4, TEST_PREC, TEST_PREC));
    CHECK_THAT(target_position5, AllCloseVector(result_position5, TEST_PREC, TEST_PREC));
}
...
TEST_CASE_METHOD(Human36, __FILE__ "_ChainSingleBodyOrientation", "")
{
    ...
    UpdateKinematicsCustom(*model, &q, NULL, NULL);
    auto target_orientation = CalcBodyWorldOrientation(*model, q, body_id_emulated[BodyFootRight], false);
...
TEST_CASE_METHOD(Human36, __FILE__ "_ManyBodyOrientations", "")
{
    ...
    UpdateKinematicsCustom(*model, &q, NULL, NULL);
    auto target_orientation1 = CalcBodyWorldOrientation(*model, q, body_id_emulated[BodyFootRight], false);
    auto target_orientation2 = CalcBodyWorldOrientation(*model, q, body_id_emulated[BodyFootLeft], false);
    auto target_orientation3 = CalcBodyWorldOrientation(*model, q, body_id_emulated[BodyHandRight], false);
    auto target_orientation4 = CalcBodyWorldOrientation(*model, q, body_id_emulated[BodyHandLeft], false);
    auto target_orientation5 = CalcBodyWorldOrientation(*model, q, body_id_emulated[BodyHead], false);
    ...
    CHECK(result);
    CHECK_THAT(0., IsClose(std::min(cs.error_norm, cs.delta_q_norm), TEST_PREC, TEST_PREC));

    UpdateKinematicsCustom(*model, &qres, NULL, NULL);
    auto result_orientation1 = CalcBodyWorldOrientation(*model, qres, body_id_emulated[BodyFootRight], false);
    auto result_orientation2 = CalcBodyWorldOrientation(*model, qres, body_id_emulated[BodyFootLeft], false);
    auto result_orientation3 = CalcBodyWorldOrientation(*model, qres, body_id_emulated[BodyHandRight], false);
    auto result_orientation4 = CalcBodyWorldOrientation(*model, qres, body_id_emulated[BodyHandLeft], false);
    auto result_orientation5 = CalcBodyWorldOrientation(*model, qres, body_id_emulated[BodyHead], false);

    CHECK_THAT(target_orientation1, AllCloseMatrix(result_orientation1, TEST_PREC, TEST_PREC));
    CHECK_THAT(target_orientation2, AllCloseMatrix(result_orientation2, TEST_PREC, TEST_PREC));
    CHECK_THAT(target_orientation3, AllCloseMatrix(result_orientation3, TEST_PREC, TEST_PREC));
    CHECK_THAT(target_orientation4, AllCloseMatrix(result_orientation4, TEST_PREC, TEST_PREC));
    CHECK_THAT(target_orientation5, AllCloseMatrix(result_orientation5, TEST_PREC, TEST_PREC));
}
