...
TEST_CASE_METHOD(TwoArms12DoF, __FILE__ "_TestAngularMomentumSimple2", "")
{
    ...
    CHECK_THAT(Vector3d(3.3, 2.54, 1.5), AllCloseVector(angular_momentum, 1.0e-1, 1.0e-1));
...
void TestCoMAccelerationUsingFD(T &obj, const double TOL = 1e-8)
{
    ...
    Utils::CalcCenterOfMass(*obj.model, obj.Q, obj.QDot, &obj.QDDot, mass, com, &com_vec, &com_acc_nom, &ang_mom,
                            &ch_ang_mom_nom);
