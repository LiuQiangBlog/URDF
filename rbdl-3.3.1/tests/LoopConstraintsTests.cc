...
struct FourBarLinkage
{
    ...
    FourBarLinkage()
        : model(), cs(), q(), qd(), qdd(), tau(), l1(2.), l2(2.), m1(2.), m2(2.), idB1(0), idB2(0), idB3(0), idB4(0),
          idB5(0), X_p(Xtrans(Vector3d(l2, 0., 0.))), X_s(Xtrans(Vector3d(0., 0., 0.)))
    {
        ...
        cs.AddLoopConstraint(idB2, idB5, X_p, X_s, SpatialVector(0, 0, 0, 1, 0, 0), bgStab, 0.1, "LoopXY_Rz",
                             userDefinedId);
...
struct FloatingFourBarLinkage
{
    ...
    FloatingFourBarLinkage()
        : model(), cs(), q(), qd(), qdd(), tau(), l1(2.), l2(2.), m1(2.), m2(2.), idB0(0), idB1(0), idB2(0), idB3(0),
          idB4(0), idB5(0), X_p(Xtrans(Vector3d(l2, 0., 0.))), X_s(Xtrans(Vector3d(0., 0., 0.)))
    {

        link1 = Body(m1, Vector3d(0.5 * l1, 0., 0.), Vector3d(0., 0., m1 * l1 * l1 / 3.));
        link2 = Body(m2, Vector3d(0.5 * l2, 0., 0.), Vector3d(0., 0., m2 * l2 * l2 / 3.));
        Vector3d vector3d_zero = Vector3d::Zero();
        Body virtual_body(0., vector3d_zero, vector3d_zero);
        Joint joint_trans(JointTypeTranslationXYZ);
        Joint joint_rev_z(JointTypeRevoluteZ);

        idB0 = model.AddBody(0, Xtrans(Vector3d(0., 0., 0.)), joint_trans, virtual_body);
        ...
        cs.AddContactConstraint(idB0, Vector3d::Zero(), Vector3d(1., 0., 0.), "ContactXYZ", userId);
        ...
        cs.AddLoopConstraint(idB2, idB5, X_p, X_s, SpatialVector(0., 0., 0., 1., 0., 0.), true, 0.1, "LoopXY_Rz");
        cs.AddLoopConstraint(idB2, idB5, X_p, X_s, SpatialVector(0., 0., 0., 0., 1., 0.));
        cs.AddLoopConstraint(idB2, idB5, X_p, X_s, SpatialVector(0., 0., 1., 0., 0., 0.));
...
struct SliderCrank3D
{
    ...
    SliderCrank3D() : model(), cs(), q(), qd(), id_p(0), id_s(0), X_p(), X_s()
    {
        ...
        double crank_link1_height = crank_link2_length - crank_link1_length + slider_height;

        Body slider(slider_mass, Vector3d::Zero(), Vector3d(1., 1., 1.));
        Body crankLink1(crank_link1_mass, Vector3d(0.5 * crank_link1_length, 0., 0.),
                        Vector3d(0., 0., crank_link1_mass * crank_link1_length * crank_link1_length / 3.));
        Body crankLink2(
            crank_link2_mass, Vector3d(0.5 * crank_link2_length, 0., 0.),
            Vector3d(crank_link2_mass * crank_link2_radius * crank_link2_radius / 2.,
                     crank_link2_mass *
                         (3. * crank_link2_radius * crank_link2_radius + crank_link2_length * crank_link2_length) / 12.,
                     crank_link2_mass *
                         (3. * crank_link2_radius * crank_link2_radius + crank_link2_length * crank_link2_length) /
                         12.));
        ...
        unsigned int id_b1 = model.AddBody(0, Xroty(-0.5 * M_PI) * Xtrans(Vector3d(0., 0., crank_link1_height)),
                                           joint_rev_z, crankLink1);
        id_s =
            model.AddBody(id_b1, Xroty(M_PI) * Xtrans(Vector3d(crank_link1_length, 0., 0.)), joint_sphere, crankLink2);
        ...
        cs.AddLoopConstraint(id_p, id_s, X_p, X_s, SpatialVector(0, 0, 0, 1, 0, 0), true, 0.1, "LoopXYZ_Rz", conId);
        ...
        Matrix3d rot_ps = (CalcBodyWorldOrientation(model, q, id_p).transpose() * X_p.E).transpose() *
        ...
        REQUIRE((CalcBodyToBaseCoordinates(model, q, id_p, X_p.r) - CalcBodyToBaseCoordinates(model, q, id_s, X_s.r))
                    .norm() < TEST_PREC);
    }
...
struct SliderCrank3DSphericalJoint
{
    ...
    SliderCrank3DSphericalJoint() : model(), cs(), q(), qd(), id_p(0), id_s(0), X_p(), X_s()
    {
        ...
        bool append = true;
        cs.AddLoopConstraint(id_p, id_s, X_p, X_s, SpatialVector(0, 0, 0, 1, 0, 0), true, 0.1, "LoopXYZ_Rz", 0);
...
TEST_CASE(__FILE__ "_TestExtendedConstraintFunctionsLoop", "")
{
    ...
    Vector3d r010 = CalcBodyToBaseCoordinates(dbj.model, dbj.q, dbj.idB1, Vector3d(0., 0., 0.), true);
    Vector3d r020 = CalcBodyToBaseCoordinates(dbj.model, dbj.q, dbj.idB2, Vector3d(0., 0., 0.), true);
    Vector3d r030 = CalcBodyToBaseCoordinates(dbj.model, dbj.q, dbj.idB2, Vector3d(dbj.l2, 0., 0.), true);
    ...
    CHECK_THAT(dr010xErr * dba.l1 * cos(dbj.q[0]), IsClose(errd[5 + 4], TEST_PREC, TEST_PREC));
    ...
    CHECK_THAT(-dr010xErr * cos(dbj.q[0]), IsClose(derrGroup1[0], TEST_PREC, TEST_PREC));
    CHECK_THAT(dr010xErr * sin(dbj.q[0]), IsClose(derrGroup1[1], TEST_PREC, TEST_PREC));
    CHECK_THAT(0., IsClose(derrGroup1[2], TEST_PREC, TEST_PREC));
    CHECK_THAT(0., IsClose(derrGroup1[3], TEST_PREC, TEST_PREC));
    CHECK_THAT(dr010xErr * dba.l1 * cos(dbj.q[0]), IsClose(derrGroup1[4], TEST_PREC, TEST_PREC));
    ...
    ForwardDynamicsConstraintsDirect(dba.model, dba.q, dba.qd, dba.tau, dba.cs, dba.qdd);
    ...
    dba.cs.calcForces(idxRyLink1, dba.model, dba.q, dba.qd, bodyIds, bodyFrames, conForces, true);
    ...
    Vector3d r0C0 = CalcBodyToBaseCoordinates(dba.model, dba.q, dba.idB2, dba.r2C2);
    Vector3d r2C0 = r0C0 - r020;

    fp = dba.m2 * dba.model.gravity;

    Matrix3d r2C0x = Matrix3d(0, -r2C0[2], r2C0[1], r2C0[2], 0, -r2C0[0], -r2C0[1], r2C0[0], 0);
    tp = r2C0x * fp;
    ...
    Matrix3d Ep0 = CalcBodyWorldOrientation(dba.model, dba.q, dba.idB1).transpose() * bodyFrames[0].E;
    Matrix3d Es0 = CalcBodyWorldOrientation(dba.model, dba.q, dba.idB2).transpose() * bodyFrames[1].E;
    ...
    CHECK_THAT(bodyFrames[0].r, AllCloseVector(rp, TEST_PREC, TEST_PREC));
    ...
    CHECK_THAT(bodyFrames[1].r, AllCloseVector(rs, TEST_PREC, TEST_PREC));
    for (unsigned int i = 0; i < 3; ++i)
    {
        for (unsigned int j = 0; j < 3; ++j)
        {
    ...
    dba.cs.calcForces(idxRyLink1, dba.model, dba.q, dba.qd, bodyIds, bodyFrames, conForces, false);
    ...
    // Check the local transformations
    CHECK_THAT(bodyFrames[0].r, AllCloseVector(dba.X_p2.r, TEST_PREC, TEST_PREC));
    for (unsigned int i = 0; i < 3; ++i)
    {
        for (unsigned int j = 0; j < 3; ++j)
        {
    ...
    CHECK_THAT(bodyFrames[1].r, AllCloseVector(dba.X_s2.r, TEST_PREC, TEST_PREC));
    for (unsigned int i = 0; i < 3; ++i)
    {
        for (unsigned int j = 0; j < 3; ++j)
        {
...
TEST_CASE_METHOD(FourBarLinkage, __FILE__ "_TestFourBarLinkageConstraintJacobian", "")
{
    ...
    REQUIRE(
        (CalcBodyToBaseCoordinates(model, q, idB2, X_p.r) - CalcBodyToBaseCoordinates(model, q, idB5, X_s.r)).norm() <
        TEST_PREC);
    ...
    qd[3] = -1.;
    qd[4] = 0.;
    REQUIRE((CalcPointVelocity6D(model, q, qd, idB2, X_p.r) - CalcPointVelocity6D(model, q, qd, idB5, X_s.r)).norm() <
            TEST_PREC);
    ...
    CHECK_THAT(errRef, AllCloseVector(err, TEST_PREC, TEST_PREC));
...
TEST_CASE_METHOD(FourBarLinkage, __FILE__ "_TestFourBarLinkageConstraintsVelocityErrors", "")
{
    ...
    CHECK_THAT(errdRef, AllCloseVector(errd, TEST_PREC, TEST_PREC));
...
TEST_CASE_METHOD(FourBarLinkage, __FILE__ "_TestFourBarLinkageQAssembly", "")
{
    ...
    CHECK_THAT(CalcBodyToBaseCoordinates(model, q, idB2, X_p.r),
               AllCloseVector(CalcBodyToBaseCoordinates(model, q, idB5, X_s.r), TEST_PREC, TEST_PREC));
...
TEST_CASE_METHOD(FourBarLinkage, __FILE__ "_TestFourBarLinkageQDotAssembly", "")
{
    ...
    CHECK_THAT(CalcPointVelocity6D(model, q, qd, idB2, X_p.r),
               AllCloseVector(CalcPointVelocity6D(model, q, qd, idB5, X_s.r), TEST_PREC, TEST_PREC));
    CHECK_THAT(errRef, AllCloseVector(err, TEST_PREC, TEST_PREC));
...
TEST_CASE_METHOD(FourBarLinkage, __FILE__ "_TestFourBarLinkageForwardDynamics", "")
{
    ...
    REQUIRE((CalcPointVelocity(model, q, qd, idB2, X_p.r) - CalcPointVelocity(model, q, qd, idB5, X_s.r)).norm() <
            TEST_PREC);
...
TEST_CASE_METHOD(FourBarLinkage, __FILE__ "_FourBarLinkageImpulse", "")
{
    ...
    CHECK_THAT(cs.v_plus, AllCloseVector(errd, TEST_PREC, TEST_PREC));
...
TEST_CASE_METHOD(SliderCrank3D, __FILE__ "_TestSliderCrank3DConstraintErrors", "")
{
    ...
    rotationVec =
        -0.5 * Vector3d(rot_ps(1, 2) - rot_ps(2, 1), rot_ps(2, 0) - rot_ps(0, 2), rot_ps(0, 1) - rot_ps(1, 0));
...
TEST_CASE_METHOD(SliderCrank3D, __FILE__ "_TestSliderCrank3DAssemblyQ", "")
{
    ...
    CHECK(success);
    CHECK_THAT(pos_p, AllCloseVector(pos_s, TEST_PREC, TEST_PREC));
...
TEST_CASE_METHOD(SliderCrank3D, __FILE__ "_TestSliderCrank3DForwardDynamics", "")
{
    ...
    Matrix3d rot_ps = (CalcBodyWorldOrientation(model, q, id_p).transpose() * X_p.E).transpose() *
                      CalcBodyWorldOrientation(model, q, id_s).transpose() * X_s.E;
    REQUIRE(
        (CalcBodyToBaseCoordinates(model, q, id_p, X_p.r) - CalcBodyToBaseCoordinates(model, q, id_p, X_p.r)).norm() <
        TEST_PREC);
    REQUIRE(rot_ps(0, 1) - rot_ps(0, 1) < TEST_PREC);
    REQUIRE((CalcPointVelocity6D(model, q, qd, id_p, X_p.r) - CalcPointVelocity6D(model, q, qd, id_p, X_p.r)).norm() <
            TEST_PREC);
...
TEST_CASE_METHOD(SliderCrank3D, __FILE__ "_TestSliderCrank3DImpulse", "")
{
    ...
    CHECK_THAT(cs.v_plus, AllCloseVector(errdDirect, TEST_PREC, TEST_PREC));
    ...
    CHECK_THAT(cs.v_plus, AllCloseVector(errdSpaceSparse, TEST_PREC, TEST_PREC));
    ...
    CHECK_THAT(cs.v_plus, AllCloseVector(errdNullSpace, TEST_PREC, TEST_PREC));
...
TEST_CASE_METHOD(FloatingFourBarLinkage, __FILE__ "_TestFloatingFourBarLinkageConstraintErrors", "")
{
    ...
    CHECK_THAT(Vector3d(1., 2., 3.), AllCloseVector(pos0, TEST_PREC, TEST_PREC));
...
TEST_CASE_METHOD(FloatingFourBarLinkage, __FILE__ "_TestFloatingFourBarLinkageQAssembly", "")
{
    ...
    REQUIRE((CalcBodyToBaseCoordinates(model, qRef, idB2, X_p.r) - CalcBodyToBaseCoordinates(model, qRef, idB5, X_s.r))
                .norm() < TEST_PREC);
    ...
    CHECK_THAT(CalcBodyToBaseCoordinates(model, q, idB2, X_p.r),
               AllCloseVector(CalcBodyToBaseCoordinates(model, q, idB5, X_s.r), TEST_PREC, TEST_PREC));
    CHECK_THAT(inRange(q[3] + q[4]), IsClose(inRange(q[5] + q[6] + q[7]), TEST_PREC, TEST_PREC));
...
TEST_CASE(__FILE__ "_ConstraintCorrectnessTest", "")
{
    ...
    SpatialVector v010 = CalcPointVelocity6D(dbj.model, dbj.q, dbj.qd, dbj.idB1, Vector3d(0., 0., 0.), true);
    SpatialVector v020 = CalcPointVelocity6D(dbj.model, dbj.q, dbj.qd, dbj.idB2, Vector3d(0., 0., 0.), true);
    SpatialVector v030 = CalcPointVelocity6D(dbj.model, dbj.q, dbj.qd, dbj.idB2, Vector3d(dbj.l2, 0., 0.), true);

    SpatialVector a010 =
        CalcPointAcceleration6D(dbj.model, dbj.q, dbj.qd, dbj.qdd, dbj.idB1, Vector3d(0., 0., 0.), true);
    SpatialVector a020 =
        CalcPointAcceleration6D(dbj.model, dbj.q, dbj.qd, dbj.qdd, dbj.idB2, Vector3d(0., 0., 0.), true);
    SpatialVector a030 =
        CalcPointAcceleration6D(dbj.model, dbj.q, dbj.qd, dbj.qdd, dbj.idB2, Vector3d(dbj.l2, 0., 0.), true);
    ...
    ForwardDynamicsConstraintsDirect(dba.model, dba.q, dba.qd, dba.tau, dba.cs, dba.qdd);
    SpatialVector a010c =
        CalcPointAcceleration6D(dba.model, dba.q, dba.qd, dba.qdd, dba.idB1, Vector3d(0., 0., 0.), true);
    SpatialVector a020c =
        CalcPointAcceleration6D(dba.model, dba.q, dba.qd, dba.qdd, dba.idB2, Vector3d(0., 0., 0.), true);
    SpatialVector a030c =
        CalcPointAcceleration6D(dba.model, dba.q, dba.qd, dba.qdd, dba.idB2, Vector3d(dba.l2, 0., 0.), true);
    ...
    ForwardDynamicsConstraintsNullSpace(dba.model, dba.q, dba.qd, dba.tau, dba.cs, dba.qdd);
    a010c = CalcPointAcceleration6D(dba.model, dba.q, dba.qd, dba.qdd, dba.idB1, Vector3d(0., 0., 0.), true);
    a020c = CalcPointAcceleration6D(dba.model, dba.q, dba.qd, dba.qdd, dba.idB2, Vector3d(0., 0., 0.), true);
    a030c = CalcPointAcceleration6D(dba.model, dba.q, dba.qd, dba.qdd, dba.idB2, Vector3d(dba.l2, 0., 0.), true);
    ...
    ForwardDynamicsConstraintsRangeSpaceSparse(dba.model, dba.q, dba.qd, dba.tau, dba.cs, dba.qdd);
    a010c = CalcPointAcceleration6D(dba.model, dba.q, dba.qd, dba.qdd, dba.idB1, Vector3d(0., 0., 0.), true);
    a020c = CalcPointAcceleration6D(dba.model, dba.q, dba.qd, dba.qdd, dba.idB2, Vector3d(0., 0., 0.), true);
    a030c = CalcPointAcceleration6D(dba.model, dba.q, dba.qd, dba.qdd, dba.idB2, Vector3d(dba.l2, 0., 0.), true);
