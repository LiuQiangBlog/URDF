...
struct CustomEulerZYXJoint : public CustomJoint
{
    ...
    virtual void jcalc(Model &model, unsigned int joint_id, const Math::VectorNd &q, const Math::VectorNd &qdot)
    {
        ...
        SpatialTransform X_J(Matrix3d(c0 * c1, s0 * c1, -s1, c0 * s1 * s2 - s0 * c2, s0 * s1 * s2 + c0 * c2, c1 * s2,
        ...
        model.c_J[joint_id].set(-c1 * qdot0 * qdot1,
    ...
    virtual void jcalc_X_lambda_S(Model &model, unsigned int joint_id, const Math::VectorNd &q)
    {
        ...
        model.X_lambda[joint_id] =
            SpatialTransform(Matrix3d(c0 * c1, s0 * c1, -s1, c0 * s1 * s2 - s0 * c2, s0 * s1 * s2 + c0 * c2, c1 * s2,
                                      c0 * s1 * c2 + s0 * s2, s0 * s1 * c2 - c0 * s2, c1 * c2),
                             Vector3d(0., 0., 0.)) *
            model.X_T[joint_id];
...
struct CustomJointMultiBodyFixture
{
    CustomJointMultiBodyFixture()
    {
        ...
        unsigned int reference_body_id11 =
            reference1.AddBody(reference_body_id10, SpatialTransform(rm1, r1), Joint(JointTypeEulerZYX), body12);

        unsigned int reference_body_id12 =
            reference1.AddBody(reference_body_id11, SpatialTransform(rm2, r2), Joint(JointTypeRevoluteX), body13);
        ...
        unsigned int custom_body_id11 =
            custom1.AddBody(custom_body_id10, SpatialTransform(rm1, r1), Joint(JointTypeEulerZYX), body12);
        ...
        unsigned int reference_body_id21 =
            reference2.AddBody(reference_body_id20, SpatialTransform(rm2, r2), Joint(JointTypeRevoluteX), body12);

        unsigned int reference_body_id22 =
            reference2.AddBody(reference_body_id21, SpatialTransform(rm1, r1), Joint(JointTypeEulerZYX), body13);
        ...
        unsigned int custom_body_id21 =
            custom2.AddBodyCustomJoint(custom_body_id20, SpatialTransform(rm2, r2), &custom_rx_joint1, body12);
        ...
        unsigned int reference_body_id31 =
            reference3.AddBody(reference_body_id30, SpatialTransform(rm1, r1), Joint(JointTypeEulerZYX), body12);

        unsigned int reference_body_id32 =
            reference3.AddBody(reference_body_id31, SpatialTransform(rm2, r2), Joint(JointTypeRevoluteX), body13);
        ...
        unsigned int custom_body_id31 =
            custom3.AddBody(custom_body_id30, SpatialTransform(rm1, r1), Joint(JointTypeEulerZYX), body12);
...
TEST_CASE_METHOD(CustomJointMultiBodyFixture, __FILE__ "_UpdateKinematics", "")
{
    ...
    for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        Matrix3d Eref = reference_model.at(idx).X_base[reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1)].E;
        Matrix3d Ecus = custom_model.at(idx).X_base[custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1)].E;

        Matrix3d Eerr = Eref - Ecus;

        CHECK_THAT(reference_model.at(idx).X_base[reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1)].E,
                   AllCloseMatrix(custom_model.at(idx).X_base[custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1)].E,
                                  TEST_PREC, TEST_PREC));

        CHECK_THAT(reference_model.at(idx).v[reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1)],
                   AllCloseVector(custom_model.at(idx).v[custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1)], TEST_PREC,
                                  TEST_PREC));

        CHECK_THAT(reference_model.at(idx).a[reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1)],
                   AllCloseVector(custom_model.at(idx).a[custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1)], TEST_PREC,
                                  TEST_PREC));
    }
}
...
TEST_CASE_METHOD(CustomJointMultiBodyFixture, __FILE__ "_Jacobians", "")
{
    ...
    for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        CalcBodySpatialJacobian(reference_model.at(idx), q.at(idx), reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1),
                                Gref);
        ...
        CalcPointJacobian6D(reference_model.at(idx), q.at(idx), reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1),
                            point_position, Gref);

        CalcPointJacobian6D(custom_model.at(idx), q.at(idx), custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1),
                            point_position, Gcus);
        ...
        CalcPointJacobian(reference_model.at(idx), q.at(idx), reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1),
                          point_position, GrefPt);

        CalcPointJacobian(custom_model.at(idx), q.at(idx), custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1),
...
TEST_CASE_METHOD(CustomJointMultiBodyFixture, __FILE__ "_InverseDynamics", "")
{
    ...
    for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        CHECK_THAT(tauRef, AllCloseVector(tauCus, TEST_PREC, TEST_PREC));
    }
}
...
TEST_CASE_METHOD(CustomJointMultiBodyFixture, __FILE__ "_CompositeRigidBodyAlgorithm", "")
{
    ...
    for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        CHECK_THAT(qddot_crba_ref, AllCloseVector(qddot_crba_cus, TEST_PREC, TEST_PREC));
    }
}
...
TEST_CASE_METHOD(CustomJointMultiBodyFixture, __FILE__ "_ForwardDynamics", "")
{
    ...
    for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        CHECK_THAT(qddotRef, AllCloseVector(qddotCus, TEST_PREC, TEST_PREC));
    }
}
...
TEST_CASE_METHOD(CustomJointMultiBodyFixture, __FILE__ "_CalcMInvTimestau", "")
{
    ...
    for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        // check.
        CHECK_THAT(qddot_minv_ref, AllCloseVector(qddot_minv_cus, TEST_PREC, TEST_PREC));
    }
}
...
TEST_CASE_METHOD(CustomJointMultiBodyFixture, __FILE__ "_ForwardDynamicsContactsKokkevis", "")
{
    ...
    for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        if (dof > 1)
        {
            ...
            // Reference
            constraint_set_ref.AddContactConstraint(reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1), contact_point,
            ...
            // Custom
            constraint_set_cus.AddContactConstraint(custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1), contact_point,
                                                    Vector3d(1., 0., 0.), "ground_x");
            ...
            ComputeConstraintImpulsesDirect(reference_model.at(idx), q.at(idx), qdot.at(idx), constraint_set_ref,
                                            qdot_plus_ref);

            ForwardDynamicsContactsKokkevis(reference_model.at(idx), q.at(idx), qdot_plus_ref, tau.at(idx),
                                            constraint_set_ref, qddot_ref);

            ComputeConstraintImpulsesDirect(custom_model.at(idx), q.at(idx), qdot.at(idx), constraint_set_cus,
                                            qdot_plus_cus);

            ForwardDynamicsContactsKokkevis(custom_model.at(idx), q.at(idx), qdot_plus_cus, tau.at(idx),
            ...
            CHECK_THAT(qddot_ref, AllCloseVector(qddot_cus, TEST_PREC, TEST_PREC));
        }
    }
}
