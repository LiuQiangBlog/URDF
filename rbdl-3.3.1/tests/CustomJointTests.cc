...
struct CustomEulerZYXJoint : public CustomJoint
{
    ...
    virtual void jcalc(Model &model, unsigned int joint_id, const Math::VectorNd &q, const Math::VectorNd &qdot)
    {
        ...
        model.X_J[joint_id].E = Matrix3d(c0 * c1, s0 * c1, -s1, c0 * s1 * s2 - s0 * c2, s0 * s1 * s2 + c0 * c2, c1 * s2,
...
TEST_CASE_METHOD(CustomJointFixture, __FILE__ "_UpdateKinematics", "")
{
    ...
    CHECK_THAT(reference_model.v[reference_body_id], AllCloseVector(custom_model.v[custom_body_id], 0., 0.));

    CHECK_THAT(reference_model.a[reference_body_id], AllCloseVector(custom_model.a[custom_body_id], 0., 0.));
}
