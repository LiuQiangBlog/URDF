...
struct SphericalJoint
{
    SphericalJoint()
    {
        ...
        joint_rot_zyx   = Joint(SpatialVector(0., 0., 1., 0., 0., 0.), SpatialVector(0., 1., 0., 0., 0., 0.),
    ...
    for (unsigned int i = 1; i < multdof3_model.mJoints.size(); i++)
    {
        ...
        if (multdof3_model.mJoints[i].mJointType == JointTypeSpherical)
        {
            Quaternion quat = Quaternion::fromZYXAngles(
                Vector3d(q_emulated[q_index + 0], q_emulated[q_index + 1], q_emulated[q_index + 2]));
...
TEST_CASE_METHOD(SphericalJoint, __FILE__ "_TestOrientation", "")
{
    ...
    CHECK_THAT(emu_orientation, AllCloseMatrix(sph_orientation, TEST_PREC, TEST_PREC));
}
...
TEST_CASE_METHOD(SphericalJoint, __FILE__ "_TestUpdateKinematics", "")
{
    ...
    Vector3d a = angular_acceleration_from_angle_rates(Vector3d(emuQ[3], emuQ[2], emuQ[1]),
    ...
    CHECK_THAT(emulated_model.v[emu_body_id], AllCloseVector(multdof3_model.v[sph_body_id], TEST_PREC, TEST_PREC));
    CHECK_THAT(emulated_model.a[emu_body_id], AllCloseVector(multdof3_model.a[sph_body_id], TEST_PREC, TEST_PREC));

    UpdateKinematics(multdof3_model, sphQ, sphQDot, sphQDDot);

    CHECK_THAT(emulated_model.v[emu_child_id], AllCloseVector(multdof3_model.v[sph_child_id], TEST_PREC, TEST_PREC));
    CHECK_THAT(emulated_model.a[emu_child_id], AllCloseVector(multdof3_model.a[sph_child_id], TEST_PREC, TEST_PREC));
}
...
TEST_CASE_METHOD(SphericalJoint, __FILE__ "_TestContactsLagrangian", "")
{
    ...
    CHECK_THAT(emu_force_lagrangian, AllCloseVector(sph_force_lagrangian, TEST_PREC, TEST_PREC));
}
...
TEST_CASE_METHOD(SphericalJoint, __FILE__ "_TestContacts", "")
{
    ...
    CHECK_THAT(emu_force_kokkevis, AllCloseVector(sph_force_kokkevis, TEST_PREC, TEST_PREC));
}
...
TEST_CASE_METHOD(SphericalJoint, __FILE__ "_TestEulerZYXvsEmulatedLagrangian", "")
{
    ...
    CHECK_THAT(QDDot_emu, AllCloseVector(QDDot_eulerzyx, TEST_PREC, TEST_PREC));
}
...
TEST_CASE_METHOD(SphericalJoint, __FILE__ "_TestEulerZYXvsEmulatedContacts", "")
{
    ...
    CHECK_THAT(QDDot_emu, AllCloseVector(QDDot_eulerzyx, TEST_PREC * QDDot_emu.norm(), TEST_PREC * QDDot_emu.norm()));
...
TEST_CASE_METHOD(SphericalJoint, __FILE__ "_TestEulerZYXvsEmulatedCRBA", "")
{
    ...
    CHECK_THAT(H_emulated, AllCloseMatrix(H_eulerzyx, TEST_PREC, TEST_PREC));
}
...
TEST_CASE(__FILE__ "_TestJointTypeTranslationZYX", "")
{
    ...
    Joint joint_emulated(SpatialVector(0., 0., 0., 1., 0., 0.), SpatialVector(0., 0., 0., 0., 1., 0.),
    ...
    CHECK_THAT(id_emulated, AllCloseVector(id_3dof, TEST_PREC * id_emulated.norm(), TEST_PREC * id_emulated.norm()));
...
TEST_CASE(__FILE__ "_TestJointTypeEulerXYZ", "")
{
    ...
    Body  body(1., Vector3d(1., 2., 1.), Matrix3d(1., 0., 0, 0., 1., 0., 0., 0., 1.));
    Joint joint_emulated(SpatialVector(1., 0., 0., 0., 0., 0.), SpatialVector(0., 1., 0., 0., 0., 0.),
    ...
    CHECK_THAT(model_emulated.X_base[3].E, AllCloseMatrix(model_3dof.X_base[1].E, TEST_PREC, TEST_PREC));
    CHECK_THAT(model_emulated.v[3], AllCloseVector(model_3dof.v[1], TEST_PREC, TEST_PREC));
...
TEST_CASE(__FILE__ "_TestJointTypeEulerZXY", "")
{
    ...
    Body  body(1., Vector3d(1., 2., 1.), Matrix3d(1., 0., 0, 0., 1., 0., 0., 0., 1.));
    Joint joint_emulated(SpatialVector(0., 0., 1., 0., 0., 0.), SpatialVector(1., 0., 0., 0., 0., 0.),
...
TEST_CASE_METHOD(Human36, __FILE__ "_TestUpdateKinematics2", "")
{
    ...
    CHECK_THAT(model_emulated->v[body_id_emulated[BodyPelvis]],
               AllCloseVector(model_3dof->v[body_id_3dof[BodyPelvis]], TEST_PREC, TEST_PREC));
    CHECK_THAT(model_emulated->v[body_id_emulated[BodyThighRight]],
               AllCloseVector(model_3dof->v[body_id_3dof[BodyThighRight]], TEST_PREC, TEST_PREC));
    CHECK_THAT(model_emulated->v[body_id_emulated[BodyShankRight]],
               AllCloseVector(model_3dof->v[body_id_3dof[BodyShankRight]], TEST_PREC, TEST_PREC));
}
...
TEST_CASE_METHOD(Human36, __FILE__ "_TestNonlinearEffects", "")
{
    ...
    CHECK_THAT(nle_emulated,
               AllCloseVector(nle_3dof, TEST_PREC * nle_emulated.norm(), TEST_PREC * nle_emulated.norm()));
}
...
TEST_CASE_METHOD(Human36, __FILE__ "_TestContactsEmulatedLagrangianKokkevis", "")
{
    ...
    CHECK_THAT(qddot_lagrangian, AllCloseVector(qddot_kokkevis, TEST_PREC * qddot_lagrangian.norm(),
...
TEST_CASE_METHOD(Human36, __FILE__ "_TestContactsEmulatedLagrangianSparse", "")
{
    ...
    CHECK_THAT(qddot_lagrangian,
               AllCloseVector(qddot_sparse, TEST_PREC * qddot_lagrangian.norm(), TEST_PREC * qddot_lagrangian.norm()));
...
TEST_CASE_METHOD(Human36, __FILE__ "_TestContactsEmulatedLagrangianNullSpace", "")
{
    ...
    CHECK_THAT(qddot_lagrangian, AllCloseVector(qddot_nullspace, TEST_PREC * qddot_lagrangian.norm(),
                                                TEST_PREC * qddot_lagrangian.norm()));
...
TEST_CASE_METHOD(Human36, __FILE__ "_TestContactsEmulatedMultdofLagrangian", "")
{
    ...
    CHECK_THAT(qddot_emulated,
               AllCloseVector(qddot_3dof, TEST_PREC * qddot_emulated.norm(), TEST_PREC * qddot_emulated.norm()));
...
TEST_CASE_METHOD(Human36, __FILE__ "_TestContactsEmulatedMultdofSparse", "")
{
    ...
    ForwardDynamicsConstraintsRangeSpaceSparse(*model_emulated, q, qdot, tau, constraints_1B1C_emulated,
                                               qddot_emulated);
    ...
    ForwardDynamicsConstraintsRangeSpaceSparse(*model_emulated, q, qdot, tau, constraints_1B4C_emulated,
                                               qddot_emulated);
    ForwardDynamicsConstraintsRangeSpaceSparse(*model_3dof, q, qdot, tau, constraints_1B4C_3dof, qddot_3dof);
    CHECK_THAT(qddot_emulated,
               AllCloseVector(qddot_3dof, TEST_PREC * qddot_emulated.norm(), TEST_PREC * qddot_emulated.norm()));

    ForwardDynamicsConstraintsRangeSpaceSparse(*model_emulated, q, qdot, tau, constraints_4B4C_emulated,
                                               qddot_emulated);
...
TEST_CASE_METHOD(Human36, __FILE__ "_TestContactsEmulatedMultdofKokkevisSparse", "")
{
    ...
    CHECK_THAT(qddot_sparse,
               AllCloseVector(qddot_kokkevis, TEST_PREC * qddot_sparse.norm(), TEST_PREC * qddot_sparse.norm()));
...
TEST_CASE_METHOD(Human36, __FILE__ "_TestContactsEmulatedMultdofKokkevisMultiple", "")
{
    ...
    CHECK_THAT(qddot_kokkevis,
               AllCloseVector(qddot_kokkevis_2, TEST_PREC * qddot_kokkevis.norm(), TEST_PREC * qddot_kokkevis.norm()));
...
TEST_CASE_METHOD(Human36, __FILE__ "_SolveMInvTimesTau", "")
{
    ...
    CHECK_THAT(qddot_solve_llt,
               AllCloseVector(qddot_minv, TEST_PREC * qddot_minv.norm(), TEST_PREC * qddot_minv.norm()));
}
...
TEST_CASE_METHOD(Human36, __FILE__ "_SolveMInvTimesTauReuse", "")
{
    ...
    for (unsigned int j = 0; j < 1; j++)
    {
        ...
        CHECK_THAT(qddot_solve_llt,
                   AllCloseVector(qddot_minv, TEST_PREC * qddot_solve_llt.norm(), TEST_PREC * qddot_solve_llt.norm()));
    }
}
