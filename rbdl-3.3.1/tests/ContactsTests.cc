...
TEST_CASE(__FILE__ "_TestExtendedConstraintFunctionsContact ", "")
{
    ...
    ConstraintSet cs;
    cs.AddContactConstraint(boxId, Vector3d(-0.5, 0, 0), Vector3d(0., 1., 0.), "LeftCorner");
    cs.AddContactConstraint(boxId, Vector3d(-0.5, 0, 0), Vector3d(1., 0., 0.));
    cs.AddContactConstraint(boxId, Vector3d(0.5, 0, 0), Vector3d(0., 1., 0.), "RightCorner");
    cs.Bind(model);
    ...
    for (unsigned int i = 0; i < 3; ++i)
    {
        ...
        {
            CHECK_THAT(bodyFrames[idxBody].E(i, j), IsClose(eye(i, j), TEST_PREC, TEST_PREC));
        }
    }
    ...
    CHECK_THAT(bodyFrames[idxGround].r, AllCloseVector(r, TEST_PREC, TEST_PREC));
    for (unsigned int i = 0; i < 3; ++i)
    {
        for (unsigned int j = 0; j < 3; ++j)
        {
            CHECK_THAT(bodyFrames[idxGround].E(i, j), IsClose(eye(i, j), TEST_PREC, TEST_PREC));
        }
    }
    ...
    cs.calcBaumgarteStabilizationForces(gIdxLeft, model, posErrUpd, velErrUpd, bgForces);
    ...
    CHECK_THAT(constraintImpulses[0][3], IsClose(-vx1 * m1, TEST_PREC, TEST_PREC));
    CHECK_THAT(constraintImpulses[0][4], IsClose(-vy1 * m1 * 0.5, TEST_PREC, TEST_PREC));
...
TEST_CASE(__FILE__ "_TestForwardDynamicsConstraintsDirectSimple", "")
{
    ...
    unsigned int id = 11;
    unsigned int autoId =
        constraint_set.AddContactConstraint(contact_body_id, contact_point, Vector3d(1., 0., 0.), "ground_xyz", id);
    ...
    CHECK_THAT(Vector3d(0., 0., 0.), AllCloseVector(point_acceleration, TEST_PREC, TEST_PREC));
...
TEST_CASE(__FILE__ "_TestForwardDynamicsConstraintsDirectMoving ", "")
{
    ...
    constraint_set.AddContactConstraint(contact_body_id, contact_point, Vector3d(1., 0., 0.), "ground_xyz");
    constraint_set.AddContactConstraint(contact_body_id, contact_point, Vector3d(0., 1., 0.));
    constraint_set.AddContactConstraint(contact_body_id, contact_point, Vector3d(0., 0., 1.));
...
TEST_CASE_METHOD(FixedBase6DoF, __FILE__ "_ForwardDynamicsContactsSingleContact", "")
{
    ...
    point_accel_lagrangian =
        CalcPointAcceleration(*model, Q, QDot, QDDot_lagrangian, contact_body_id, contact_point, true);
    point_accel_contacts = CalcPointAcceleration(*model, Q, QDot, QDDot_contacts, contact_body_id, contact_point, true);

    CHECK_THAT(constraint_set_lagrangian.force[0], IsClose(constraint_set.force[0], TEST_PREC, TEST_PREC));
    CHECK_THAT(contact_normal.dot(point_accel_lagrangian),
               IsClose(contact_normal.dot(point_accel_contacts), TEST_PREC, TEST_PREC));
    CHECK_THAT(point_accel_lagrangian, AllCloseVector(point_accel_contacts, TEST_PREC, TEST_PREC));
    CHECK_THAT(QDDot_lagrangian, AllCloseVector(QDDot_contacts, TEST_PREC, TEST_PREC));
}
...
TEST_CASE_METHOD(FixedBase6DoF, __FILE__ "_ForwardDynamicsContactsSingleContactRotated", "")
{
    ...
    point_accel_contacts_opt =
        CalcPointAcceleration(*model, Q, QDot, QDDot_contacts_opt, contact_body_id, contact_point, true);

    CHECK_THAT(constraint_set_lagrangian.force[0], IsClose(constraint_set.force[0], TEST_PREC, TEST_PREC));
    CHECK_THAT(contact_normal.dot(point_accel_lagrangian),
               IsClose(contact_normal.dot(point_accel_contacts_opt), TEST_PREC, TEST_PREC));
    CHECK_THAT(point_accel_lagrangian, AllCloseVector(point_accel_contacts_opt, TEST_PREC, TEST_PREC));
    CHECK_THAT(QDDot_lagrangian, AllCloseVector(QDDot_contacts_opt, TEST_PREC, TEST_PREC));
}
...
TEST_CASE_METHOD(FixedBase6DoF, __FILE__ "_ForwardDynamicsContactsOptDoubleContact", "")
{
    ...
    CHECK_THAT(constraint_set_lagrangian.force, AllCloseVector(constraint_set.force, TEST_PREC, TEST_PREC));
...
TEST_CASE_METHOD(FixedBase6DoF9DoF, __FILE__ "_ForwardDynamicsContactsOptMultipleContactsMultipleBodiesMoving", "")
{
    ...
    CHECK_THAT(QDDot_lagrangian, AllCloseVector(QDDot, TEST_PREC, TEST_PREC));
}
...
TEST_CASE_METHOD(Human36, __FILE__ "_ForwardDynamicsContactsFixedBody", "")
{
    ...
    constraint_upper_trunk.AddContactConstraint(body_id_3dof[BodyUpperTrunk], Vector3d(1.1, 2.2, 3.3),
                                                Vector3d(1., 0., 0.));
    ...
    CHECK_THAT(qddot_lagrangian, AllCloseVector(qddot, TEST_PREC, TEST_PREC * qddot_lagrangian.norm() * 10.));
    CHECK_THAT(qddot_lagrangian, AllCloseVector(qddot_sparse, TEST_PREC, TEST_PREC * qddot_lagrangian.norm() * 10.));
}
...
TEST_CASE_METHOD(Human36, __FILE__ "_ForwardDynamicsContactsImpulses", "")
{
    ...
    CHECK_THAT(Vector3d(0., 0., 0.), AllCloseVector(heel_right_velocity, TEST_PREC, TEST_PREC));
}
