...
TEST_CASE_METHOD(ScrewJoint1DofFixedBase, __FILE__ "_UpdateKinematics", "")
{
    ...
    CHECK_THAT(Xrot(1, Vector3d(0, 0, 1)).E, AllCloseMatrix(model->X_base[roller].E, 0., 0.));
    CHECK_THAT(Vector3d(1., 0., 0.), AllCloseVector(model->X_base[roller].r, 0., 0.));
    CHECK_THAT(SpatialVector(0., 0., 2., cos(q[0]) * 2, -sin(q[0]) * 2., 0.), AllCloseVector(model->v[roller], 0., 0.));
    ...
    CHECK_THAT(a0, AllCloseVector(v0, 1e-5, 1e-5));
}
...
TEST_CASE_METHOD(ScrewJoint1DofFixedBase, __FILE__ "_Jacobians", "")
{
    ...
    CHECK_THAT(Vector3d(1 + cos(1), sin(1), 3), AllCloseVector(refPtBaseCoord, 0., 0.));
    ...
    CHECK_THAT(Gexpected, AllCloseMatrix(GrefPt, 0., 0.));
}
