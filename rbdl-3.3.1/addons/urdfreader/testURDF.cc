...
int main()
{
    std::string modelFile =
        "D:\\VideoNotes\\Handbook_For_Robot_Beginners\\Code\\Thirdparty\\rbdl-3.3.1\\addons\\urdfreader\\test.urdf";
    // LoadURDF
    {
        Model model;
        ...
        Model partial_model;
        modelLoaded =
            PartialURDFReadFromFile(modelFile.c_str(), &partial_model, "Mbase_link", {"arm_left_1_link"}, false);
    }
    // CenterOfMass
    {
    ...
    // Checks model COM against known values
    {
        Model model;
        bool modelLoaded =
            PartialURDFReadFromFile(modelFile.c_str(), &model, "torso_lift_link", {"arm_left_tool_link"}, false, false);
        ...
        Model gripper_model;
        bool gripperModelLoaded =
            PartialURDFReadFromFile(modelFile.c_str(), &gripper_model, "arm_left_tool_link", {}, false);
