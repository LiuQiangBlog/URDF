...
int main()
{
    std::string modelFile = "D:\\VideoNotes\\Handbook_For_Robot_Beginners\\Code\\Thirdparty\\rbdl-3.3.1\\addons\\urdfreader\\test.urdf";
    //LoadURDF
    {
        Model       model;
        bool modelLoaded = URDFReadFromFile(modelFile.c_str(), &model, false);
        Model partial_model;
        modelLoaded = PartialURDFReadFromFile(modelFile.c_str(), &partial_model, "Mbase_link", {"arm_left_1_link"}, false);
    }
    //CenterOfMass
    {
...
//        CHECK_THAT(model.mBodies.front().mMass, IsClose(0.01, TEST_PREC, TEST_PREC));
    ...
    {
        Model       model;
        bool modelLoaded = PartialURDFReadFromFile(modelFile.c_str(), &model, "torso_lift_link", {"arm_left_tool_link"}, false, false);
...
//        CHECK_THAT(mass, IsClose(8.035202, TEST_PREC, TEST_PREC));
        ...
        Model gripper_model;
        bool  gripperModelLoaded = PartialURDFReadFromFile(modelFile.c_str(), &gripper_model, "arm_left_tool_link", {}, false);
...
//        CHECK(gripper_model.qdot_size == 2);
        ...
        RigidBodyDynamics::UpdateKinematics(gripper_model, q_gripper_zero, qdot_gripper_zero, qdot_gripper_zero);

        SpatialRigidBodyInertia rbi_gripper = gripper_model.X_base[1].apply(gripper_model.I[1]);
...
//        CHECK_THAT(gripper_com.transpose()[2], IsClose(0.03504, TEST_PREC, TEST_PREC));
        ...
        double   gripper_mass;
        RigidBodyDynamics::Utils::CalcCenterOfMass(gripper_model, q_gripper_zero, qdot_gripper_zero, NULL, gripper_mass,
                                                   gripper_model_com);
...
//        CHECK_THAT(gripper_mass, IsClose(0.21996, TEST_PREC, TEST_PREC));
        // Check that the root link (1st link) has the weight of the fixed links
//        CHECK_THAT(gripper_model.mBodies.front().mMass, IsClose(0.90276, TEST_PREC, TEST_PREC));
    }
    return 0;
}
