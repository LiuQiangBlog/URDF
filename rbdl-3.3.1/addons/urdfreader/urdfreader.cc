...
{
    Joint rbdl_joint;
    if (urdf_joint->type == UrdfJointType::REVOLUTE || urdf_joint->type == UrdfJointType::CONTINUOUS)
    {
    ...
    else if (urdf_joint->type == UrdfJointType::FLOATING)
    {
        ...
        rbdl_joint = Joint(SpatialVector(0., 0., 0., 1., 0., 0.), SpatialVector(0., 0., 0., 0., 1., 0.),
    ...
    else if (urdf_joint->type == UrdfJointType::PLANAR)
    {
        ...
        ostringstream error_msg;
        error_msg << "Error while processing joint '" << urdf_joint->name << "': planar joints not yet supported!"
                  << endl;
        throw RBDLFileParseError(error_msg.str());
    }
    return rbdl_joint;
...
Body get_rbdl_body(ConstLinkPtr &urdf_link, bool is_root_link)
{
    ...
    if (urdf_link->inertial.has_value())
    {
        ...
        if (!is_root_link)
        {
            I->origin.rotation.RPY(link_inertial_rpy_temp.x, link_inertial_rpy_temp.y, link_inertial_rpy_temp.z);
        ...
        if (is_root_link)
        {
            if (link_inertial_mass == 0. &&
                (I->ixx != 0 || I->ixy != 0 || I->ixz != 0 || I->iyy != 0 || I->iyz != 0 || I->izz != 0))
            {
                              ...
                              bool                  verbose)
{
    ...
    for (j = 0; j < joint_names.size(); j++)
    {
        ...
        {
            ostringstream error_msg;
            error_msg << "Error while processing joint '" << urdf_joint->name << "': parent link '" << urdf_parent->name
        ...
        SpatialTransform rbdl_joint_frame = t3;
...
void construct_model(Model *rbdl_model, ModelPtr urdf_model, const string &root_link, bool floating_base, bool verbose)
{
                             ...
                             bool                  verbose)
{
    ...
    for (const std::string &tip_link : tip_links)
    {
        ...
        while (parent_link.compare(root_link) != 0)
        {
            ...
            {
                ostringstream error_msg;
                error_msg << "Error while processing tip link '" << tip_link << "' as reached root link is '"
                          << parent_link << "', couldn't find desired root link '" << root_link << "' in the tree"
                          << endl;
                throw RBDLFileParseError(error_msg.str());
            }
...
RBDL_ADDON_DLLAPI bool URDFReadFromFile(const char *filename, Model *model, bool floating_base, bool verbose)
{
...
RBDL_ADDON_DLLAPI bool URDFReadFromString(const char *model_xml_string, Model *model, bool floating_base, bool verbose)
{
    assert(model);
