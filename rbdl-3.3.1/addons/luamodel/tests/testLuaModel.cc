...
TEST_CASE(__FILE__ "_LoadMotionCaptureMarkers", "")
{
    ...
    for (unsigned int i = 0; i < updMarkerSet.size(); ++i)
    {
        ...
        if (std::strcmp(updMarkerSet[i].name.c_str(), "LASI") == 0)
        {
            ...
            CHECK_THAT(updMarkerSet[i].point_local[0], IsClose(0.047794, TEST_PREC, TEST_PREC));
            CHECK_THAT(updMarkerSet[i].point_local[1], IsClose(0.200000, TEST_PREC, TEST_PREC));
            CHECK_THAT(updMarkerSet[i].point_local[2], IsClose(0.070908, TEST_PREC, TEST_PREC));
            flag_found = true;
        }
        ...
        {
            CHECK(updMarkerSet[i].body_id == model.GetBodyId("pelvis"));
            CHECK_THAT(updMarkerSet[i].point_local[0], IsClose(-0.106106, TEST_PREC, TEST_PREC));
            CHECK_THAT(updMarkerSet[i].point_local[1], IsClose(0.200000, TEST_PREC, TEST_PREC));
            CHECK_THAT(updMarkerSet[i].point_local[2], IsClose(0.070908, TEST_PREC, TEST_PREC));
            flag_found = true;
        }
        ...
        if (std::strcmp(updMarkerSet[i].name.c_str(), "RTHI") == 0)
        {
            ...
            CHECK_THAT(updMarkerSet[i].point_local[0], IsClose(-0.007376, TEST_PREC, TEST_PREC));
            CHECK_THAT(updMarkerSet[i].point_local[1], IsClose(0.000000, TEST_PREC, TEST_PREC));
            CHECK_THAT(updMarkerSet[i].point_local[2], IsClose(-0.243721, TEST_PREC, TEST_PREC));
            flag_found = true;
        }
        if (std::strcmp(updMarkerSet[i].name.c_str(), "RKNE") == 0)
        {
            CHECK(updMarkerSet[i].body_id == model.GetBodyId("thigh_right"));
            CHECK_THAT(updMarkerSet[i].point_local[0], IsClose(-0.011611, TEST_PREC, TEST_PREC));
            CHECK_THAT(updMarkerSet[i].point_local[1], IsClose(0.000000, TEST_PREC, TEST_PREC));
            CHECK_THAT(updMarkerSet[i].point_local[2], IsClose(-0.454494, TEST_PREC, TEST_PREC));
            flag_found = true;
        }
        CHECK(flag_found);
    }
...
TEST_CASE(__FILE__ "_LoadLocalFrames", "")
{
    ...
    CHECK_THAT(updLocalFrameSet[0].r[0], IsClose(0.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[0].r[1], IsClose(0.2, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[0].r[2], IsClose(0.0, TEST_PREC, TEST_PREC));

    CHECK_THAT(updLocalFrameSet[0].E(0, 0), IsClose(1.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[0].E(0, 1), IsClose(0.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[0].E(0, 2), IsClose(0.0, TEST_PREC, TEST_PREC));

    CHECK_THAT(updLocalFrameSet[0].E(1, 0), IsClose(0.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[0].E(1, 1), IsClose(1.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[0].E(1, 2), IsClose(0.0, TEST_PREC, TEST_PREC));

    CHECK_THAT(updLocalFrameSet[0].E(2, 0), IsClose(0.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[0].E(2, 1), IsClose(0.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[0].E(2, 2), IsClose(1.0, TEST_PREC, TEST_PREC));

    CHECK(std::strcmp("Pocket_R", updLocalFrameSet[1].name.c_str()) == 0);
    CHECK(updLocalFrameSet[1].body_id == thighRightId);
    CHECK_THAT(updLocalFrameSet[1].r[0], IsClose(0.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[1].r[1], IsClose(-0.2, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[1].r[2], IsClose(0.0, TEST_PREC, TEST_PREC));

    CHECK_THAT(updLocalFrameSet[1].E(0, 0), IsClose(1.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[1].E(0, 1), IsClose(0.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[1].E(0, 2), IsClose(0.0, TEST_PREC, TEST_PREC));

    CHECK_THAT(updLocalFrameSet[1].E(1, 0), IsClose(0.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[1].E(1, 1), IsClose(1.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[1].E(1, 2), IsClose(0.0, TEST_PREC, TEST_PREC));

    CHECK_THAT(updLocalFrameSet[1].E(2, 0), IsClose(0.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[1].E(2, 1), IsClose(0.0, TEST_PREC, TEST_PREC));
    CHECK_THAT(updLocalFrameSet[1].E(2, 2), IsClose(1.0, TEST_PREC, TEST_PREC));
}
TEST_CASE(__FILE__ "_LoadPoints", "")
{
    ...
    CHECK_THAT(updPointSet[0].point_local[0], IsClose(-0.080, TEST_PREC, TEST_PREC));
    CHECK_THAT(updPointSet[0].point_local[1], IsClose(-0.042, TEST_PREC, TEST_PREC));
    CHECK_THAT(updPointSet[0].point_local[2], IsClose(-0.091, TEST_PREC, TEST_PREC));

    CHECK_THAT(updPointSet[1].point_local[0], IsClose(-0.080, TEST_PREC, TEST_PREC));
    CHECK_THAT(updPointSet[1].point_local[1], IsClose(0.042, TEST_PREC, TEST_PREC));
    CHECK_THAT(updPointSet[1].point_local[2], IsClose(-0.091, TEST_PREC, TEST_PREC));

    CHECK_THAT(updPointSet[2].point_local[0], IsClose(0.181788, TEST_PREC, TEST_PREC));
    CHECK_THAT(updPointSet[2].point_local[1], IsClose(-0.054000, TEST_PREC, TEST_PREC));
    CHECK_THAT(updPointSet[2].point_local[2], IsClose(-0.091000, TEST_PREC, TEST_PREC));

    CHECK_THAT(updPointSet[3].point_local[0], IsClose(0.181788, TEST_PREC, TEST_PREC));
    CHECK_THAT(updPointSet[3].point_local[1], IsClose(0.054000, TEST_PREC, TEST_PREC));
    CHECK_THAT(updPointSet[3].point_local[2], IsClose(-0.091000, TEST_PREC, TEST_PREC));
}
...
TEST_CASE(__FILE__ "_LoadConstrainedLuaModel", "")
{
    ...
    bool modelLoaded =
        LuaModelReadFromFileWithConstraints(modelFile.c_str(), &model, constraintSets, constraintSetNames, false);
    ...
    std::vector<unsigned int> bodyIds = constraintSets[0].contactConstraints[0]->getBodyIds();
    CHECK(bodyIds.size() == 2);
    CHECK(bodyIds[0] == baseId);
    CHECK(bodyIds[1] == rootId);

    std::vector<Vector3d> normalVectors = constraintSets[0].contactConstraints[0]->getConstraintNormalVectors();

    // (all contact constraints between the same pair of bodies are grouped)
    CHECK(normalVectors.size() == 1);
    CHECK_THAT(normalVectors[0][0], IsClose(1., TEST_PREC, TEST_PREC));
    CHECK_THAT(normalVectors[0][1], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(normalVectors[0][2], IsClose(0., TEST_PREC, TEST_PREC));
    ...
    normalVectors = constraintSets[0].contactConstraints[1]->getConstraintNormalVectors();
    CHECK(normalVectors.size() == 2);

    CHECK_THAT(normalVectors[0][0], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(normalVectors[0][1], IsClose(1., TEST_PREC, TEST_PREC));
    CHECK_THAT(normalVectors[0][2], IsClose(0., TEST_PREC, TEST_PREC));

    CHECK_THAT(normalVectors[1][0], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(normalVectors[1][1], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(normalVectors[1][2], IsClose(1., TEST_PREC, TEST_PREC));
    ...
    std::vector<SpatialVector> axis = constraintSets[0].loopConstraints[0]->getConstraintAxes();
    CHECK(axis.size() == 1);
    CHECK_THAT(axis[0][0], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[0][1], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[0][2], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[0][3], IsClose(1., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[0][4], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[0][5], IsClose(0., TEST_PREC, TEST_PREC));
    ...
    CHECK_THAT(axis[0][1], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[0][2], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[0][3], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[0][4], IsClose(1., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[0][5], IsClose(0., TEST_PREC, TEST_PREC));
    ...
    normalVectors = constraintSets[1].contactConstraints[0]->getConstraintNormalVectors();
    ...
    CHECK_THAT(normalVectors[1][0], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(normalVectors[1][1], IsClose(1., TEST_PREC, TEST_PREC));
    CHECK_THAT(normalVectors[1][2], IsClose(0., TEST_PREC, TEST_PREC));

    CHECK_THAT(normalVectors[2][0], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(normalVectors[2][1], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(normalVectors[2][2], IsClose(1., TEST_PREC, TEST_PREC));
    ...
    axis = constraintSets[1].loopConstraints[0]->getConstraintAxes();
    CHECK(axis.size() == 2);
    CHECK_THAT(axis[0][0], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[0][1], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[0][2], IsClose(0., TEST_PREC, TEST_PREC));
    ...
    CHECK_THAT(axis[1][0], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[1][1], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[1][2], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[1][3], IsClose(0., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[1][4], IsClose(1., TEST_PREC, TEST_PREC));
    CHECK_THAT(axis[1][5], IsClose(0., TEST_PREC, TEST_PREC));

    CHECK(constraintSets[1].isBaumgarteStabilizationEnabled(groupIndex) == false);

    std::vector<unsigned int> phasing;
    bool constraintSetPhasingLoaded = LuaModelGetConstraintSetPhases(modelFile.c_str(), constraintSetNames, phasing);
...
TEST_CASE(__FILE__ "_LoadMuscleTorqueGenerators", "")
{
    ...
    CHECK(std::strcmp(mtgInfoSet[i].name.c_str(), "AnkleFlexion_R_FpeHalfScale") == 0);
    unsigned int iRef = 5;
    CHECK_THAT(mtgSet[i].getPassiveTorqueScale(), IsClose(0.5, TEST_PREC, TEST_PREC));
    ...
    CHECK_THAT(tmiRef.fiberPassiveTorqueAngleMultiplier,
               IsClose(tmi.fiberPassiveTorqueAngleMultiplier * 2.0, TEST_PREC, TEST_PREC));
    i = 7;
    // Check that the max_isometric_torque_scale is working
    CHECK(std::strcmp(mtgInfoSet[i].name.c_str(), "AnkleFlexion_R_FisoHalfScale") == 0);
    CHECK_THAT(mtgSet[iRef].getMaximumActiveIsometricTorque(),
               IsClose(mtgSet[i].getMaximumActiveIsometricTorque() * 2.0, TEST_PREC, TEST_PREC));
    i = 8;
    // Check that max_angular_velocity_scale is working
    CHECK(std::strcmp(mtgInfoSet[i].name.c_str(), "AnkleFlexion_R_OmegaHalfScale") == 0);
    CHECK_THAT(mtgSet[iRef].getMaximumConcentricJointAngularVelocity(),
               IsClose(mtgSet[i].getMaximumConcentricJointAngularVelocity() * 2.0, TEST_PREC, TEST_PREC));

    i = 9;
    // UnitExtensor
    CHECK(std::strcmp(mtgInfoSet[i].name.c_str(), "UnitExtensor_R") == 0);
    mtgSet[i].calcTorqueMuscleInfo(0, 0, 1, tmi);
    double angleSign = mtgInfoSet[i].angle_sign;
    CHECK_THAT(tmi.fiberActiveTorqueAngleMultiplier, IsClose(1.0, TEST_PREC, TEST_PREC));
    ...
    CHECK_THAT(tmi.fiberActiveTorqueAngleMultiplier, IsClose(faRef, TEST_PREC, TEST_PREC));
    // The UnitExtensors passive curve reaches a unit torque at 1 radian of flexion
    CHECK_THAT(tmi.fiberPassiveTorqueAngleMultiplier, IsClose(1.0, TEST_PREC, TEST_PREC));

    mtgSet[i].calcTorqueMuscleInfo(-1 * angleSign, 0, 1, tmi);
    CHECK_THAT(tmi.fiberActiveTorqueAngleMultiplier, IsClose(faRef, TEST_PREC, TEST_PREC));
    // The UnitExtensor has a maximum isometric torque of 1 Nm
    CHECK_THAT(mtgSet[i].getMaximumActiveIsometricTorque(), IsClose(1., TEST_PREC, TEST_PREC));
    // The UnitExtensor has a maximum angular velocity of 1 rad/sec
    CHECK_THAT(mtgSet[i].getMaximumConcentricJointAngularVelocity(), IsClose(1., TEST_PREC, TEST_PREC));

    i = 10;
    // UnitFlexor
    CHECK(std::strcmp(mtgInfoSet[i].name.c_str(), "UnitFlexor_R") == 0);
    ...
    CHECK_THAT(mtgSet[i].getMaximumActiveIsometricTorque(), IsClose(1., TEST_PREC, TEST_PREC));
    // The UnitExtensor has a maximum angular velocity of 1 rad/sec
    CHECK_THAT(mtgSet[i].getMaximumConcentricJointAngularVelocity(), IsClose(1. * angleSign, TEST_PREC, TEST_PREC));

    i = 11;
    CHECK(std::strcmp(mtgInfoSet[i].name.c_str(), "KneeExtension_R_Anderson2007") == 0);
    CHECK(std::strcmp(mtgInfoSet[i].data_set.c_str(), "Anderson2007") == 0);
    CHECK(std::strcmp(mtgInfoSet[i].age_group.c_str(), "SeniorOver65") == 0);
    CHECK(std::strcmp(mtgInfoSet[i].gender.c_str(), "female") == 0);
    ...
    CHECK_THAT(mtgSet[i].getSubjectMass(), IsClose(81.68, TEST_PREC, TEST_PREC));
    CHECK_THAT(mtgSet[i].getSubjectHeight(), IsClose(1.73, TEST_PREC, TEST_PREC));

    CHECK_THAT(mtgSet[i].getActiveTorqueAngleCurveAngleScaling(), IsClose(2.0, TEST_PREC, TEST_PREC));
    ...
    CHECK(std::fabs(tmi.fiberActiveTorqueAngleMultiplier - tmiRef.fiberActiveTorqueAngleMultiplier) > TEST_PREC);
}
...
TEST_CASE(__FILE__ "_ModelHeaderGeneration", "")
{
    ...
    bool constrainedModelLoaded =
        LuaModelReadFromFileWithConstraints(modelFile.c_str(), &model, conSet, constraintSetNames, false);
    ...
    bool markersLoaded = LuaModelReadMotionCaptureMarkers(modelFile.c_str(), &model, markerSet, false);
    CHECK(markersLoaded);
...
//--------------------------------------------
    ...
    bool modelHeaderGenerated = LuaModelWriteModelHeaderEntries(headerFile.c_str(), model, false);
    CHECK(modelHeaderGenerated);

    bool pointsHeaderGenerated = LuaModelWritePointsHeaderEntries(headerFile.c_str(), pointSet, true);
    CHECK(pointsHeaderGenerated);

    bool markerHeaderGenerated = LuaModelWriteMotionCaptureMarkerHeaderEntries(headerFile.c_str(), markerSet, true);
    CHECK(markerHeaderGenerated);

    bool localFrameHeaderGenerated = LuaModelWriteLocalFrameHeaderEntries(headerFile.c_str(), localFrames, true);
    CHECK(localFrameHeaderGenerated);

    bool constraintSetHeaderGenerated =
        LuaModelWriteConstraintSetHeaderEntries(headerFile.c_str(), constraintSetNames, conSet, true);

    bool phasingHeaderGenerated =
        LuaModelWriteConstraintSetPhaseHeaderEntries(headerFile.c_str(), constraintSetNames, phasing, true);
    CHECK(phasingHeaderGenerated);

//--------------------------------------------
#ifdef RBDL_BUILD_ADDON_MUSCLE

    bool mtgHeaderGenerated =
        LuaModelWriteMillard2016TorqueMuscleHeaderEntries(headerFile.c_str(), mtgSet, mtgSetInfo, true);
