                            ...
                            const std::string              &entryPrefix)
{
    ...
    for (unsigned int i = 0; i < enumEntries.size(); ++i)
    {
        ...
        for (unsigned int j = unsigned(int(entryPrefix.size() + enumEntries[i].size())); j < numChars; ++j)
        {
            updFileStream << " ";
        }
        if (i == 0)
                                           ...
                                           const std::vector<unsigned int> &indexEntries)
{
    ...
    for (unsigned int i = 0; i < enumEntries.size(); ++i)
    {
        ...
        if (indexEntries[i] > 9)
        {
            indexPaddingTemp = 1 + unsigned(int(std::floor(log10(double(indexEntries[i])))));
    ...
    for (unsigned int i = 0; i < enumEntries.size(); ++i)
    {
        ...
        if (indexEntries.size() > 0)
        {
            for (unsigned int j = 0; j < indexEntries[0].size(); ++j)
            {
                ...
                {
                    indexPaddingTemp = 1 + unsigned(int(std::floor(log10(double(indexEntries[i][j])))));
...
std::vector<std::string> LuaModelGetConstraintSetNames(const char *filename)
{
    ...
    for (size_t ci = 0; ci < constraint_keys.size(); ++ci)
    {
        ...
        {
            throw Errors::RBDLFileParseError("Invalid constraint found in model.constraint_sets: "
                                    ...
                                    std::vector<unsigned int>      &constraint_set_phases)
{
    ...
    for (unsigned int i = 1; i < phases; ++i)
    {
        ...
        if (found == false)
        {
            throw Errors::RBDLFileParseError("constraint_phases lists the constraint_set name " + phaseName +
                                         ...
                                         bool                            verbose)
{
    ...
    {
        throw Errors::RBDLFileParseError("Number of constraint sets different from"
                                      ...
                                      bool                            verbose)
{
    ...
    for (size_t i = 0; i < constraint_set_names.size(); ++i)
    {
        ...
        if (!model_table["constraint_sets"][conName.c_str()].exists())
        {
        ...
        size_t num_constraints = model_table["constraint_sets"][conName.c_str()].length();
        ...
        for (size_t ci = 0; ci < num_constraints; ++ci)
        {
            ...
            if (!model_table["constraint_sets"][conName.c_str()][ci + 1]["constraint_type"].exists())
            {
                throw Errors::RBDLFileParseError("constraint_type not specified.\n");
            }

            string constraintType =
                model_table["constraint_sets"][conName.c_str()][ci + 1]["constraint_type"].getDefault<string>("");
            std::string constraint_name =
                model_table["constraint_sets"][conName.c_str()][ci + 1]["name"].getDefault<string>("");

            bool enable_stabilization =
                model_table["constraint_sets"][conName.c_str()][ci + 1]["enable_stabilization"].getDefault<bool>(false);
            double stabilization_parameter = 0.1;

            if (enable_stabilization)
            {
                stabilization_parameter =
                    model_table["constraint_sets"][conName.c_str()][ci + 1]["stabilization_parameter"]
                        .getDefault<double>(0.1);
                if (stabilization_parameter <= 0.0)
                {
                    std::stringstream errormsg;
                    errormsg << "Invalid stabilization parameter: " << stabilization_parameter << " must be > 0.0"
                             << std::endl;
                    throw Errors::RBDLFileParseError(errormsg.str());
                }
            }
            ...
            if (constraintType == "contact")
            {
                ...
                if (model_table["constraint_sets"][conName.c_str()][ci + 1]["id"].exists())
                {
                    constraint_user_id = unsigned(
                        int(model_table["constraint_sets"][conName.c_str()][ci + 1]["id"].getDefault<double>(0.)));
                }
                ...
                if (model_table["constraint_sets"][conName.c_str()][ci + 1]["point_name"].exists())
                {
                    ...
                    if (pointFound == false)
                    {
                        ...
                        errormsg << "Could not find a point with the name: " << pointName
                                 << " which is used in constraint set " << constraint_name << endl;
                        throw Errors::RBDLFileParseError(errormsg.str());
                    }
                }
                else
                {
                    if (!model_table["constraint_sets"][conName.c_str()][ci + 1]["body"].exists())
                    {
                        throw Errors::RBDLFileParseError("body not specified.\n");
                    }
                    bodyId = model->GetBodyId(
                        model_table["constraint_sets"][conName.c_str()][ci + 1]["body"].getDefault<string>("").c_str());

                    bodyPoint = model_table["constraint_sets"][conName.c_str()][ci + 1]["point"].getDefault<Vector3d>(
                        Vector3d::Zero());
                }
                ...
                if (model_table["constraint_sets"][conName.c_str()][ci + 1]["normal_sets"].exists())
                {

                    normalSetsMatrix =
                        model_table["constraint_sets"][conName.c_str()][ci + 1]["normal_sets"].getDefault<MatrixNd>(
                            MatrixNd::Zero(1, 1));

                    if (normalSetsMatrix.cols() != 3)
                    {
                        std::ostringstream errormsg;
                        errormsg << "The normal_sets field must be m x 3, the one read for " << conName.c_str()
                                 << " has an normal_sets of size " << normalSetsMatrix.rows() << " x "
                                 << normalSetsMatrix.cols()
                                 << ". In addition the normal_sets field should resemble:" << endl;
                ...
                else if (model_table["constraint_sets"][conName.c_str()][ci + 1]["normal"].exists())
                {

                    normal = model_table["constraint_sets"][conName.c_str()][ci + 1]["normal"].getDefault<Vector3d>(
                        Vector3d::Zero());
                    normalSets.push_back(normal);
                }
                else
                ...
                std::string contactName =
                    model_table["constraint_sets"][conName.c_str()][ci + 1]["name"].getDefault<string>("").c_str();

                for (unsigned int c = 0; c < normalSets.size(); ++c)
                {
                    constraint_sets[i].AddContactConstraint(bodyId, bodyPoint, normalSets[c], contactName.c_str(),
                                                            constraint_user_id);
                }
                ...
                if (verbose)
                {
                    ...
                    cout << "  body = " << model->GetBodyName(bodyId) << endl;
                    cout << "  body point = " << bodyPoint.transpose() << endl;
            ...
            else if (constraintType == "loop")
            {
                ...
                if (model_table["constraint_sets"][conName.c_str()][ci + 1]["predecessor_local_frame"].exists())
                {
                    ...
                    {
                        ostringstream errormsg;
                        errormsg << "Could not find a local frame with the name: " << localFrameName
                                 << " which is used in constraint set " << constraint_name << endl;
                        throw Errors::RBDLFileParseError(errormsg.str());
                    }
                }
                ...
                {
                    if (!model_table["constraint_sets"][conName.c_str()][ci + 1]["predecessor_body"].exists())
                    {
                        throw Errors::RBDLFileParseError("predecessor_body not specified.\n");
                    }

                    idPredecessor =
                        model->GetBodyId(model_table["constraint_sets"][conName.c_str()][ci + 1]["predecessor_body"]
                                             .getDefault<string>("")
                                             .c_str());
                    Xp = model_table["constraint_sets"][conName.c_str()][ci + 1]["predecessor_transform"]
                             .getDefault<SpatialTransform>(SpatialTransform());
                }

                if (model_table["constraint_sets"][conName.c_str()][ci + 1]["successor_local_frame"].exists())
                {
                ...
                else
                {
                    ...
                    if (!model_table["constraint_sets"][conName.c_str()][ci + 1]["successor_body"].exists())
                    {
                        throw Errors::RBDLFileParseError("successor_body not specified.\n");
                    }

                    idSuccessor =
                        model->GetBodyId(model_table["constraint_sets"][conName.c_str()][ci + 1]["successor_body"]
                                             .getDefault<string>("")
                                             .c_str());

                    Xs = model_table["constraint_sets"][conName.c_str()][ci + 1]["successor_transform"]
                             .getDefault<SpatialTransform>(SpatialTransform());
                }
                ...
                axisSets.resize(0);
                if (model_table["constraint_sets"][conName.c_str()][ci + 1]["axis_sets"].exists())
                {
                    axisSetsMatrix =
                        model_table["constraint_sets"][conName.c_str()][ci + 1]["axis_sets"].getDefault<MatrixNd>(
                            MatrixNd::Zero(1, 1));

                    if (axisSetsMatrix.cols() != 6)
                    {
                        std::stringstream errormsg;
                        errormsg << "The axis_sets field must be m x 6, the one read for " << conName.c_str()
                                 << " has an axis_sets of size " << axisSetsMatrix.rows() << " x "
                                 << axisSetsMatrix.cols()
                                 << ". In addition the axis_sets field should resemble:" << endl;
                ...
                else if (model_table["constraint_sets"][conName.c_str()][ci + 1]["axis"].exists())
                {
                    axis = model_table["constraint_sets"][conName.c_str()][ci + 1]["axis"].getDefault<SpatialVector>(
                        SpatialVector::Zero());
                ...
                for (unsigned int r = 0; r < axisSets.size(); ++r)
                {
                    constraint_id = constraint_sets[i].AddLoopConstraint(
                        idPredecessor, idSuccessor, Xp, Xs, axisSets[r], enable_stabilization, stabilization_parameter,
                        constraint_name.c_str(), constraint_user_id);
                }
                ...
                if (verbose)
                {
                    cout << "  type = loop" << endl;
                    cout << "  name = " << constraint_name << std::endl;
                    cout << "  predecessor body = " << model->GetBodyName(idPredecessor) << endl;
                    cout << "  successor body = " << model->GetBodyName(idSuccessor) << endl;
                    cout << "  predecessor body transform = " << endl << Xp << endl;
                    cout << "  successor body transform = " << endl << Xs << endl;
                    ...
                    if (enable_stabilization)
                    {
                        cout << "  stabilization_parameter = " << stabilization_parameter << endl;
                    }
                    cout << "  constraint name = " << constraint_name.c_str() << endl;
                }
            }
            else
            {
                             ...
                             bool                            verbose)
{
    ...
    unsigned int localFrameCount = unsigned(int(model_table["local_frames"].length()));
                        ...
                        bool                            verbose)
{
    ...
    if (pointCount > 0)
    {
        ...
        for (unsigned int i = 1; i <= pointCount; ++i)
        {
            ...
            if (verbose)
            {
                cout << "Point '" << upd_point_set[i - 1].name << "' (PointName = " << upd_point_set[i - 1].name << ")"
                     << endl;
                cout << "  body        = " << upd_point_set[i - 1].body_name
                     << " (id = " << upd_point_set[i - 1].body_id << ")" << endl;
                cout << "  point_local  = '" << upd_point_set[i - 1].point_local.transpose() << endl;
            }
        }
    }
    return true;
...
bool LuaModelReadHumanMetaData(const char *filename, HumanMetaData &human_meta_data, bool verbose)
{
    ...
    if (subjectCount != 1)
    {
        ...
        errormsg << "human_meta_data should contain data "
                 << "for only 1 subject but has data for " << subjectCount << endl;
...
//==============================================================================
#ifdef RBDL_BUILD_ADDON_MUSCLE
std::vector<unsigned int> getQIndex(const RigidBodyDynamics::Model *model, const char *childBodyName)
{
...
unsigned int getMillard2016TorqueMuscleTypeId(std::string name)
{
    ...
    {
        if (name.find(std::string(JointTorqueSet.names[i])) != std::string::npos)
        {
            break;
        }
        ++i;
                                             ...
                                             bool                                        verbose)
{
    ...
    for (unsigned int i = 1; i <= mtgCount; ++i)
    {
        ...
        if (verbose)
        {
            if (i == 1)
            {
                ...
                std::cout << std::setw(20) << "Name" << std::setw(10) << "Angle-Sign" << std::setw(11) << "Torque-Sign"
                          << std::endl;
            }
            std::cout << std::setw(24) << mtgInfo.name << std::setw(3) << mtgInfo.angle_sign << std::setw(3)
                      << mtgInfo.torque_sign << std::endl;
        }
    }
    ...
    {
        cerr << "Unknown gender in subject metadata, " << human_meta_data.gender << endl;
        abort();
    }
    ...
    {
        subjectInfo.ageGroup = AgeGroupSet::Middle55To65;
    }
    ...
    {
        cerr << "Unknown age group in subject metadata, " << human_meta_data.age_group << endl;
        abort();
    }
    ...
    for (unsigned int i = 0; i < mtgCount; ++i)
    {
        ...
        {
            cerr << "Error: the data_set entry for " << updMtgSetInfo[i].name << " is " << updMtgSetInfo[i].data_set
        ...
        updMtgSet[i] = Millard2016TorqueMuscle(mtgDataSet, mtgSubjectInfo,
                                               int(getMillard2016TorqueMuscleTypeId(updMtgSetInfo[i].name)),
                                               updMtgSetInfo[i].joint_angle_offset, updMtgSetInfo[i].angle_sign,
                                               updMtgSetInfo[i].torque_sign, updMtgSetInfo[i].name);
        ...
        {
            updMtgSet[i].setNormalizedDampingCoefficient(updMtgSetInfo[i].passive_element_damping_coeff);
        }

        if (!std::isnan(updMtgSetInfo[i].passive_element_torque_scale))
        {
            updMtgSet[i].setPassiveTorqueScale(updMtgSetInfo[i].passive_element_torque_scale);
        }
        ...
        {
            updMtgSet[i].fitPassiveTorqueScale(updMtgSetInfo[i].fit_passive_torque_scale[0],
            ...
            !std::isnan(updMtgSetInfo[i].fit_passive_torque_offset[1]))
        {
            ...
            updMtgSet[i].fitPassiveCurveAngleOffset(updMtgSetInfo[i].fit_passive_torque_offset[0],
        ...
        {
            updMtgSet[i].setMaximumActiveIsometricTorque(updMtgSetInfo[i].max_isometric_torque);
        }

        if (!std::isnan(updMtgSetInfo[i].max_angular_velocity))
        {
            updMtgSet[i].setMaximumConcentricJointAngularVelocity(updMtgSetInfo[i].max_angular_velocity);
        }
        if (!std::isnan(updMtgSetInfo[i].active_torque_angle_blending))
        {
            updMtgSet[i].setActiveTorqueAngleCurveBlendingVariable(updMtgSetInfo[i].active_torque_angle_blending);
        }
        ...
        {
            updMtgSet[i].setTorqueAngularVelocityCurveBlendingVariable(updMtgSetInfo[i].torque_velocity_blending);
        }
        if (!std::isnan(updMtgSetInfo[i].active_torque_angle_scale))
        {
            updMtgSet[i].setActiveTorqueAngleCurveAngleScaling(updMtgSetInfo[i].active_torque_angle_scale);
        }
    }
    ...
    for (unsigned int idJoint = 1; idJoint < model->mJoints.size(); ++idJoint)
    {
        ...
        else
        {
...
#else
                if (fabs(fabs(model->mJoints[idJoint].mJointAxes[0][j]) - 1.) < std::numeric_limits<double>::epsilon())
                {
                    axisIndex = j;
                }
#endif
            ...
            if (model->multdof3_w_index[idJoint] > 0.)
            {
                ...
                qNames[model->multdof3_w_index[idJoint]] = std::string("Q_").append(ss.str());
            }
        }
    }
}
...
bool LuaModelWriteModelHeaderEntries(const char *filename, const RigidBodyDynamics::Model &model, bool append)
{
    ...
    */
    ...
    for (unsigned int idJoint = 1; idJoint < model.mJoints.size(); ++idJoint)
    {
        ...
        // Get the name of the child body associated with this index
        for (map<string, unsigned int>::const_iterator it = model.mBodyNameMap.begin(); it != model.mBodyNameMap.end();
             ++it)
        {
            if (it->second == idChild)
            {
    ...
    appendEnumNameIndexStructToFileStream(headerFile, "BodyMap", "BodyId", bodyNames, "BodyId_", bodyIndex);
    appendEnumNameIndexStructToFileStream(headerFile, "PositionMap", "PositionId", qNames, "", qIndex);
    appendEnumNameIndexStructToFileStream(headerFile, "VelocityMap", "VelocityId", qDotNames, "", qDotIndex);
    appendEnumNameIndexStructToFileStream(headerFile, "AccelerationMap", "AccelerationId", qDDotNames, "", qDotIndex);
    appendEnumNameIndexStructToFileStream(headerFile, "ForceMap", "ForceId", tauNames, "", qDotIndex);
...
//==============================================================================
RBDL_ADDON_DLLAPI
bool LuaModelWritePointsHeaderEntries(const char *header_file_name, const std::vector<Point> &point_set, bool append)
{
    ...
    if (point_set.size() > 0)
    {
        ...
        appendEnumNameIndexStructToFileStream(headerFile, "PointMap", "PointId", names, "Point_", indices);
    }
    headerFile.flush();
    headerFile.close();
                                                   ...
                                                   bool                                    append)
{
    ...
    if (marker_set.size() > 0)
    {
        ...
        indices.push_back(std::numeric_limits<unsigned int>::max());
        appendEnumNameIndexStructToFileStream(headerFile, "MotionCaptureMarkerMap", "MarkerId", names, "Marker_",
                                              indices);
    }
    headerFile.flush();
    headerFile.close();
                                          ...
                                          bool                           append)
{
    ...
    if (local_frame_set.size() > 0)
    {
        ...
        indices.push_back(std::numeric_limits<unsigned int>::max());
        appendEnumNameIndexStructToFileStream(headerFile, "LocalFrameMap", "LocalFrameId", names, "LocalFrame_",
                                              indices);
    }
    headerFile.flush();
    headerFile.close();
                                                  ...
                                                  bool                             append)
{
    ...
    if (constraint_phases.size() > 0)
    {
        ...
        appendEnumStructToFileStream(headerFile, "ConstraintSetPhaseMap", enumTypeAndFieldNames, enumEntries,
                                     entryPrefix, indexTypeAndFieldNames, indexMatrix);
    }
    headerFile.flush();
    headerFile.close();
    return true;
                                             ...
                                             bool                                                 append)
{
    ...
    {
        ostringstream errormsg;
        errormsg << "constraint_set_names (size: " << constraint_set_names.size() << ") and "
                 << "constraint_sets "
                 << "(size: " << constraint_sets.size() << ")"
                 << " must have the same size but do not." << endl;
        throw Errors::RBDLError(errormsg.str());
    }
    ...
    for (unsigned int i = 0; i < constraint_set_names.size(); ++i)
    {
        ...
        {
            groupNames.push_back(std::string(constraint_sets[i].constraints[j]->getName()));
        }
        ...
        for (unsigned int j = 0; j < constraint_sets[i].constraints.size(); ++j)
        {
            for (unsigned int k = 0; k < constraint_sets[i].constraints[j]->getConstraintSize(); ++k)
            {
                ...
                conEnumVector[2] = std::string(constraint_sets[i].constraints[j]->getName());
    ...
    if (constraint_sets.size() > 0)
    {
        ...
        appendEnumNameIndexStructToFileStream(headerFile, "ConstraintSetMap", "ConstraintSetId", conSetNames, "CS_",
                                              conSetIndex);
        ...
        appendEnumStructToFileStream(headerFile, "ConstraintMap", conEnumTypeAndFieldNames, conEnumMatrix,
                                     conEnumPrefix, conIndexTypeAndFieldNames, conIndexMatrix);
    }
    headerFile.flush();
    headerFile.close();
    ...
    bool                                                                            append)
{
    ...
    {
        ostringstream errormsg;
        errormsg << "mtg_set (size: " << mtg_set.size() << ") and "
                 << "mtg_set_info "
                 << "(size: " << mtg_set_info.size() << ")"
                 << " must have the same size but do not." << endl;
        throw Errors::RBDLError(errormsg.str());
    }
    ...
    {
        names.push_back("Last");
        appendEnumToFileStream(headerFile, "Millard2016TorqueMuscleId", names, 0, "MTG_");
        indices.push_back(std::numeric_limits<unsigned int>::max());
        appendEnumNameIndexStructToFileStream(headerFile, "Millard2016TorqueMuscleMap", "Millard2016TorqueMuscleId",
                                              names, "MTG_", indices);
    }
    headerFile.flush();
    headerFile.close();
