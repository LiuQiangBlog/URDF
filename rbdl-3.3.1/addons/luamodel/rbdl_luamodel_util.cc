...
int main(int argc, char *argv[])
{
    ...
    if (constraint_sets)
    {
        ...
        std::vector<RigidBodyDynamics::ConstraintSet> constraint_sets(constraint_set_names.size(),
                                                                      RigidBodyDynamics::ConstraintSet());
        result = RigidBodyDynamics::Addons::LuaModelReadFromFileWithConstraints(
            filename.c_str(), &model, constraint_sets, constraint_set_names, verbose);
    }
    else
    {
        result = RigidBodyDynamics::Addons::LuaModelReadFromFile(filename.c_str(), &model, verbose);
    }
