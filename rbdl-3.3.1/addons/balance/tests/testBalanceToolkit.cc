...
TEST_CASE(__FILE__ "_NumericalTestFootPlacementEstimator", "")
{
    ...
    Vector3d r0C0 = Vector3d(6.003900000000000e-01, 2.167700000000000e-01, 5.957600000000000e-01);
    Vector3d v0C0 = Vector3d(3.020800000000000e-01, 2.088000000000000e-02, 2.757000000000000e-02);
    Matrix3d JC0 = Matrix3d(5.165410000000000e+00, 5.842000000000000e-02, 4.450500000000000e-01, 5.842000000000000e-02,
                            6.110220000000000e+00, -1.848600000000000e-01, 4.450500000000000e-01,
                            -1.848600000000000e-01, 2.099090000000000e+00);

    Vector3d r0P0 = Vector3d(6.003900000000000e-01, 2.167700000000000e-01, 0.000000000000000e+00);
    Vector3d v0P0 = Vector3d(-3.212179353090099e-02, 5.833672380300754e-01, 1.784919742991831e-02);
    Vector3d HP0 = Vector3d(-1.263996880000000e+00, 2.426477808000000e+01, -8.467000000000000e-02);

    double f = -8.670397733112623e-12;
    double phi = 1.551340426396078e-01;
    double projectionError = 3.484674002384912e-03;

    Vector3d HC0 = Vector3d(-2.005000000000000e-02, 6.268060000000000e+00, -8.467000000000000e-02);
    Vector3d w0C0 = Vector3d(-2.019572919578354e-02, 1.027672539346732e+00, 5.444914458275742e-02);

    Vector3d n = Vector3d(-5.202130399910231e-02, 9.986459752736367e-01, 0.000000000000000e+00);
    Vector3d u = Vector3d(9.986459752736367e-01, 5.202130399910231e-02, 0.000000000000000e+00);
    Vector3d k = Vector3d(-0.000000000000000e+00, -0.000000000000000e+00, 1.000000000000000e+00);

    Vector3d r0F0 = Vector3d(6.934351408607322e-01, 2.216168923704711e-01, 0.000000000000000e+00);
    ...
    Joint jointFloatingBase = Joint(SpatialVector(0., 0., 0., 1., 0., 0.), SpatialVector(0., 0., 0., 0., 1., 0.),
                                    SpatialVector(0., 0., 0., 0., 0., 1.), SpatialVector(1., 0., 0., 0., 0., 0.),
                                    SpatialVector(0., 1., 0., 0., 0., 0.), SpatialVector(0., 0., 1., 0., 0., 0.));

    unsigned int idB1 = model.AddBody(0, Xtrans(Vector3d(0., 0, 0.)), jointFloatingBase, body1, "body1");
    ...
    BalanceToolkit::CalculateFootPlacementEstimator(model, q, qd, pointOnGroundPlane, groundPlaneNormal, fpeInfo,
                                                    omegaSmall, evaluate_derivatives, update_kinematics);
...
TEST_CASE(__FILE__ "_InertiaCalculations", "")
{
    ...
    Matrix3d I0 = Matrix3d(Ixx0, 0, 0, 0, Iyy0, 0, 0, 0, Izz0);

    Matrix3d I1 = Matrix3d(Ixx1, 0, 0, 0, Iyy1, 0, 0, 0, Izz1);
    ...
    unsigned int idB1 =
        model.AddBody(0, Xtrans(Vector3d(0., 0, 0.)), jointFloatingBase, cylinderUpper, "cylinderUpper");
    unsigned int idB2 =
        model.AddBody(0, Xtrans(Vector3d(0., 0, 0.)), jointFloatingBase, cylinderLower, "cylinderLower");
    ...
    Matrix3d I0Test = model.X_base[idB1].E * fpeInfo.JC0 * model.X_base[idB1].E.transpose();
