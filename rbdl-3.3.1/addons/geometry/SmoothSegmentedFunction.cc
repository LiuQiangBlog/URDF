...
SmoothSegmentedFunction::SmoothSegmentedFunction()
    : _x0(NAN), _x1(NAN), _y0(NAN), _y1(NAN), _dydx0(NAN), _dydx1(NAN), _name("NOT_YET_SET")
{
                                                         ...
                                                         const std::string                       &name)
{
    ...
    if (mX.rows() != 6 || mY.rows() != 6 || mX.cols() != mY.cols())
    {
        ...
        //! [Size Mismatch]
        errormsg << "SmoothSegmentedFunction::updSmoothSegmentedFunction " << _name.c_str()
...
void SmoothSegmentedFunction::scale(double xScale, double yScale)
{
    ...
    if (abs(xScale) <= SQRTEPS)
    {
        ostringstream errormsg;
        //! [Invalid Parameter]
        errormsg << "SmoothSegmentedFunction::scale " << _name.c_str()
...
double SmoothSegmentedFunction::calcInverseValue(double y, double xGuess) const
{
    ...
    if (idx == -1)
    {
        if ((y - _y1) * _dydx1 >= 0 && fabs(_dydx1) > numeric_limits<double>::epsilon())
        {
            xVal = (y - _y1) / _dydx1 + _x1;
        }
        else if ((_y0 - y) * _dydx0 >= 0 && fabs(_dydx0) > numeric_limits<double>::epsilon())
        {
...
double SmoothSegmentedFunction::calcValue(const RigidBodyDynamics::Math::VectorNd &ax) const
{
    ...
    if (!(ax.size() == 1))
    {
        ...
        errormsg << "SmoothSegmentedFunction::calcValue " << _name.c_str()
                 ...
                 << "designed only for 1D functions, but a function with " << ax.size() << " elements was entered"
                 << endl;
    ...
    */
    ...
    for (int i = 0; i < (signed)derivComponents.size(); i++)
    {
        if (!(derivComponents[i] == 0))
        {
            ...
            errormsg << "SmoothSegmentedFunction::calcDerivative " << _name.c_str()
                     ...
                     << " but derivComponents had a value of " << derivComponents[i] << " in it" << endl;
            throw RigidBodyDynamics::Errors::RBDLInvalidParameterError(errormsg.str());
        }
    }
    //! [Dof Mismatch]
    if (!(derivComponents.size() <= 6))
    {
        ...
        errormsg << "SmoothSegmentedFunction::calcDerivative " << _name.c_str()
                 ...
                 << " but derivComponents had a size of " << derivComponents.size() << endl;
        throw RigidBodyDynamics::Errors::RBDLDofMismatchError(errormsg.str());
    }
    //! [Dof Mismatch]
    ...
    if (!(ax.size() == 1))
    {
        ...
        errormsg << "SmoothSegmentedFunction::calcValue " << _name.c_str()
                 << ": Argument x must have only 1 element, as this function is "
                 << "designed only for 1D functions, but ax had a size of " << ax.size() << endl;
        throw RigidBodyDynamics::Errors::RBDLDofMismatchError(errormsg.str());
    }
...
RigidBodyDynamics::Math::VectorNd SmoothSegmentedFunction::getCurveDomain() const
{
...
SmoothSegmentedFunction::calcSampledCurve(int maxOrder, double domainMin, double domainMax) const
{
    ...
    if (!(maxOrder <= getMaxDerivativeOrder()))
    {
        ...
        errormsg << "SmoothSegmentedFunction::calcSampledCurve "
                 << "Derivative order past the maximum computed order requested" << endl;
...
void SmoothSegmentedFunction::getXControlPoints(RigidBodyDynamics::Math::MatrixNd &mat) const
{
...
void SmoothSegmentedFunction::getYControlPoints(RigidBodyDynamics::Math::MatrixNd &mat) const
{
                                                ...
                                                const std::string                 &filename) const
{
    ...
    if (!datafile)
    {
        datafile.close();
        ostringstream errormsg;
        errormsg << "SmoothSegmentedFunction::printMatrixToFile " << _name.c_str()
