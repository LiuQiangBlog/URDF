...
public:
     ...
     */
    explicit Constant(T value, int argumentSize = 1) : argumentSize(argumentSize), value(value) {}
...
public:
     ...
     */
    explicit Linear(const RigidBodyDynamics::Math::VectorNd &coefficients) : coefficients(coefficients) {}
    T calcValue(const RigidBodyDynamics::Math::VectorNd &x) const
    {
...
public:
     ...
     *    powers
     */
    Polynomial(const RigidBodyDynamics::Math::VectorNd &coefficients) : coefficients(coefficients) {}
    T calcValue(const RigidBodyDynamics::Math::VectorNd &x) const
    {
        assert(x.size() == 1);
        double arg   = x[0];
...
public:
     ...
     */
    Sinusoid(double amplitude, double frequency, double phase = 0) : a(amplitude), w(frequency), p(phase) {}
...
public:
    ...
    Step(const T &y0, const T &y1, double x0, double x1)
        : m_y0(y0), m_y1(y1), m_yr(y1 - y0), m_zero(double(0) * y0), m_x0(x0), m_x1(x1), m_ooxr(1 / (x1 - x0)),
          m_sign(copysign(1, m_ooxr))
    {
        /*
