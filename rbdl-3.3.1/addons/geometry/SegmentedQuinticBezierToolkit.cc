...
*/
void SegmentedQuinticBezierToolkit::printMatrixToFile(const VectorNd &col0, const MatrixNd &data, std::string &filename)
{
...
/*Detailed Computational Costs
  ...
  1     13    9    23
*/
MatrixNd SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(double x0,
                                                                             double y0,
                                                                             double dydx0,
                                                                             double x1,
                                                                             double y1,
...
21      20    13
*/
double SegmentedQuinticBezierToolkit::calcQuinticBezierCurveVal(double u1, const VectorNd &pts)
{
    double val = -1;
    ...
    */
    ...
    val = pts[0] * v5 * 1.0 + pts[1] * u1 * v4 * 5.0 + pts[2] * u2 * v3 * 10.0 + pts[3] * u3 * v2 * 10.0 +
          pts[4] * u4 * v1 * 5.0 + pts[5] * u5 * 1.0;
                                                                      ...
                                                                      int order)
{
    ...
    */
    ...
    case 5:
    {
        ...
        double t47 = (dydu * t4);
        double t49 = d5ydu5 * t1 - 4 * d4ydu4 * t4 * d2xdu2 + 12 * d3ydu3 * t9 * t11 - 6 * t14 * d3xdu3 -
                     24 * d2ydu2 * t18 * t20 + 24 * t23 * t24 - 4 * t27 * d4xdu4 + 24 * dydu / t17 / dxdu * t33 -
    ...
    case 6:
    {
        ...
        double t76 = 20 * d4ydu4 * t3 * t5 - 60 * d3ydu3 * t9 * t11 + 60 * t14 * t15 + 120 * d2ydu2 * t19 * t21 -
                     180 * t24 * t25 + 30 * t28 * t29 + 40 * t28 * t32 - 120 * dydu / t8 / t1 * t21 * d2xdu2 +
                     240 * t41 * t11 * d3xdu3 - 60 * t45 * t5 * d4xdu4 + 20 * t49 * d3xdu3 * d4xdu4 +
                     10 * t49 * d2xdu2 * d5xdu5 + d6ydu6 * t56 - 90 * t45 * d2xdu2 * t29 - t62 * d6xdu6 -
                     5 * d5ydu5 * t61 * d2xdu2 - 10 * t67 * d3xdu3 - 5 * t70 * d5xdu5 - 10 * t73 * d4xdu4;

        double t100 = d5ydu5 * t56 - 4 * t67 * d2xdu2 + 12 * t14 * t5 - 6 * t73 * d3xdu3 - 24 * t24 * t11 +
                      24 * t28 * t15 - 4 * t70 * d4xdu4 + 24 * t41 * t21 - 36 * t45 * t25 + 6 * t49 * t29 +
                      8 * t49 * t32 - t62 * d5xdu5;

        double t116 = d4ydu4 * t56 - 3 * t73 * d2xdu2 + 6 * t28 * t5 - 3 * t70 * d3xdu3 - 6 * t45 * t11 +
                      6 * t49 * t15 - t62 * d4xdu4;
        ...
        double t155 = t76 * t56 - 4 * t100 * t61 * d2xdu2 + 12 * t116 * t3 * t5 - 6 * t120 * d3xdu3 -
                      24 * t129 * t9 * t11 + 24 * t133 * t15 - 4 * t136 * d4xdu4 + 24 * t141 * t19 * t21 -
                      36 * t145 * t25 + 6 * t148 * t29 + 8 * t148 * t32 - t153 * d5xdu5;

        double t169 = t100 * t56 - 3 * t120 * d2xdu2 + 6 * t133 * t5 - 3 * t136 * d3xdu3 - 6 * t145 * t11 +
                      6 * t148 * t15 - t153 * d4xdu4;
...
*/
double SegmentedQuinticBezierToolkit::calcQuinticBezierCurveDerivU(double u, const VectorNd &pts, int order)
{
    double val = -1;
    /*
    SimTK_ERRCHK_ALWAYS( (u>=0 && u <= 1) ,
    ...
    */
    ...
    case 5:
    {
        val = p0 * (-0.120e3) + p1 * 0.600e3 + p2 * (-0.1200e4) + p3 * 0.1200e4 + p4 * (-0.600e3) + p5 * 0.120e3;
    }
    break;
    default:
        val = 0;
...
*/
double SegmentedQuinticBezierToolkit::calcU(double ax, const VectorNd &bezierPtsX, double tol, int maxIter)
{
    ...
    */
    if (!(ax >= minX && ax <= maxX))
    {
        ...
        errormsg << "SegmentedQuinticBezierToolkit::calcU:"
                 ...
                 << "Bezier curve specified by the control points in bezierPtsX." << endl;
        throw RigidBodyDynamics::Errors::RBDLInvalidParameterError(errormsg.str());
    }
    ...
    if (abs(f) > tol)
    {
        ...
        while (iterBisection < MAXITER_INITIALSOLN && min(abs(fL), abs(fR)) > UTOL_INITIALSOLN)
        {
            u = 0.5 * (uL + uR);
            f = calcQuinticBezierCurveVal(u, bezierPtsX) - ax;
