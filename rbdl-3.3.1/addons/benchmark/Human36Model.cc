...
double SegmentLengths[SegmentNameLast] = {0.1457, 0.4222, 0.4403, 0.1037, 0.2155,
                                          0.2421, 0.2817, 0.2689, 0.0862, 0.2429};

double SegmentMass[SegmentNameLast] = {0.8154, 10.3368, 3.1609, 1.001, 16.33, 15.96, 1.9783, 1.1826, 0.4453, 5.0662};

double SegmentCOM[SegmentNameLast][3] = {{0., 0., 0.0891},  {0., 0., -0.1729}, {0., 0., -0.1963}, {0.1254, 0., -0.0516},
                                         {0., 0., 0.1185},  {0., 0., 0.1195},  {0., 0., -0.1626}, {0., 0., -0.1230},
                                         {0., 0., -0.0680}, {0., 0., 1.1214}};

double SegmentRadiiOfGyration[SegmentNameLast][3] = {
    {0.0897, 0.0855, 0.0803}, {0.1389, 0.0629, 0.1389}, {0.1123, 0.0454, 0.1096}, {0.0267, 0.0129, 0.0254},
    {0.0970, 0.1009, 0.0825}, {0.1273, 0.1172, 0.0807}, {0.0803, 0.0758, 0.0445}, {0.0742, 0.0713, 0.0325},
...
void generate_human36model(RigidBodyDynamics::Model *model)
{
    ...
    Joint free_flyer(SpatialVector(0., 0., 0., 1., 0., 0.), SpatialVector(0., 0., 0., 0., 1., 0.),
                     ...
                     SpatialVector(0., 0., 1., 0., 0., 0.), SpatialVector(1., 0., 0., 0., 0., 0.));

    Joint rot_yxz(SpatialVector(0., 1., 0., 0., 0., 0.), SpatialVector(1., 0., 0., 0., 0., 0.),
    ...
    // trunk
    model->AddBody(pelvis_id, Xtrans(Vector3d(0., 0., SegmentLengths[SegmentPelvis])), rot_yxz, middle_trunk_body,
                   "middletrunk");
    unsigned int uppertrunk_id = model->AppendBody(Xtrans(Vector3d(0., 0., SegmentLengths[SegmentMiddleTrunk])), fixed,
                                                   upper_trunk_body, "uppertrunk");

    // right arm
    model->AddBody(uppertrunk_id, Xtrans(Vector3d(0., -0.1900, SegmentLengths[SegmentUpperTrunk])), rot_yxz,
