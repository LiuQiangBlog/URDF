...
void report_run(const Model &model, const SampleData &data, const char *run_name)
{
    register_run(model, data, run_name);

    if (!json_output)
    {
        cout << "#DOF: " << setw(3) << model.dof_count << " #samples: " << data.count << " duration = " << setw(10)
             << data.durations.sum() << "(s)"
             << " (~" << setw(10) << data.durations.mean() << "(s) per call)" << endl;
    }
}

void report_constraints_run(const Model &model, const SampleData &data, const char *run_name)
{
    register_run(model, data, run_name);
...
double run_forward_dynamics_lagrangian_benchmark(Model *model, int sample_count)
{
    ...
    {
        timer_start(&tinfo);
        ForwardDynamicsLagrangian(*model, sample_data.q[i], sample_data.qdot[i], sample_data.tau[i],
                                  sample_data.qddot[i], Math::LinearSolverPartialPivLU, NULL, &H, &C);
        sample_data.durations[i] = timer_stop(&tinfo);
    }
                                                  ...
                                                  int                sample_count)
{
    ...
    for (int i = 0; i < sample_count; i++)
    {
        InverseDynamicsConstraintsRelaxed(*model, sample_data.q[i], sample_data.qdot[i], sample_data.qddot[i],
                                          *constraint_set, qddot, sample_data.tau[i]);
    }
...
double run_contacts_lagrangian_benchmark(Model *model, ConstraintSet *constraint_set, int sample_count)
{
    ...
    {
        timer_start(&tinfo);
        ForwardDynamicsConstraintsDirect(*model, sample_data.q[i], sample_data.qdot[i], sample_data.tau[i],
...
double run_contacts_lagrangian_sparse_benchmark(Model *model, ConstraintSet *constraint_set, int sample_count)
{
    ...
    {
        timer_start(&tinfo);
        ForwardDynamicsConstraintsRangeSpaceSparse(*model, sample_data.q[i], sample_data.qdot[i], sample_data.tau[i],
                                                   *constraint_set, sample_data.qddot[i]);
        sample_data.durations[i] = timer_stop(&tinfo);
    }
...
double run_contacts_null_space(Model *model, ConstraintSet *constraint_set, int sample_count)
{
    ...
    {
        timer_start(&tinfo);
        ForwardDynamicsConstraintsNullSpace(*model, sample_data.q[i], sample_data.qdot[i], sample_data.tau[i],
                                            *constraint_set, sample_data.qddot[i]);
        sample_data.durations[i] = timer_stop(&tinfo);
    }
...
double run_contacts_kokkevis_benchmark(Model *model, ConstraintSet *constraint_set, int sample_count)
{
    ...
    {
        timer_start(&tinfo);
        ForwardDynamicsContactsKokkevis(*model, sample_data.q[i], sample_data.qdot[i], sample_data.tau[i],
                                        *constraint_set, sample_data.qddot[i]);
        sample_data.durations[i] = timer_stop(&tinfo);
    }
...
double
run_single_inverse_kinematics_benchmark(Model *model, std::vector<InverseKinematicsConstraintSet> &CS, int sample_count)
{
    TimerInfo tinfo;
    timer_start(&tinfo);
