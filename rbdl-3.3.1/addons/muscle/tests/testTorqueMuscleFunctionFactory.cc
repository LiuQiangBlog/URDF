...
TEST_CASE(__FILE__ "_Anderson2007ActiveTorqueAngleCurve", "")
{
    ...
    RigidBodyDynamics::Math::VectorNd curveDomain = andersonTaCurve.getCurveDomain();
    ...
    bool areCurveDerivativesGood =
        areCurveDerivativesCloseToNumericDerivatives(andersonTaCurve, andersonTaCurveSample, TOL_DX);
    CHECK(areCurveDerivativesGood);

    bool curveIsContinuous = isCurveC2Continuous(andersonTaCurve, andersonTaCurveSample, TOL_BIG);
    CHECK(curveIsContinuous);

    if (FLAG_PLOT_CURVES)
    {
        andersonTaCurve.printCurveToCSVFile(FILE_PATH, "anderson2007ActiveTorqueAngleCurve", angleActiveMin,
                                            angleActiveMax);
    }
}
...
TEST_CASE(__FILE__ "_Anderson2007PassiveTorqueAngleCurve", "")
{
    ...
    for (int z = 0; z < 2; ++z)
    {
        ...
        TorqueMuscleFunctionFactory::createAnderson2007PassiveTorqueAngleCurve(
            scale, c1, b1 * curveSign, k1, b2 * curveSign, k2, "test_passiveTorqueAngleCurve", andersonTpCurve);

        RigidBodyDynamics::Math::VectorNd curveDomain = andersonTpCurve.getCurveDomain();
        ...
        // cout << "   Continuity and Smoothness Testing " << endl;
        bool areCurveDerivativesGood =
            areCurveDerivativesCloseToNumericDerivatives(andersonTpCurve, andersonTpCurveSample, TOL_DX);
        CHECK(areCurveDerivativesGood);

        bool curveIsContinuous = isCurveC2Continuous(andersonTpCurve, andersonTpCurveSample, TOL_BIG);
        CHECK(curveIsContinuous);
    ...
    SmoothSegmentedFunction andersonTpCurve = SmoothSegmentedFunction();
    TorqueMuscleFunctionFactory::createAnderson2007PassiveTorqueAngleCurve(
        scale, c1, b1, k1, b2, k2, "test_passiveTorqueAngleCurve", andersonTpCurve);

    if (FLAG_PLOT_CURVES)
    {
        andersonTpCurve.printCurveToCSVFile(FILE_PATH, "anderson2007PassiveTorqueAngleCurve",
...
TEST_CASE(__FILE__ "_Anderson2007ActiveTorqueVelocityCurve", "")
{
    ...
    TorqueMuscleFunctionFactory::createAnderson2007ActiveTorqueVelocityCurve(
        c4, c5, c6, minEccentricMultiplier, maxEccentricMultiplier, "test_anderson2007ActiveTorqueVelocityCurve",
        andersonTvCurve);

    RigidBodyDynamics::Math::VectorNd curveDomain = andersonTvCurve.getCurveDomain();
    ...
    RigidBodyDynamics::Math::MatrixNd andersonTvCurveSample =
        andersonTvCurve.calcSampledCurve(6, angularVelocityMin - 0.1, angularVelocityMax + 0.1);
    ...
    double val = abs(andersonTvCurve.calcValue(angularVelocityMax / angularVelocityMax));
    CHECK(val < TOL_BIG);

    double maxTv = andersonTvCurve.calcValue(angularVelocityMin / angularVelocityMax);
    CHECK(maxTv >= minEccentricMultiplier);
    CHECK(maxTv <= maxEccentricMultiplier);

    CHECK(abs(andersonTvCurve.calcDerivative(angularVelocityMax / angularVelocityMax, 1)) < TOL_SMALL);
    CHECK(abs(andersonTvCurve.calcDerivative(angularVelocityMin / angularVelocityMax, 1)) < TOL_SMALL);

    // cout << "   passed " << endl;
    // cout << "    Continuity and Smoothness Testing" << endl;

    bool areCurveDerivativesGood =
        areCurveDerivativesCloseToNumericDerivatives(andersonTvCurve, andersonTvCurveSample, TOL_DX);

    CHECK(areCurveDerivativesGood);

    bool curveIsContinuous = isCurveC2Continuous(andersonTvCurve, andersonTvCurveSample, TOL_BIG);
    CHECK(curveIsContinuous);
    ...
    if (FLAG_PLOT_CURVES)
    {
        andersonTvCurve.printCurveToCSVFile(FILE_PATH, "anderson2007ActiveTorqueVelocityCurve", angularVelocityMin,
                                            angularVelocityMax);
    }
}
//==============================================================================
TEST_CASE(__FILE__ "_TorqueAngularVelocityCurve", "")
{
    ...
    for (int i = 0; i < npts; ++i)
    {
        ...
        if (w > wmaxC)
        {
            ...
            CHECK(abs(tvX.calcDerivative(w, 1) - slopeAtConcentricOmegaMax) < TOL_BIG);
        }
        else if (w > 0 && w <= wmaxC)
        {
        ...
        else if (w < wmaxE)
        {
            ...
            CHECK(abs(tvX.calcDerivative(w, 1) - slopeAtEccentricOmegaMax) < TOL_SMALL);
        }
    }
    ...
    RigidBodyDynamics::Math::MatrixNd tvCurveSample =
        tv.calcSampledCurve(6, angularVelocityMin - 0.1, angularVelocityMax + 0.1);

    bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(tv, tvCurveSample, TOL_DX);
    ...
    if (FLAG_PLOT_CURVES)
    {
        tv.printCurveToCSVFile(FILE_PATH, "millard2016TorqueVelocityCurve", -1.1, 1.1);
    }
}
...
TEST_CASE(__FILE__ "_PassiveTorqueAngleCurve", "")
{
    ...
    TorqueMuscleFunctionFactory::createPassiveTorqueAngleCurve(angleAtZeroTorque0, angleAtOneNormTorque0, curveName0,
                                                               tp);

    TorqueMuscleFunctionFactory::createPassiveTorqueAngleCurve(angleAtZeroTorque1, angleAtOneNormTorque1,
                                                               stiffnessAtLowTorque1, stiffnessAtOneNormTorque1,
                                                               curviness1, curveName1, tpX);
    ...
    CHECK(abs(tpX.calcDerivative(angleAtOneNormTorque1, 1) - stiffnessAtOneNormTorque1) < TOL_SMALL);
    ...
    RigidBodyDynamics::Math::MatrixNd tpSample0 = tp.calcSampledCurve(6, curveDomain0[0] - 0.1, curveDomain0[1] + 0.1);

    RigidBodyDynamics::Math::MatrixNd tpSample1 = tpX.calcSampledCurve(6, curveDomain1[0] - 0.1, curveDomain1[1] + 0.1);

    bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(tp, tpSample0, TOL_DX);

    CHECK(areCurveDerivativesGood);

    areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(tpX, tpSample1, TOL_DX);
    ...
    if (FLAG_PLOT_CURVES)
    {
        tp.printCurveToCSVFile(FILE_PATH, "millard2016PassiveTorqueAngleCurve", curveDomain0[0] - 0.1,
                               curveDomain0[1] + 0.1);
    }
}
...
TEST_CASE(__FILE__ "_ActiveTorqueAngleCurve", "")
{
    ...
    TorqueMuscleFunctionFactory::createGaussianShapedActiveTorqueAngleCurve(angleAtOneNormTorque,
                                                                            angularStandardDeviation, curveName0, ta);

    TorqueMuscleFunctionFactory::createGaussianShapedActiveTorqueAngleCurve(
        angleAtOneNormTorque, angularStandardDeviation, minSlopeAtShoulders, minValueAtShoulders, curviness, curveName1,
        taX);

    CHECK(abs(ta.calcValue(angleAtOneNormTorque) - 1.0) < TOL_SMALL);

    CHECK(abs(ta.calcValue(angleAtOneNormTorque + 10.0 * angularStandardDeviation)) < TOL_SMALL);
    CHECK(abs(ta.calcValue(angleAtOneNormTorque - 10.0 * angularStandardDeviation)) < TOL_SMALL);

    CHECK(abs(taX.calcValue(angleAtOneNormTorque) - 1.0) < TOL_SMALL);

    double err = abs(taX.calcDerivative(angleAtOneNormTorque + delta, 1) + minSlopeAtShoulders);
    CHECK(err < TOL_SMALL);
    err = abs(taX.calcDerivative(angleAtOneNormTorque - delta, 1) - minSlopeAtShoulders);
    CHECK(err < TOL_SMALL);
    ...
    RigidBodyDynamics::Math::MatrixNd taSample0 = ta.calcSampledCurve(6, curveDomain0[0] - 0.1, curveDomain0[1] + 0.1);

    RigidBodyDynamics::Math::MatrixNd taSample1 = taX.calcSampledCurve(6, curveDomain1[0] - 0.1, curveDomain1[1] + 0.1);

    bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(ta, taSample0, TOL_DX);
    ...
    if (FLAG_PLOT_CURVES)
    {
        ta.printCurveToCSVFile(FILE_PATH, "millard2016ActiveTorqueAngleCurve", curveDomain0[0] - 0.1,
                               curveDomain0[1] + 0.1);
    }
}
...
TEST_CASE(__FILE__ "_TendonTorqueAngleCurve", "")
{
    ...
    TorqueMuscleFunctionFactory::createTendonTorqueAngleCurve(angularStretchAtOneNormTorque, curveName0, tt);

    TorqueMuscleFunctionFactory::createTendonTorqueAngleCurve(angularStretchAtOneNormTorque, stiffnessAtOneNormTorque,
    ...
    CHECK(abs(ttX.calcDerivative(angularStretchAtOneNormTorque, 1) - stiffnessAtOneNormTorque) < TOL_SMALL);
    ...
    RigidBodyDynamics::Math::MatrixNd ttSample0 = tt.calcSampledCurve(6, curveDomain0[0] - 0.1, curveDomain0[1] + 0.1);

    RigidBodyDynamics::Math::MatrixNd ttSample1 = ttX.calcSampledCurve(6, curveDomain1[0] - 0.1, curveDomain1[1] + 0.1);

    bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(tt, ttSample0, TOL_DX);

    CHECK(areCurveDerivativesGood);

    areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(ttX, ttSample1, TOL_DX);
    ...
    if (FLAG_PLOT_CURVES)
    {
        tt.printCurveToCSVFile(FILE_PATH, "millard2016ActiveTorqueAngleCurve", curveDomain0[0] - 0.1,
...
TEST_CASE(__FILE__ "_DampingBlendingCurve", "")
{
    ...
    TorqueMuscleFunctionFactory::createDampingBlendingCurve(-1.0, curveName0, negOne);

    TorqueMuscleFunctionFactory::createDampingBlendingCurve(1.0, curveName0, posOne);
    ...
    RigidBodyDynamics::Math::MatrixNd sampleNegOne =
        negOne.calcSampledCurve(6, curveDomainNegOne[0] - 0.1, curveDomainNegOne[1] + 0.1);

    RigidBodyDynamics::Math::MatrixNd samplePosOne =
        posOne.calcSampledCurve(6, curveDomainPosOne[0] - 0.1, curveDomainPosOne[1] + 0.1);

    bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(negOne, sampleNegOne, TOL_DX);

    CHECK(areCurveDerivativesGood);

    areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(posOne, samplePosOne, TOL_DX);
    ...
    if (FLAG_PLOT_CURVES)
    {
        negOne.printCurveToCSVFile(FILE_PATH, "millard2016DampingBlendingCurve", curveDomainNegOne[0] - 0.1,
