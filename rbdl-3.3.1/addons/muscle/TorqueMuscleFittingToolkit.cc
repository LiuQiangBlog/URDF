                                                           ...
                                                           bool verbose)
{
    ...
    {
        Millard2016TorqueMuscle *mutableTqMcl = const_cast<Millard2016TorqueMuscle *>(&tqMcl);
    ...
    {
        if (fabs(jointAngularVelocity[i]) > fabs(maxConcentricAngularVelocityData))
        {
    ...
    omegaMax = fabs(tqMcl.getMaximumConcentricJointAngularVelocity() * tvOmegaMaxScale);

    // If the optimization run fails, the values from tmf are passed out to
    // the user to help them debug the problem.
    tqMcl.calcTorqueMuscleDataFeatures(jointTorqueSingleSided, jointAngle, jointAngularVelocity, taLambda, tpLambda,
                                       tvLambda, taAngleScaling, taAngleAtOneNormTorque, tpOffset, omegaMax, tiso, tmf);

    Millard2016TorqueMuscle *mutableTqMcl = const_cast<Millard2016TorqueMuscle *>(&tqMcl);

    SmartPtr<FitTorqueMuscleParameters> fittingProblem =
        new FitTorqueMuscleParameters(jointAngle, jointAngularVelocity, jointTorqueSingleSided, activationUpperBound,
                                      passiveTorqueAngleMultiplierUpperBound, taLambda, tvLambda, *mutableTqMcl);
    ...
    if (status == Solve_Succeeded)
    {
        ...
        if (status == Solve_Succeeded)
        {
            ...
            tqMcl.calcTorqueMuscleDataFeatures(jointTorqueSingleSided, jointAngle, jointAngularVelocity, taLambda,
                                               tpLambda, tvLambda, taAngleScaling, taAngleAtOneNormTorque, tpOffset,
                                               omegaMax, tiso, tmf);
        }
    }
    ...
    : mJointAngle(jointAngle), mJointAngularVelocity(jointAngularVelocity), mJointTorque(jointTorque), mTqMcl(tqMcl)
{
    ...
    double tvScale = max(fabs(omegaMin), fabs(omegaMax)) / fabs(mTqMcl.getMaximumConcentricJointAngularVelocity());
    double taScale = fabs(angleMax - angleMin) / mTqMcl.getActiveTorqueAngleCurveWidth();
...
bool FitTorqueMuscleParameters::eval_f(Ipopt::Index n, const Ipopt::Number *x, bool new_x, Ipopt::Number &obj_value)
{
    assert(n == mN);
...
bool FitTorqueMuscleParameters::eval_grad_f(Ipopt::Index n, const Ipopt::Number *x, bool new_x, Ipopt::Number *grad_f)
{
    assert(n == mN);
                                       ...
                                       Ipopt::Number *g)
{
    ...
    for (unsigned int i = mConIdxTauActMaxStart; i <= mConIdxTauActMaxEnd; ++i)
    {
        mTqMcl.updTorqueMuscleSummary(mMaxActivation, mJointAngle[j], mJointAngularVelocity[j], mTaLambda, mTpLambda,
    ...
    for (unsigned int i = mConIdxTauActMinStart; i <= mConIdxTauActMinEnd; ++i)
    {
        mTqMcl.updTorqueMuscleSummary(mMinActivation, mJointAngle[j], mJointAngularVelocity[j], mTaLambda, mTpLambda,
                                      mTvLambda, mTaAngleScale, mTaAngleAtOneNormTorque, mTpAngleOffset, omegaMax,
                                      tauMax, mTms);
                                           ...
                                           Ipopt::Number *values)
{
    ...
    else
    {
        ...
        for (unsigned int i = mConIdxTauActMinStart; i <= mConIdxTauActMinEnd; ++i)
        {
            mTqMcl.updTorqueMuscleInfo(mMinActivation, mJointAngle[j], mJointAngularVelocity[j], mTaLambda, mTpLambda,
                                       mTvLambda, mTaAngleScale, mTaAngleAtOneNormTorque, mTpAngleOffset, omegaMax,
                                       tauMax, mTmi);
...
double FitTorqueMuscleParameters::getSolutionActiveTorqueAngleAngleScaling()
{
    return mTaAngleScale;
}
double FitTorqueMuscleParameters::getSolutionTorqueAngularVelocityOmegaMaxScale()
{
    return mTvOmegaMaxScale;
}
double FitTorqueMuscleParameters::getSolutionPassiveTorqueAngleBlendingParameter()
{
    return mTpLambda;
}
double FitTorqueMuscleParameters::getSolutionPassiveTorqueAngleCurveOffset()
{
    return mTpAngleOffset;
}

double FitTorqueMuscleParameters::getSolutionMaximumActiveIsometricTorqueScale()
{
    return mTauScaling;
}
...
RigidBodyDynamics::Math::VectorNd &FitTorqueMuscleParameters::getConstraintError()
{
