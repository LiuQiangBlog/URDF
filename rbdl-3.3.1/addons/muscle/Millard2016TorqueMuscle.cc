...
const char *AgeGroupSet::names[] = {"Young18To25", "Middle55To65", "SeniorOver65"};
...
const char *Anderson2007::AgeGroupNames[] = {"Young18To25", "Middle55To65", "SeniorOver65"};
      ...
      mSignOfConcentricAnglularVelocity(mSignOfJointTorque), mMuscleName(name), mDataSet(dataSet)
{
    ...
    if (joint == -1 || jointDirection == -1)
    {
        ...
        errormsg << "Millard2016TorqueMuscle::"
                 << "Millard2016TorqueMuscle:" << mMuscleName << ": A jointTorqueDirection of " << jointTorque
                 << " does not exist.";
    ...
    if (abs(abs(mSignOfJointAngle) - 1) > EPSILON)
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "Millard2016TorqueMuscle:" << mMuscleName
                 << ": signOfJointAngleRelativeToAnderson2007 must be [-1,1] not " << mSignOfJointAngle;
        throw RigidBodyDynamics::Errors::RBDLError(errormsg.str());
    }
    ...
    if (abs(abs(mSignOfConcentricAnglularVelocity) - 1) > EPSILON)
    {
        ...
        cerr << "Millard2016TorqueMuscle::"
             << "Millard2016TorqueMuscle:" << mMuscleName << ": signOfJointAngularVelocityDuringConcentricContraction "
             << "must be [-1,1] not " << mSignOfConcentricAnglularVelocity;
        throw RigidBodyDynamics::Errors::RBDLError(errormsg.str());
    }
    ...
    if (abs(abs(mSignOfJointTorque) - 1) > EPSILON)
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "Millard2016TorqueMuscle:" << mMuscleName << ": mSignOfJointTorque must be [-1,1] not "
                 << mSignOfJointTorque;
        throw RigidBodyDynamics::Errors::RBDLError(errormsg.str());
    }
    ...
    if (mSubjectHeightInMeters <= 0)
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "Millard2016TorqueMuscle:" << mMuscleName << ": mSubjectHeightInMeters > 0, but it's "
    ...
    if (mSubjectMassInKg <= 0)
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "Millard2016TorqueMuscle:" << mMuscleName << ": mSubjectMassInKg > 0, but it's "
                 << mSubjectMassInKg;
        throw RigidBodyDynamics::Errors::RBDLError(errormsg.str());
    }
    ...
    case DataSet::Anderson2007:
    {
        ...
        for (int i = 0; i < 36; ++i)
        {
            ...
            if (abs(Anderson2007Table3Mean[i][jointIdx] - (double)joint) < EPSILON &&
                abs(Anderson2007Table3Mean[i][dirIdx] - (double)jointDirection) < EPSILON &&
                abs(Anderson2007Table3Mean[i][genderIdx] - (double)gender) < EPSILON &&
    ...
    default:
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "Millard2016TorqueMuscle:" << mMuscleName << "The requested DataSet does not exist.";
        throw RigidBodyDynamics::Errors::RBDLError(errormsg.str());
    }
    };
    ...
    if (idx == -1)
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "Millard2016TorqueMuscle:" << mMuscleName << "The combination of data set (" << mDataSet << ")"
                 << " joint (" << joint << ")"
...
*/
double Millard2016TorqueMuscle::calcJointTorque(double jointAngle, double jointAngularVelocity, double activation) const
{
    ...
    Millard2016TorqueMuscle *mutableThis = const_cast<Millard2016TorqueMuscle *>(this);
    updTorqueMuscleSummary(activation, jointAngle, jointAngularVelocity, mTaLambda, mTpLambda, mTvLambda,
                           mTaAngleScaling, mAngleAtOneNormActiveTorque, mPassiveCurveAngleOffset, mOmegaMax,
                           mMaxActiveIsometricTorque, mutableThis->mTmSummary);
                                             ...
                                             TorqueMuscleSummary &tms) const
{
    ...
    updInvertTorqueMuscleSummary(jointTorque, jointAngle, jointAngularVelocity, mTaLambda, mTpLambda, mTvLambda,
                                 mTaAngleScaling, mAngleAtOneNormActiveTorque, mPassiveCurveAngleOffset, mOmegaMax,
                                 mMaxActiveIsometricTorque, tms);
}

double Millard2016TorqueMuscle::calcMaximumActiveIsometricTorqueScalingFactor(double jointAngle,
                                                                              double jointAngularVelocity,
                                                                              double activation,
                                                   ...
                                                   TorqueMuscleInfo &tmi) const
{
    ...
    updTorqueMuscleInfo(activation, jointAngle, jointAngularVelocity, mTaLambda, mTpLambda, mTvLambda, mTaAngleScaling,
                        mAngleAtOneNormActiveTorque, mPassiveCurveAngleOffset, mOmegaMax, mMaxActiveIsometricTorque,
                        tmi);
}
...
void Millard2016TorqueMuscle::setNormalizedDampingCoefficient(double betaUpd)
{
    if (betaUpd < 0)
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "setNormalizedDampingCoefficient:" << mMuscleName << "mBetaMax is " << betaUpd
                 << " but mBetaMax must be > 0 " << endl;
...
double Millard2016TorqueMuscle::getMaximumConcentricJointAngularVelocity() const
{
    if (mMuscleCurvesAreDirty)
    {
        Millard2016TorqueMuscle *mutableThis = const_cast<Millard2016TorqueMuscle *>(this);
...
double Millard2016TorqueMuscle::getTorqueVelocityMultiplierAtHalfOmegaMax() const
{
    if (mMuscleCurvesAreDirty)
    {
        Millard2016TorqueMuscle *mutableThis = const_cast<Millard2016TorqueMuscle *>(this);
...
void Millard2016TorqueMuscle::setTorqueVelocityMultiplierAtHalfOmegaMax(double tvAtHalfOmegaMax)
{
    if (mDataSet == DataSet::Anderson2007)
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "setTorqueVelocityMultiplierAtHalfOmegaMax:" << mMuscleName
...
void Millard2016TorqueMuscle::setMaximumConcentricJointAngularVelocity(double maxAngularVelocity)
{
    if (fabs(maxAngularVelocity) < SQRTEPSILON)
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "setMaximumJointAngularVelocity:" << mMuscleName
                 << " The value of maxJointAngularVelocity needs to be greater "
                 << " than sqrt(epsilon), but it is " << maxAngularVelocity;
        throw RigidBodyDynamics::Errors::RBDLInvalidParameterError(errormsg.str());
    }
...
double Millard2016TorqueMuscle::getActiveTorqueAngleCurveWidth() const
{
    if (mMuscleCurvesAreDirty)
    {
        Millard2016TorqueMuscle *mutableThis = const_cast<Millard2016TorqueMuscle *>(this);
...
double Millard2016TorqueMuscle::getJointAngleAtOneNormalizedPassiveIsometricTorque() const
{
    if (mMuscleCurvesAreDirty)
    {
        Millard2016TorqueMuscle *mutableThis = const_cast<Millard2016TorqueMuscle *>(this);
...
double Millard2016TorqueMuscle::getJointAngleAtSmallestNormalizedPassiveIsometricTorque() const
{
    if (mMuscleCurvesAreDirty)
    {
        Millard2016TorqueMuscle *mutableThis = const_cast<Millard2016TorqueMuscle *>(this);
...
void Millard2016TorqueMuscle::fitPassiveCurveAngleOffset(double jointAngleTarget, double passiveFiberTorqueTarget)
{
    mMuscleCurvesAreDirty = true;
    ...
    if (passiveFiberTorqueTarget < SQRTEPSILON)
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "fitPassiveTorqueScale:" << mMuscleName << ": passiveTorque " << passiveFiberTorqueTarget
                 << " but it should be greater than sqrt(eps)" << endl;
        throw RigidBodyDynamics::Errors::RBDLInvalidParameterError(errormsg.str());
    }
    ...
    //                           -currentFiberAngle;
    double fiberAngleOffset = calcFiberAngle(jointAngleTarget) - currentFiberAngle;

    setPassiveCurveAngleOffset(fiberAngleOffset);
    updateTorqueMuscleCurves();
}

void Millard2016TorqueMuscle::fitPassiveTorqueScale(double jointAngleTarget, double passiveFiberTorqueTarget)
{
    mMuscleCurvesAreDirty = true;
    ...
    double passiveTorqueScale = normPassiveFiberTorqueTarget / normPassiveFiberTorqueCurrent;
    ...
    TorqueMuscleDataFeatures                &tmf) const
{
    ...
    for (unsigned int i = 0; i < jointAngle.rows(); ++i)
    {
        ...
        updInvertTorqueMuscleSummary(jointTorque[i], jointAngle[i], jointAngularVelocity[i],
                                     activeTorqueAngleBlendingVariable, passiveTorqueAngleBlendingVariable,
                                     torqueVelocityBlendingVariable, activeTorqueAngleAngleScaling,
...
const SmoothSegmentedFunction &Millard2016TorqueMuscle::getActiveTorqueAngleCurve() const
{
...
const SmoothSegmentedFunction &Millard2016TorqueMuscle::getPassiveTorqueAngleCurve() const
{
    // This must be updated if the parameters have changed
    if (mMuscleCurvesAreDirty)
    {
...
const SmoothSegmentedFunction &Millard2016TorqueMuscle::getTorqueAngularVelocityCurve() const
{
    // This must be updated if the parameters have changed
    if (mMuscleCurvesAreDirty)
    {
...
double Millard2016TorqueMuscle::getPassiveTorqueAngleCurveBlendingVariable() const
{
    return mTpLambda;
}

double Millard2016TorqueMuscle::getTorqueAngularVelocityCurveBlendingVariable() const
{
    return mTvLambda;
}
...
void Millard2016TorqueMuscle::setActiveTorqueAngleCurveAngleScaling(double angleScaling)
{
    if (angleScaling < SQRTEPSILON)
    {
        ostringstream errormsg;
        cerr << "Millard2016TorqueMuscle::"
             << "setActiveTorqueAngleCurveAngleScaling:" << mMuscleName << ": angleScaling must be > sqrt(eps), this "
             << angleScaling << " is outside the acceptable range." << endl;
        throw RigidBodyDynamics::Errors::RBDLInvalidParameterError(errormsg.str());
    }
...
void Millard2016TorqueMuscle::setActiveTorqueAngleCurveBlendingVariable(double blendingVariable)
{
    if (blendingVariable < 0. || blendingVariable > 1.0)
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "setActiveTorqueAngleCurveBlendingVariable:" << mMuscleName
                 << ": blending variable must be [0,1] and this " << blendingVariable
                 << " is outside the acceptable range." << endl;
        throw RigidBodyDynamics::Errors::RBDLInvalidParameterError(errormsg.str());
    }
    mMuscleCurvesAreDirty = true;
...
void Millard2016TorqueMuscle::setPassiveTorqueAngleCurveBlendingVariable(double blendingVariable)
{
    if (blendingVariable < 0. || blendingVariable > 1.0)
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "setPassiveTorqueAngleCurveBlendingVariable:" << mMuscleName
                 << ": blending variable must be [0,1] and this " << blendingVariable
                 << " is outside the acceptable range." << endl;
        throw RigidBodyDynamics::Errors::RBDLInvalidParameterError(errormsg.str());
    }
...
void Millard2016TorqueMuscle::setTorqueAngularVelocityCurveBlendingVariable(double blendingVariable)
{
    ...
    if (blendingVariable < 0. || blendingVariable > 1.0)
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "setTorqueAngularVelocityCurveBlendingVariable:" << mMuscleName
                 << ": blending variable must be [0,1] and this " << blendingVariable
                 << " is outside the acceptable range." << endl;
        throw RigidBodyDynamics::Errors::RBDLInvalidParameterError(errormsg.str());
    }
...
void Millard2016TorqueMuscle::setFittedParameters(const TorqueMuscleParameterFittingData &fittedParameters)
{
    ...
    if (!fittedParameters.fittingConverged)
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "setTorqueMuscleParameters:" << mMuscleName
                 << ": The fittingConverged field of fittedParameters is false! " << endl;
        throw RigidBodyDynamics::Errors::RBDLInvalidParameterError(errormsg.str());
    }

    setPassiveTorqueAngleCurveBlendingVariable(fittedParameters.passiveTorqueAngleBlendingVariable);
    setActiveTorqueAngleCurveBlendingVariable(fittedParameters.activeTorqueAngleBlendingVariable);
    setTorqueAngularVelocityCurveBlendingVariable(fittedParameters.torqueVelocityBlendingVariable);
    setPassiveCurveAngleOffset(fittedParameters.passiveTorqueAngleCurveOffset);
    setMaximumActiveIsometricTorque(fittedParameters.maximumActiveIsometricTorque);
    setMaximumConcentricJointAngularVelocity(fittedParameters.maximumAngularVelocity);
    setActiveTorqueAngleCurveAngleScaling(fittedParameters.activeTorqueAngleAngleScaling);
}
...
double Millard2016TorqueMuscle::calcFiberAngularVelocity(double jointAngularVelocity) const
{
    return mSignOfConcentricAnglularVelocity * jointAngularVelocity;
}

double Millard2016TorqueMuscle::calcJointAngularVelocity(double fiberAngularVelocity) const
{
...
void Millard2016TorqueMuscle::updateTorqueMuscleCurves()
{
    ...
    case DataSet::Anderson2007:
    {
        ...
        mScaleFactorAnderson2007 = mSubjectHeightInMeters * mSubjectMassInKg * Gravity;

        if (mUseTabularMaxActiveIsometricTorque)
        {
            mMaxActiveIsometricTorque = mScaleFactorAnderson2007 * mAnderson2007c1c2c3c4c5c6[0];
        }
        ...
        TorqueMuscleFunctionFactory::createAnderson2007ActiveTorqueAngleCurve(
            mAnderson2007c1c2c3c4c5c6[1], mAnderson2007c1c2c3c4c5c6[2], tempName.append("_taCurve"), mTaCurve);

        tempName = mMuscleName;

        TorqueMuscleFunctionFactory::createAnderson2007ActiveTorqueVelocityCurve(
            mAnderson2007c1c2c3c4c5c6[3], mAnderson2007c1c2c3c4c5c6[4], mAnderson2007c1c2c3c4c5c6[5], 1.1, 1.4,
            tempName.append("_tvCurve"), mTvCurve);

        mTorqueVelocityMultiplierAtHalfOmegaMax = mTvCurve.calcValue(0.5);

        tempName = mMuscleName;

        double normMaxActiveIsometricTorque = mMaxActiveIsometricTorque / mScaleFactorAnderson2007;

        TorqueMuscleFunctionFactory::createAnderson2007PassiveTorqueAngleCurve(
            mScaleFactorAnderson2007, normMaxActiveIsometricTorque, mAnderson2007b1k1b2k2[0], mAnderson2007b1k1b2k2[1],
            mAnderson2007b1k1b2k2[2], mAnderson2007b1k1b2k2[3], tempName.append("_tpCurve"), mTpCurve);
        ...
        {
            mAngleAtSmallestNormPassiveTorque = std::numeric_limits<double>::signaling_NaN();
        }
        ...
        if (fabs(tpAtX0) > SQRTEPSILON || fabs(tpAtX1) > SQRTEPSILON)
        {
            ...
            mAngleAtOneNormPassiveTorque =
                calcInverseBlendedCurveValue(1.0, argGuess, mTpLambda, mTpLambdaMax, mTpCurve);
            mAngleAtOneNormPassiveTorque += mPassiveCurveAngleOffset;
        }
        else
        {
            mAngleAtOneNormPassiveTorque = std::numeric_limits<double>::signaling_NaN();
        }
    ...
    case DataSet::Gymnast:
    {
        if (mUseTabularOmegaMax)
        {
            mOmegaMax = mGymnastParams[Gymnast::OmegaMax];
        }
        if (mUseTabularMaxActiveIsometricTorque)
        {
            mMaxActiveIsometricTorque = mGymnastParams[Gymnast::TauMax];
        }
        mAngleAtOneNormActiveTorque = mGymnastParams[Gymnast::ActiveAngleAtOneNormTorque];

        TorqueMuscleFunctionFactory::createGaussianShapedActiveTorqueAngleCurve(
            mGymnastParams[Gymnast::ActiveAngleAtOneNormTorque],
            mGymnastParams[Gymnast::ActiveAngularStandardDeviation], tempName.append("_taCurve"), mTaCurve);

        TorqueMuscleFunctionFactory::createPassiveTorqueAngleCurve(mGymnastParams[Gymnast::PassiveAngleAtZeroTorque],
                                                                   mGymnastParams[Gymnast::PassiveAngleAtOneNormTorque],
                                                                   tempName.append("_tpCurve"), mTpCurve);
        ...
        {
            mTorqueVelocityMultiplierAtHalfOmegaMax = mGymnastParams[Gymnast::TvAtHalfMaxConcentricVelocity];
        }

        TorqueMuscleFunctionFactory::createTorqueVelocityCurve(mGymnastParams[Gymnast::TvAtMaxEccentricVelocity],
        ...
        mAngleAtSmallestNormPassiveTorque =
            mGymnastParams[Gymnast::PassiveAngleAtZeroTorque] + mPassiveCurveAngleOffset;

        // Verify that the curve hits zero at this angle
        if (fabs(mTpCurve.calcValue(mAngleAtSmallestNormPassiveTorque)) > SQRTEPSILON)
        {
            mAngleAtSmallestNormPassiveTorque = numeric_limits<double>::signaling_NaN();
        }
    ...
    default:
    {
        ostringstream errormsg;
        errormsg << "Millard2016TorqueMuscle::"
                 << "Millard2016TorqueMuscle:" << mMuscleName << "mDataSet has a value of " << mDataSet
    ...
    RigidBodyDynamics::Addons::Geometry::SmoothSegmentedFunction const &curve) const
{
    ...
    case 0:
    {
        ...
        case 0:
        {
            output = (1. - blendingVariable) * curve.calcValue(curveArgument) + blendingVariable * maximumBlendingValue;
        }
        break;
        case 1:
        {
...
double Millard2016TorqueMuscle::calcFiberAngleGivenNormalizedPassiveTorque(double normPassiveFiberTorque,
                                                                           double blendingVariable,
...
void Millard2016TorqueMuscle::updTorqueMuscleSummaryCurveValues(double fiberAngle,
                                                                ...
                                                                TorqueMuscleSummary &updTms) const
{
    ...
    double taAngle = (fiberAngle - activeTorqueAngleAtOneNormTorque) / activeTorqueAngleAngleScaling +
                     activeTorqueAngleAtOneNormTorque;

    updTms.fiberActiveTorqueAngleMultiplier =
        calcBlendedCurveDerivative(taAngle, activeTorqueAngleBlendingVariable, mTaLambdaMax, 0, 0, mTaCurve);

    updTms.fiberPassiveTorqueAngleMultiplier = calcBlendedCurveDerivative(
        fiberAngle - passiveTorqueAngleCurveOffset, passiveTorqueAngleBlendingVariable, mTpLambdaMax, 0, 0, mTpCurve);

    updTms.fiberTorqueAngularVelocityMultiplier = calcBlendedCurveDerivative(
        normFiberAngularVelocity, torqueAngularVelocityBlendingVariable, mTvLambdaMax, 0, 0, mTvCurve);
                                                  ...
                                                  TorqueMuscleInfo &updTmi) const
{
    ...
    double D_taAngle_D_angleScaling = -(fiberAngle - activeTorqueAngleAtOneNormTorque) /
                                      (activeTorqueAngleAngleScaling * activeTorqueAngleAngleScaling);

    double D2_taAngle_D_angleScaling2 = -2.0 * D_taAngle_D_angleScaling / activeTorqueAngleAngleScaling;

    // Update force component values
    double ta = calcBlendedCurveDerivative(taAngle, activeTorqueAngleBlendingVariable, mTaLambdaMax, 0, 0, mTaCurve);

    double tp = calcBlendedCurveDerivative(fiberAngle - passiveTorqueAngleCurveOffset,
                                           passiveTorqueAngleBlendingVariable, mTpLambdaMax, 0, 0, mTpCurve);

    double tv =
        calcBlendedCurveDerivative(omegaNorm, torqueAngularVelocityBlendingVariable, mTvLambdaMax, 0, 0, mTvCurve);
    ...
    double D_ta_D_taAngle =
        calcBlendedCurveDerivative(taAngle, activeTorqueAngleBlendingVariable, mTaLambdaMax, 1, 0, mTaCurve);
    ...
    double D_tp_D_fiberAngle = calcBlendedCurveDerivative(
        fiberAngle - passiveTorqueAngleCurveOffset, passiveTorqueAngleBlendingVariable, mTpLambdaMax, 1, 0, mTpCurve);
    ...
    double D_ta_D_taLambda =
        calcBlendedCurveDerivative(taAngle, activeTorqueAngleBlendingVariable, mTaLambdaMax, 0, 1, mTaCurve);

    double D_tp_D_tpLambda = calcBlendedCurveDerivative(
        fiberAngle - passiveTorqueAngleCurveOffset, passiveTorqueAngleBlendingVariable, mTpLambdaMax, 0, 1, mTpCurve);
    double D_tv_D_tvLambda =
        calcBlendedCurveDerivative(omegaNorm, torqueAngularVelocityBlendingVariable, mTvLambdaMax, 0, 1, mTvCurve);

    // 2nd derivatives w.r.t fitting-related variables.
    double D2_ta_D_taLambda2 =
        calcBlendedCurveDerivative(taAngle, activeTorqueAngleBlendingVariable, mTaLambdaMax, 0, 2, mTaCurve);

    double D2_tv_D_tvLambda2 =
        calcBlendedCurveDerivative(omegaNorm, torqueAngularVelocityBlendingVariable, mTvLambdaMax, 0, 2, mTvCurve);
    double D2_tp_D_tpLambda2 = calcBlendedCurveDerivative(
        fiberAngle - passiveTorqueAngleCurveOffset, passiveTorqueAngleBlendingVariable, mTpLambdaMax, 0, 2, mTpCurve);

    double D2_tp_D_fiberAngle2 = calcBlendedCurveDerivative(
        fiberAngle - passiveTorqueAngleCurveOffset, passiveTorqueAngleBlendingVariable, mTpLambdaMax, 2, 0, mTpCurve);

    double D2_ta_D_taAngle2 =
        calcBlendedCurveDerivative(taAngle, activeTorqueAngleBlendingVariable, mTaLambdaMax, 2, 0, mTaCurve);

    double D2_ta_D_angleScaling2 = D2_ta_D_taAngle2 * D_taAngle_D_angleScaling * D_taAngle_D_angleScaling +
    ...
    double D2_tp_D_tpLambda_D_fiberAngle = calcBlendedCurveDerivative(
        fiberAngle - passiveTorqueAngleCurveOffset, passiveTorqueAngleBlendingVariable, mTpLambdaMax, 1, 1, mTpCurve);
    ...
    updTmi.fiberStiffness =
        maxActIsoTorque * (activation * D_ta_D_fiberAngle * tv + D_tp_D_fiberAngle + D_tb_D_fiberAngle);

    updTmi.jointStiffness = mSignOfJointTorque * updTmi.fiberStiffness * D_fiberAngle_D_jointAngle;

    updTmi.fiberActivePower = updTmi.fiberActiveTorque * updTmi.fiberAngularVelocity;

    updTmi.fiberPassivePower = updTmi.fiberPassiveTorque * updTmi.fiberAngularVelocity;

    updTmi.fiberPower = updTmi.fiberActivePower + updTmi.fiberPassivePower;
    ...
    updTmi.DjointTorque_Dactivation = mSignOfJointTorque * maxActIsoTorque * (ta * tv);

    // tau = signTq*tauIso*(a*ta(theta)*tv(thetaDot) + tp(theta)*(1-beta*omegaNorm)
    // Dtau_Dtheta =   signTq*tauIso*(a*Dta_Dtheta(theta)*tv(thetaDot)
    //                           + Dtp_Dtheta(theta)*(1-beta*omegaNorm)
    updTmi.DjointTorque_DjointAngle = mSignOfJointTorque * maxActIsoTorque *
                                      (activation * D_ta_D_fiberAngle * tv + (D_tp_D_fiberAngle + D_tb_D_fiberAngle)) *
    ...
    updTmi.DjointTorque_DjointAngularVelocity =
        mSignOfJointTorque * maxActIsoTorque *
        (activation * ta * (D_tv_D_fiberAngularVelocity * D_fiberAngularVelocity_D_jointAngularVelocity) +
         (D_tb_D_fiberAngularVelocity * D_fiberAngularVelocity_D_jointAngularVelocity));

    // tau = signTq*tauIso*(a*ta(theta)*tv(thetaDot) + tp(theta)*(1-beta*omegaNorm)
    // Dtau_DtaLambda = signTq*tauIso*(a * Dta(theta)_Dlambda * tv(thetaDot)
    updTmi.DjointTorque_DactiveTorqueAngleBlendingVariable =
        mSignOfJointTorque * maxActIsoTorque * (activation * D_ta_D_taLambda * tv);
    ...
    updTmi.DjointTorque_DpassiveTorqueAngleBlendingVariable =
        mSignOfJointTorque * maxActIsoTorque * (D_tp_D_tpLambda + D_tb_D_tpLambda);

    // tau = signTq*tauIso*(a*ta(theta)*tv(thetaDot) + tp(theta)*(1-beta*omegaNorm)
    // Dtau_tvLambda = signTq*tauIso*(a*ta(theta)*Dtv_Dlambda(thetaDot)
    updTmi.DjointTorque_DtorqueAngularVelocityBlendingVariable =
        mSignOfJointTorque * maxActIsoTorque * (activation * ta * D_tv_D_tvLambda);

    // mSignOfJointTorque*( (maxActIsoTorque*(activation*ta*tv)
    //                    + maxActIsoTorque*(tp+tb)))
    updTmi.DjointTorque_DmaximumIsometricTorque = mSignOfJointTorque * ((activation * ta * tv) + (tp + tb));

    updTmi.DjointTorque_DpassiveTorqueAngleCurveAngleOffset =
        mSignOfJointTorque * maxActIsoTorque * (D_tp_D_fiberAngleOffset + D_tb_D_fiberAngleOffset);
    //* D_fiberAngle_D_jointAngle;
    ...
    updTmi.DjointTorque_DmaximumAngularVelocity =
        mSignOfJointTorque * maxActIsoTorque * (activation * ta * D_tv_D_omegaMax + D_tb_D_omegaMax);
    ...
    updTmi.DjointTorque_DactiveTorqueAngleAngleScaling =
        mSignOfJointTorque * maxActIsoTorque * (activation * D_ta_D_angleScaling * tv);
    ...
    */
    ...
    // 0: d2t/da2
    updTmi.fittingInfo[0] = mSignOfJointTorque * (maxActIsoTorque * (activation * D2_ta_D_taLambda2 * tv));

    // 1: d2t/dadv
    updTmi.fittingInfo[1] = mSignOfJointTorque * (maxActIsoTorque * (activation * D_ta_D_taLambda * D_tv_D_tvLambda));

    // 2: d2t/dv2
    updTmi.fittingInfo[2] = mSignOfJointTorque * (maxActIsoTorque * (activation * ta * D2_tv_D_tvLambda2));
    ...
    // 5: d2t/dp2
    updTmi.fittingInfo[5] = mSignOfJointTorque * (maxActIsoTorque * (D2_tp_D_tpLambda2 + D2_tb_D_tpLambda2));
    // 6: d2t/dado
    updTmi.fittingInfo[6] = 0.;
    // 7: d2t/dvdo
    updTmi.fittingInfo[7] = 0.;
    // 8: d2t/dpdo
    updTmi.fittingInfo[8] =
        mSignOfJointTorque * (maxActIsoTorque * (D2_tp_D_tpLambda_D_fiberAngleOffset +
                                                 D2_tb_D_tpLambda_D_fiberAngleOffset)); //*D_fiberAngle_D_jointAngle);
    // 9: d2t/do2
    updTmi.fittingInfo[9] =
        mSignOfJointTorque *
        (maxActIsoTorque * (D2_tp_D_fiberAngleOffset2 + D2_tb_D_fiberAngleOffset2)); //*D_fiberAngle_D_jointAngle);
    // 10: d2tp/dp2
    ...
    // 13: d2t/ds2
    updTmi.fittingInfo[13] = mSignOfJointTorque * maxActIsoTorque * (activation * D2_ta_D_angleScaling2 * tv);
    // 14: d2t/dsdm
    updTmi.fittingInfo[14] =
        mSignOfJointTorque * maxActIsoTorque * (activation * D_ta_D_angleScaling * D_tv_D_omegaMax);

    // 15: d2t/dm2
    updTmi.fittingInfo[15] =
        mSignOfJointTorque * maxActIsoTorque * (activation * ta * D2_tv_D_omegaMax2 + D2_tb_D_omegaMax2);
    // 16: d2t/dsdp
    updTmi.fittingInfo[16] = 0.;

    // 17: d2t/dmdp
    updTmi.fittingInfo[17] = mSignOfJointTorque * maxActIsoTorque * (D2_tb_D_omegaMax_D_tpLambda);
    // 18: d2t/dp2
    updTmi.fittingInfo[18] = mSignOfJointTorque * (maxActIsoTorque * (D2_tp_D_tpLambda2 + D2_tb_D_tpLambda2));
    // 19: d2t/dsdo
    updTmi.fittingInfo[19] = 0.;

    // 20: d2t/dmdo
    updTmi.fittingInfo[20] = mSignOfJointTorque * maxActIsoTorque * (D2_tb_D_omegaMax_D_tpOffset);

    // 21: d2t/dpdo
    updTmi.fittingInfo[21] =
        mSignOfJointTorque *
        (maxActIsoTorque * (D2_tp_D_tpLambda_D_fiberAngleOffset + D2_tb_D_tpLambda_D_fiberAngleOffset));

    // 22: d2t/do2
    updTmi.fittingInfo[22] =
        mSignOfJointTorque * (maxActIsoTorque * (D2_tp_D_fiberAngleOffset2 + D2_tb_D_fiberAngleOffset2));
                                                           ...
                                                           TorqueMuscleSummary &updTms) const
{
    ...
    updTms.activation = ((updTms.fiberTorque / maxActIsoTorque) -
                         (updTms.fiberPassiveTorqueAngleMultiplier + updTms.fiberNormalizedDampingTorque)) /
