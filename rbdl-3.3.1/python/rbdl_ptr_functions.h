...
RBDL_DLLAPI
void UpdateKinematicsCustomPtr(Model &model, const double *q_ptr, const double *qdot_ptr, const double *qddot_ptr)
{
                          ...
                          bool                  update_kinematics)
{
    ...
    SpatialTransform point_trans =
        SpatialTransform(Matrix3d::Identity(), CalcBodyToBaseCoordinates(model, Q, body_id, point_position, false));
        ...
        if (model.mJoints[j].mJointType != JointTypeCustom)
        {
            ...
            else if (model.mJoints[j].mDoFCount == 3)
            {
                G.block(0, q_index, 3, 3) =
                    ((point_trans * model.X_base[j].inverse()).toMatrix() * model.multdof3_S[j]).block(3, 0, 3, 3);
            }
        }
        else
        {
            ...
            G.block(0, q_index, 3, model.mCustomJoints[k]->mDoFCount) =
                ((point_trans * model.X_base[j].inverse()).toMatrix() * model.mCustomJoints[k]->S)
                        ...
                        std::vector<Math::SpatialVector> *f_ext)
{
    ...
    for (unsigned int i = 1; i < model.mBodies.size(); i++)
    {
        ...
        else if (model.mJoints[i].mJointType == JointTypeCustom)
        {
            ...
            model.a[i] =
                model.X_lambda[i].apply(model.a[lambda]) + model.c[i] + model.mCustomJoints[k]->S * customJointQDDot;
        }
    ...
    for (unsigned int i = model.mBodies.size() - 1; i > 0; i--)
    {
        ...
        {
            unsigned int k = model.mJoints[i].custom_joint_index;
            Tau.block(model.mJoints[i].q_index, 0, model.mCustomJoints[k]->mDoFCount, 1) =
...
RBDL_DLLAPI
void NonlinearEffectsPtr(Model &model, const double *q_ptr, const double *qdot_ptr, const double *tau_ptr)
{
    LOG << "-------- " << __func__ << " --------" << std::endl;
...
RBDL_DLLAPI
inline void
CompositeRigidBodyAlgorithmPtr(Model &model, const double *q_ptr, double *H_ptr, bool update_kinematics = true)
{
    using namespace RigidBodyDynamics::Math;
                        ...
                        std::vector<Math::SpatialVector> *f_ext)
{
    ...
    for (i = model.mBodies.size() - 1; i > 0; i--)
    {
        ...
        else if (model.mJoints[i].mJointType == JointTypeCustom)
        {
            ...
            std::cout << "S*U: " << model.mCustomJoints[kI]->S.transpose() * model.mCustomJoints[kI]->U << std::endl;

            model.mCustomJoints[kI]->Dinv =
                (model.mCustomJoints[kI]->S.transpose() * model.mCustomJoints[kI]->U).inverse().eval();
                                      ...
                                      unsigned int                       max_iter)
{
    ...
    for (unsigned int ik_iter = 0; ik_iter < max_iter; ik_iter++)
    {
        ...
        MatrixNd JJTe_lambda2_I = J * J.transpose() + dlambda * dlambda * MatrixNd::Identity(e.size(), e.size());
                                  ...
                                  bool            update_kinematics)
{
    ...
    assert((com_acceleration == NULL && change_of_angular_momentum == NULL) || (qddot_ptr != NULL));
