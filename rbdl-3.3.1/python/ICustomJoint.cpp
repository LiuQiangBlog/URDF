...
void ICustomJoint::jcalc(Model &model, unsigned int joint_id, const Math::VectorNd &q, const Math::VectorNd &qdot)
{
    if (this->m_obj)
    {
        int error;
...
void ICustomJoint::jcalc_X_lambda_S(Model &model, unsigned int joint_id, const Math::VectorNd &q)
{
    if (this->m_obj)
    {
        int error;
