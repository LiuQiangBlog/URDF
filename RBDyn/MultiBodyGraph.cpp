                   ...
                   int s, int par, const sva::PTransformd &Xti, const sva::PTransformd &Xbase)
    {
        ...
        for (Arc &a : curNode->arcs)
        {
            if (a.next != fromNode)
            {
                ...
                makeTree(a.next, curNode, a.joint, curInd, nextInd, curInd, a.X * XFrom.inv(),
...
MultiBodyGraph
MultiBodyGraph::fixJoints(const MultiBodyGraph &other, const std::vector<std::string> &jointsToFix, bool fixAllJoints)
{
    ...
    if (fixAllJoints)
    {
        for (auto &jointPtr : mbg.joints_)
        {
            ...
            {
                jointPtr.reset(
                    new Joint(Joint::Type::Fixed, Eigen::Vector3d::Zero(), jointPtr->forward(), jointPtr->name()));
            }
        }
    }
    else
    ...
    for (auto &node : mbg.nodes_)
    {
        ...
        for (auto &arc : node->arcs)
        {
            ...
            for (const auto &n : jointsToFix)
            {
                ...
                {
                    arc.joint =
                        Joint(Joint::Type::Fixed, Eigen::Vector3d::Zero(), arc.joint.forward(), arc.joint.name());
                    break;
                }
            }
        }
        ...
        if (it->joint.name() != parentJointName)
        {
            if (it->joint.name() == jointName)
            {
                ...
                sva::RBInertiad newInertia =
                    mergeInertia(node.body.inertia(), mergeSubNodes(*it->next, jointName, jointPosByName), it->joint,
                                 it->X, jointPosByName);
