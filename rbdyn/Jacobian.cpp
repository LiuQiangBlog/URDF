...
Jacobian::jacobian(const MultiBody &mb, const MultiBodyConfig &mbc, const sva::PTransformd &X_0_p)
{
...
const Eigen::MatrixXd &
Jacobian::vectorJacobian(const MultiBody &mb, const MultiBodyConfig &mbc, const Eigen::Vector3d &vector)
{
    const std::vector<Joint> &joints = mb.joints();
...
const Eigen::MatrixXd &
Jacobian::vectorBodyJacobian(const MultiBody &mb, const MultiBodyConfig &mbc, const Eigen::Vector3d &vector)
{
    const std::vector<Joint> &joints = mb.joints();
...
sva::MotionVecd
Jacobian::velocity(const MultiBody & /* mb */, const MultiBodyConfig &mbc, const sva::PTransformd &X_b_p) const
{
    const auto N = static_cast<size_t>(jointsPath_.back());
...
void Jacobian::expandAdd(const MultiBody &mb, const Eigen::Ref<const Eigen::MatrixXd> &jac, Eigen::MatrixXd &res) const
{
...
const Eigen::MatrixXd &
Jacobian::sJacobian(const MultiBody &mb, const MultiBodyConfig &mbc, const sva::PTransformd &X_0_p)
{
...
const Eigen::MatrixXd &
Jacobian::sVectorJacobian(const MultiBody &mb, const MultiBodyConfig &mbc, const Eigen::Vector3d &vec)
{
    checkMatchBodyPos(mb, mbc);
    checkMatchMotionSubspace(mb, mbc);
...
const Eigen::MatrixXd &
Jacobian::sVectorBodyJacobian(const MultiBody &mb, const MultiBodyConfig &mbc, const Eigen::Vector3d &vec)
{
    checkMatchBodyPos(mb, mbc);
    checkMatchMotionSubspace(mb, mbc);
...
sva::MotionVecd
Jacobian::sVelocity(const MultiBody &mb, const MultiBodyConfig &mbc, const sva::PTransformd &X_b_p) const
{
    checkMatchBodyPos(mb, mbc);
    checkMatchBodyVel(mb, mbc);
