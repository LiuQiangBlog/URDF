...
void InverseStatics::inverseStatics(const MultiBody &mb, MultiBodyConfig &mbc)
{
    ...
    for (int i = static_cast<int>(joints.size()) - 1; i >= 0; --i)
    {
        ...
        VectorXd::Map(mbc.jointTorque[ui].data(), joints[ui].dof()) =
            f_[ui].vector().transpose() * mbc.motionSubspace[ui];

        if (pred[ui] != -1)
            f_[static_cast<size_t>(pred[ui])] += mbc.parentToSon[ui].transMul(f_[ui]);
    }
}
